...
| AuditRequest | class（DTO） | 处方审核请求值对象，对齐需求文档 3.4.2 输入契约。包含处方药品列表（prescriptionItems，每项含 drugId/drugName/dose/unit/frequency/duration/route 七个字段，其中 unit 为可选 String 字段，用于单位一致性校验和剂量超限判定；与 DosageCheckRequest.unit 业务角色一致——用于 DosageUnitGroup 分组换算）、患者个体信息（patientInfo，含 patientId/age/gender/allergyHistory（string，过敏史文本）/allergyDetails（List\<AllergyDetail\>，可选扩展容器）/comorbidities（List\<String\>，合并症列表））、处方标识（prescriptionId，必填） |
| PrescriptionItem | class（DTO） | 处方药品条目值对象，复用于 AuditRequest.prescriptionItems、SubmitRequest.prescriptionItems、PrescriptionAssistRequest.existingPrescription.drugs、PrescriptionAssistResponse.prescriptionDraft.drugs 四个端点。包含 drugId（String，药品编码）、drugName（String，药品名称）、dose（BigDecimal，剂量数值）、unit（String，可选——剂量单位，值域限定为 DosageUnitGroup 枚举中各分组的单位值如 mg/g/ml/IU；不传时按业务约定回退）、frequency（String，用药频率如 tid/bid）、duration（Integer，疗程天数）、route（String，给药途径如 po/iv） |
...
| SubmitRequest | class（DTO） | 处方提交请求值对象，对齐处方提交端点输入契约。包含处方标识（prescriptionId，String，必填）、处方药品列表（prescriptionItems，List\<PrescriptionItem\>，每项含 drugId/drugName/dose/unit/frequency/duration/route 七个字段，其中 unit 为可选 String 字段，用于单位一致性校验和剂量超限判定）、强制提交标记（forceSubmit，boolean，默认 false）、审核记录标识（auditRecordId，Long，可选——forceSubmit=true 时必填，用于定位 WARN 审核记录；forceSubmit=false 时可选；无审核结果或 PASS 场景不传） |
| SubmitResponse | class（DTO） | 处方提交响应值对象，包含提交结果状态（submitted，boolean）、处方单号（prescriptionOrderId，String，提交成功时填充）、阻断信息（blockInfo，BlockResponse，提交被阻断时填充）、错误码（errorCode，String，可选——如 RX_AUDIT_PRESCRIPTION_MODIFIED、RX_AUDIT_CONCURRENT_SUBMIT）、WARN 结果（warnResult，WarnResult，可选——步③ forceSubmit=false + 最新审核为 WARN + 处方未变更路径下填充，承载风险等级/告警列表/审核记录标识/处方哈希，供前端展示当前审核结果并提示医生选择强制提交或修改处方；errorCode=null） |
| WarnResult | class（DTO） | WARN 级审核结果摘要值对象，承载 SubmitResponse.warnResult。包含 riskLevel（AuditRiskLevel 枚举，必填——值域为 WARN）、alerts（List\<WarnAlert\>，必填——WARN 级告警列表）、auditRecordId（Long，必填——对应 WARN 级 AuditRecord.auditId，供前端确认时回传 forceSubmit=true 场景使用）、prescriptionHash（String，必填——原始处方结构化哈希，供前端后续 forceSubmit 路径的版本校验参考） |
| WarnAlert | class（DTO） | WARN 告警条目值对象，承载 WarnResult.alerts 数组每项结构。包含 alertCode（String，必填——告警编码）、alertMessage（String，必填——告警消息）、severity（AlertSeverity 枚举，必填——值域为 WARNING，与 AuditAlert 的 severity 字段共用 AlertSeverity 枚举） |
...
| PrescriptionAssistResponse | class（DTO） | AI 辅助开方主端点响应值对象，对齐需求文档 3.4.10 输出契约。新增 prescriptionId 可选回写字段（String，后端自动生成时填充）包含处方草案（prescriptionDraft，含 drugs 数组，每项含 drugId/drugName/dose/unit/frequency/duration/route 七个字段，其中 unit 为可选 String 字段，用于单位一致性校验和剂量超限判定）、剂量告警列表（doseWarnings，List\<DoseWarning\>，每项含 drugId/warningType/message/severity）、过敏冲突告警（allergyWarnings，List\<AllergyWarningItem\>，每项含 drugId/allergen/severity）、顶层错误码（errorCode，可选，String——AI 返回无可推荐药品时填充 RX_ASSIST_AI_NO_RECOMMENDATION）、免责声明标记（disclaimerRequired） |
...
职责：封装处方审核请求值对象，对齐需求文档 3.4.2 输入契约。包含处方药品列表（prescriptionItems，List\<PrescriptionItem\>，每项含 drugId/drugName/dose/unit/frequency/duration/route 七个字段，其中 unit 为可选 String 字段，用于单位一致性校验和剂量超限判定；与 DosageCheckRequest.unit 业务角色一致——用于 DosageUnitGroup 分组换算）、患者个体信息（patientInfo，含 patientId/age/gender/allergyHistory（string，过敏史文本——由后端从健康档案实体拼接 allergen 名称列表为中文逗号分隔字符串，为 single source of truth）/allergyDetails（List\<AllergyDetail\>，可选扩展容器——后端 Service 层优先从健康档案实体自动提取为 single source of truth，前端传入值仅作为 fallback/离线场景覆盖）/comorbidities（List\<String\>，合并症列表——由后端从健康档案慢病列表自动提取 diseaseName））、处方标识（prescriptionId，必填）。
...
- prescriptionItems（List\<PrescriptionItem\>，每项含 drugId/drugName/dose/unit/frequency/duration/route 七个字段，提交处方完整药品列表；unit 字段与 AuditRequest.prescriptionItems 业务角色一致——用于 DosageUnitGroup 分组换算和剂量超限判定）
...
- warnResult（WarnResult，可选——步③ forceSubmit=false + 最新审核结果为 WARN + 处方未变更（哈希一致）路径下填充，承载当前 WARN 审核结果供前端展示并提示医生选择强制提交或修改处方；此时 submitted=false, errorCode=null, blockInfo=null——三态字段联合表达"未阻断但需医生决策"的中间态。**此路径不使用 errorCode 承载风险语义**，避免与 forceSubmit=true 校验失败的 errorCode 语义混淆）

**SubmitResponse 多状态字段约束**：

| 路径 | submitted | blockInfo | errorCode | warnResult | HTTP |
|------|----------|-----------|-----------|------------|------|
| 步① CRITICAL 剂量阻断 | false | BlockResponse（blockCode=RX_BLOCK_CRITICAL_DOSE） | null | null | 422 |
| 步② 审核 BLOCK 阻断 | false | BlockResponse（blockCode=RX_BLOCK_AUDIT_BLOCK） | null | null | 422 |
...
| 步③ forceSubmit=true + WARN 校验通过 | true | null | null | null | 200 |
| 步③ forceSubmit=true + 处方版本不一致 | false | null | RX_AUDIT_PRESCRIPTION_MODIFIED | null | 409 |
| 步③ forceSubmit=true + 无 WARN 审核 | false | null | RX_AUDIT_FORCE_SUBMIT_INVALID | null | 409 |
| 步③ forceSubmit=true + 并发冲突 | false | null | RX_AUDIT_CONCURRENT_SUBMIT | null | 409 |
| 步③ forceSubmit=false + 无审核/PASS | true | null | null | null | 200 |
...
职责：AI 辅助开方主端点响应值对象，对齐需求文档 3.4.10 输出契约。包含处方草案（prescriptionDraft，含 drugs 数组，每项含 drugId/drugName/dose/unit/frequency/duration/route 七个字段，其中 unit 为可选 String 字段，用于单位一致性校验和剂量超限判定；与 DosageCheckRequest.unit 业务角色一致——用于 DosageUnitGroup 分组换算）、剂量告警列表（doseWarnings，List\<DoseWarning\>，每项含 drugId/warningType（DoseWarningType 枚举）/message/severity（DosageAlertLevel 枚举））、过敏冲突告警（allergyWarnings，List\<AllergyWarningItem\>，每项含 drugId/allergen/severity（AllergyWarningSeverity 枚举））、顶层错误码（errorCode，可选，String——AI 返回无可推荐药品时填充 RX_ASSIST_AI_NO_RECOMMENDATION）、免责声明标记（disclaimerRequired，boolean，固定 true）。
...
```json
{
  ...
  "prescriptionItems": [
    {"drugId": "DRG001", "drugName": "阿莫西林", "dose": 500, "unit": "mg", "frequency": "tid", "duration": 7, "route": "po"}
  ],
  "patientInfo": {
    "patientId": "P001",
    "age": 45,
...
```json
{
  "prescriptionDraft": {
    "drugs": [
      {"drugId": "DRG002", "drugName": "头孢克肟", "dose": 100, "unit": "mg", "frequency": "bid", "duration": 5, "route": "po"}
    ]
  },
  "doseWarnings": [],
...
| WARN 路径响应设计 | SubmitResponse 通过 warnResult（WarnResult 类型）承载 WARN 风险信息，不使用 errorCode 字段 | errorCode 在 WARN 路径下恒为 null——避免与 forceSubmit=true 校验失败的 errorCode 语义混淆；warnResult 字段独立承载 riskLevel/alerts/auditRecordId/prescriptionHash 四要素供前端决策展示；RX_AUDIT_WARN_REQUIRES_FORCE_SUBMIT 错误码从代码中移除（OOD 任何位置均未定义此错误码） |
| PrescriptionItem.unit 字段 | PrescriptionItem 七个字段（drugId/drugName/dose/unit/frequency/duration/route），unit 为可选 String 字段，值域限定为 DosageUnitGroup 枚举中各分组的单位值 | 处方审核/辅助开方/处方提交流程中均需进行单位一致性校验和剂量超限判定，unit 字段缺失将导致跨组单位无法换算；unit 与 DosageCheckRequest.unit 业务角色一致，统一通过 DosageUnitGroup 分组换算；不传时按业务约定回退（由 DrugFacade 提供的药品基础信息填充默认值） |
...
**PrescriptionItem.unit 与 DosageCheckRequest.unit 映射关系**：PrescriptionItem（用于 AuditRequest.prescriptionItems / SubmitRequest.prescriptionItems / PrescriptionAssistRequest.existingPrescription.drugs / PrescriptionAssistResponse.prescriptionDraft.drugs）中的 unit 字段与 DosageCheckRequest（单药品即时校验）中的 unit 字段业务角色一致——均用于 DosageUnitGroup 分组换算和剂量超限判定。DosageThresholdService.check() 对处方审核/辅助开方流程中的 prescriptionItems 列表逐项执行单位一致性校验时，复用相同的 DosageUnitGroup 分组换算规则——unit 不属于同组时输出 warningType=OVER_SINGLE_DOSE + alertLevel=WARN + errorCode=RX_ASSIST_UNIT_MISMATCH。unit 字段在 PrescriptionItem 中为可选，不传时按业务约定回退（具体回退规则由 admin 模块的药品基础信息实体 drug_base_info.doseUnit 提供默认值，由 DrugFacade.findByDrugCode(drugCode) 返回时一并填充）。
...
- PrescriptionCheckItem：drugId / drugName / dose / unit（可选 String，与业务层 PrescriptionItem.unit 字段业务角色一致——用于 DosageUnitGroup 分组换算）/ frequency / duration / route
...
扩展字段：diagnosis（string，必填）、examResults（List\<ExamResultItem\>，可选，每项含 itemName/value/unit/referenceRange/status）、patientInfo（object，含 patientId/age/gender/allergyHistory/allergyDetails/comorbidities）、existingPrescription（object，可选，含 drugs 数组，每项含 drugId/drugName/dose/unit（可选 String）/frequency/duration/route）、encounterId（string，可选）。

#### PrescriptionAssistResponse（ai-api 层）

扩展字段：prescriptionDraft（object，含 drugs 数组每项含 drugId/drugName/dose/unit（可选 String）/frequency/duration/route）、doseWarnings（List\<DoseWarningItem\>，每项含 drugId/warningType/message/severity）、allergyWarnings（List\<AllergyWarningItem\>，每项含 drugId/allergen/severity）、disclaimerRequired（bool，固定 true）。
