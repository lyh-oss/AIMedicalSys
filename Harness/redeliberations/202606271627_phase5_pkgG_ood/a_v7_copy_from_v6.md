# Phase 5 包 G — AI 进阶底座 架构级 OOD 设计方案（v8）
...
classDiagram
    ...
    class AbstractCapabilityExecutor~T, R~ {
        <<abstract>>
        #doDegrade(startTime, degradeReason, request, capabilityId) AiResult~R~~
        #doPreDegradeCheck(context, degradationStrategies) boolean
        #doRecordMetrics(startTime, capabilityId, departmentId, ...) void
...
**UserId 与 SessionId 的上下文来源**（v5 新增）：
- `userId` 由 `CapabilityExecutor.execute()` 内部从 Spring `SecurityContextHolder.getContext().getAuthentication().getName()` 提取当前操作用户标识
- `sessionId` 从业务请求 DTO（`AiRequestBase.sessionId`）继承，或从 HTTP Request 的 `X-Session-ID` Header 中提取（由 `AiOrchestrator` 在委托前注入）
- 提取后的 userId 和 sessionId 传入 `ExperimentManager.assign()` 用于 A/B 实验确定性哈希分桶
...
**降级策略注入机制**（v4 新增，v5 细化装配路径，v6 统一命名约定，v8 明确推导规则）：每个 `CapabilityExecutor` 实现注入各自的 `List<DegradationStrategy>`，策略列表通过 Spring `@Qualifier` 按能力标识注入。**Bean name 推导规则**：`capabilityId` 全大写字符串（如 `"TRIAGE"`、`"RX_AUDIT"`）通过 `StringUtils.uncapitalize(capabilityId).toLowerCase()` + `"Strategies"` 推导出 Qualifier 名称，即将首字母小写后拼接——如 `"TRIAGE"` → `@Qualifier("triageStrategies")`、`"RX_AUDIT"` → `@Qualifier("rxAuditStrategies")`。此规则由 `AiPlatformConfig` 在构建策略映射表 `Map<String, List<DegradationStrategy>>` 时内部执行，对外透明。由 `AiPlatformConfig` 中的 `ai.degradation.strategies` 配置驱动。降级预检时，`execute()` 内部按 `getOrder()` 升序遍历策略链。
...
- **包含**：降级预检（同完整管线）、实验分流仅用于 departmentId 提取（Phase 4 DTO 尚未继承 `AiRequestBase`，departmentId 通过 `extractDepartmentId()` 独立获取）、模型路由空值检查后直接委托 Phase 4 业务服务、指标采集与 `recordSuccess/Failure/Degraded`（同完整管线）
- **不包含**：模板渲染（`PromptTemplateManager.render()`）、模型路由到 LLM 调用的完整链路、结构化输出解析
- **降级路径**：同完整管线——降级预检命中后走本地规则降级或 `AiResult.degraded()`

**departmentId 提取策略**（v8 新增）：Phase 4 薄适配器型 CapabilityExecutor 的 `request` DTO 尚未继承 `AiRequestBase` 基类，无 `getDepartmentId()` 方法。薄适配器使用独立的 `extractDepartmentId(request)` 提取方法，从 Spring `RequestContextHolder` 或 HTTP `X-Department-ID` Header 中获取当前科室标识。若无法获取则不传入（departmentId=null），Prompt 模板回退到通用模板。

薄适配器的 `execute()` 伪代码（v6 新增 supplyAsync 异步包装与委托调用异常处理）：
```
CapabilityExecutorThinAdapter.execute(request, capabilityId):
  return CompletableFuture.supplyAsync(() -> {
    startTime = System.currentTimeMillis()
    departmentId = extractDepartmentId(request)  // 从 RequestContext / SecurityContext 独立提取

    // 降级预检（同完整管线）
    context = slidingWindowMetricsStore.buildDegradationContext(capabilityId, ...)
    for each strategy in this.degradationStrategies:
      if strategy.shouldDegrade(context):
        return doThinDegrade(startTime, strategy.getClass().getSimpleName(), capabilityId, departmentId)
    ...
    try {
      ...
      metricsCollector.record(AiCallRecord.failure(capabilityId, elapsedMs, e.getMessage(), departmentId, ...))
      slidingWindowMetricsStore.recordFailure(capabilityId)
      return AiResult.failure(e.getMessage())
    } catch (Exception e):
      // 基础设施异常（网络超时、服务不可用）：走降级路径
      return doThinDegrade(startTime, "InfrastructureError:" + e.getClass().getSimpleName(), capabilityId, departmentId)

    elapsedMs = System.currentTimeMillis() - startTime
    metricsCollector.record(AiCallRecord.success(capabilityId, elapsedMs, departmentId, ...))
    slidingWindowMetricsStore.recordSuccess(capabilityId, elapsedMs)
    return AiResult.success(result)
  }, llmCallExecutor)

doThinDegrade(startTime, degradeReason, capabilityId, departmentId):
  elapsedMs = System.currentTimeMillis() - startTime
  metricsCollector.record(AiCallRecord.degraded(capabilityId, elapsedMs, degradeReason, departmentId, ...))
...
AbstractCapabilityExecutor.execute(request, capabilityId):
  return CompletableFuture.supplyAsync(() -> {
    ...
    context.setDepartmentId(doExtractDepartmentId(request))
    for each strategy in this.degradationStrategies (sorted by getOrder() asc):
      if strategy.shouldDegrade(context):
        return doDegrade(startTime, strategy.getClass().getSimpleName(), request, capabilityId)

    // 抽象方法——子类特化（正常执行管线）
    return doExecuteInternal(startTime, request, capabilityId, context.getDepartmentId())
  }, llmCallExecutor)

// 子类需实现的步骤
abstract doExecuteInternal(startTime, request, capabilityId, departmentId):
    // 模板渲染（完整管线）/ 直接委托（薄适配器）
    // 实验分流（完整管线）/ 跳过（薄适配器）
    // 模型路由
    // LLM 调用
...
**与 CircuitBreakerDegradationStrategy 的交互优先级**（v5 新增）：
- 两个组件独立维护健康状态，但执行时序决定了优先级：
...
- **交互规则**：
  ...
  - 熔断器降级后的探测机制由 `CircuitBreakerDegradationStrategy` 内部状态机（OPEN→HALF_OPEN→CLOSED）驱动，不依赖 `ModelEndpointHealthManager.tryProbe()`；端点降级后的探测机制由 `ModelEndpointHealthManager` 自主管理
...
AiOrchestrator.handle(capabilityId, request):
  ...
  8.   // 异常来源示例：模板渲染 NPE、Jackson 序列化失败、SecurityContext 访问异常
  9.   // 处理方式：记录错误日志 + 返回 AiResult.failure() 而非传播为 CompletionException
   10.   log.error("CapabilityExecutor 执行异常: capabilityId={}, error={}", capabilityId, e.getMessage(), e)
    11.   metricsCollector.record(AiCallRecord.failure(capabilityId, LocalDateTime.now(), "InternalError:" + e.getClass().getSimpleName(), ...))
...
CapabilityExecutor.execute(request, capabilityId):
  // 异步边界：管线主体通过 supplyAsync 委派到共享 LlmCallExecutor 线程池
  return CompletableFuture.supplyAsync(() -> {
    startTime = System.currentTimeMillis()

    // 降级预检（遍历注入的策略链，按 @Qualifier("{capabilityId}Strategies") 模式注入）
    context = slidingWindowMetricsStore.buildDegradationContext(capabilityId, this.getClass().getSimpleName())
    ...
    renderedPrompt = promptTemplateManager.render(capabilityId, request.getDepartmentId(), variables, assignment.getTargetPromptVersion())
    modelRoute = modelRouter.route(capabilityId, assignment)
    if modelRoute == null:
      return doDegrade(startTime, "NoAvailableRoute", request, capabilityId)
    ...
    if endpointState == UNAVAILABLE:
      if not endpointHealthManager.tryProbe(modelRoute.getEndpointId()):
        return doDegrade(startTime, "EndpointUnavailable", request, capabilityId)
...
// 降级路径处理（提取为私有方法，被 supplyAsync lambda 内部调用）
doDegrade(startTime, degradeReason, request, capabilityId):
  elapsedMs = System.currentTimeMillis() - startTime
