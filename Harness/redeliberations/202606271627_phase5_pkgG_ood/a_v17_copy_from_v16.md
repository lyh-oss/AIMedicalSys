# Phase 5 包 G — AI 进阶底座 架构级 OOD 设计方案（v17）
...
│   │   ├── HttpApiLlmChatService.java     # HTTP API 调用实现
│   │   ├── SpringAiLlmChatService.java    # Spring AI ChatModel 实现
...
classDiagram
    ...
    class LlmChatService {
        ...
        +structuredChat(LlmChatRequest, Class~T~) CompletableFuture~AiResult~T~~
    }
    ...
    class AiPlatformConfig {
        ...
        +metricsAsyncExecutor() Executor
        +degradationStrategyMap() Map
        +capabilityTimeoutConfig() Map
    }
    ...
    LlmChatStreamService <|.. HttpApiLlmChatService
    LlmChatStreamService <|.. SpringAiLlmChatService
...
class DiagnosisCapabilityExecutor extends AbstractCapabilityExecutor<DiagnosisRequest, DiagnosisResponse> {
    ...
    DiagnosisCapabilityExecutor(
        ...
        super(promptTemplateManager, modelRouter, llmChatService, structuredOutputParser,
...
- `structuredChat(LlmChatRequest, Class<T>)` → `CompletableFuture<AiResult<T>>`：结构化输出方法，强制模型输出符合给定 Java 类型的结构化数据，优先使用模型原生 tool_use / function_call / JSON mode 能力。输入校验与 chat() 相同，目标类型 `Class<T>` 必须可由 Jackson 反序列化
...
**Vault 不可达降级行为**：
- Vault 查询超时（默认 2 秒）：若缓存中存在旧凭据则继续使用旧凭据并延长 TTL 30 秒，触发 WARN 日志；若缓存也无数据则抛出 `CredentialUnavailableException`，由 `LlmChatService` 捕获后转为降级路径
...
1. 从 YAML/属性源绑定 `@ConfigurationProperties(prefix = "ai")` 配置
2. 通过 `@Bean` 方法暴露公共基础设施 Bean（线程池、策略 Map、超时配置等）
3. 实现 `EnvironmentPostProcessor` 完成 `ai.platform.enabled` 到 `ai.mock.enabled` 的反向转发
4. 实现 `ApplicationContextAware` 在 `@PostConstruct` 阶段构建降级策略映射表
...
public Executor llmCallExecutor() {
    // 从 YAML ai.platform.executor.llm-call.* 绑定配置
...
public Executor metricsAsyncExecutor() {
    // 从 YAML ai.platform.executor.metrics-async.* 绑定配置
    // 核心线程 2，最大线程 4，队列容量 1000
    // 拒绝策略 DiscardPolicy + 日志 WARN（指标丢失可接受，不阻塞主路径）
...
public Map<String, Duration> capabilityTimeoutConfig() {
    // 从 YAML ai.execution.timeout.per-capability 绑定
    return this.timeoutConfig;
}
```

**EnvironmentPostProcessor 实现**：
```
AiPlatformConfig.postProcessEnvironment(ConfigurableEnvironment env, SpringApplication app):
...
**类型定位**：使用 class 而非 interface，因配置装配为单一职责的具体实现，无多态替换需求。标注 `@Configuration` + `@ConfigurationProperties(prefix = "ai")` + `implements EnvironmentPostProcessor, ApplicationContextAware`。
...
### 3.10 `extractFromRequestContext()` — HTTP Header 提取工具方法（protected method，归属 `AbstractCapabilityExecutor`）

**职责**：从当前 HTTP 请求上下文中提取指定 Header 值，供薄适配器型 CapabilityExecutor 在 `execute()` 入口处（容器线程）提取就诊上下文。
...
**调用方与使用场景**：
- `AiOrchestrator.handle()` catch 块中 Phase 4 DTO 兼容路径（行 1625-1628）
- 薄适配器 `doExtractDepartmentId()`、`doExtractVisitId()`、`doExtractPatientId()`、`doExtractSessionId()` 各方法

**为何定义为 protected 方法而非独立工具类**：仅在 `AbstractCapabilityExecutor` 及其子类中使用，不对外暴露；protected 访问权限使子类可重写（如单元测试中注入 MockHttpServletRequest）。
...
AiOrchestrator.handle(capabilityId, request):
  ...
  15.  inputSummary = request != null ? StringUtils.truncate(request.toString(), 500) : null
   ...
   25.    departmentId = extractFromRequestContext("X-Department-ID")
   26.    visitId = extractFromRequestContext("X-Visit-ID")
   27.    patientId = extractFromRequestContext("X-Patient-ID")
   28.    sessionId = extractFromRequestContext("X-Session-ID")
...
doExecuteInternal(startTime, request, capabilityId, departmentId, userId, sessionId, callerRole, callerId, visitId, patientId, inputSummary):
    ...
    chatRequest = new LlmChatRequest(
        ...
        options: new LlmChatOptions(modelId: modelRoute.getModelId(), ...))

    // 优先使用 structuredChat（模型原生结构化输出模式，如 tool_use / function_call / JSON mode）
    // 若 structuredChat 失败或模型不支持，回退到 chat() + StructuredOutputParser.parse()
    try:
      parsedResult = llmChatService.structuredChat(chatRequest, outputType)
    catch (Exception e):
      // structuredChat 回退路径：使用 chat() + 传统 JSON 文本解析
    ...
    metricsCollector.record(AiCallRecord.success(capabilityId, LocalDateTime.now(), elapsedMs,
        departmentId, modelRoute.getModelId(), tokenUsage != null ? retryCount : 0, tokenUsage != null ? tokenUsage.getPromptTokens() : null,
...
ThinAdapterCapabilityExecutor.doExecuteInternal(startTime, request, capabilityId, departmentId, userId, sessionId, callerRole, callerId, visitId, patientId, inputSummary):
    ...
    // 避免嵌套提交到同一线程池产生排队死锁
    // 差异 3：独立 thinAdapterTimeout 超时控制（默认 60s，可通过 YAML 按能力配置）
    ...
    try:
        ...
        return doDegrade(startTime, "TIMEOUT:ThinAdapterTimeout", request, capabilityId, departmentId, callerRole, callerId, visitId, patientId, sessionId, inputSummary, null, promptVersion)
    catch (BusinessException e):
