...
| 质询意见 | 回应 |
|---------|------|
| **[问题 A] 协作关系断言的引用指向错误**：需求要素表中声明协作关系「缺少端到端运行时协作视图，详见问题 5」，但问题 5 讨论的是 HTTP Header 提取的隐含依赖假设，与端到端运行时协作视图无关。 | **承认质询成立**。原报告中"详见问题 5"的交叉引用确实指向了不相关的问题。实际情况是：v20 设计文档中端到端运行时协作视图以 §4.1 管线伪代码形式存在（AiOrchestrator.handle() → CapabilityExecutor.execute() → doExecuteInternal() 完整执行流），配合 §2.3 的静态类图和 §3.2 的统一探测时序图，协作视图已有实质性覆盖。**应在需求要素对照表中将「协作关系」状态从「部分覆盖」修正为「✓ 覆盖」**，理由：§2.3 类图表达组件静态协作关系，§4.1 伪代码表达运行时调用链，§3.2 时序图表达特定场景（统一探测）的跨组件交互，整体协作关系已有适度覆盖。 |
| **[问题 B] 对未验证的需求维度未采取行动**：报告在「设计风格一致性」维度标注「§1.2 末段声明参照但未验证」，承认未验证一项核心用户需求却不展开调查。 | **承认质询成立**。经本 v3 审查补充对比验证，结论如下：v20 文档与 Phase0（`Docs/04_ood_phase0.md`）及 Phase1ABD（`Docs/05_ood_phase1_B.md`）在设计风格上保持了一致性——(a) 章节编号方案一致（1.概述 → 2.模块划分 → 3.核心抽象 → 4.关键行为契约 → 5.错误处理 → 6.并发设计 → 7.设计决策 → ...）；(b) 核心抽象表格式一致（「抽象 | 类型形态 | 职责定位」三列表）；(c) 字段定义表格式一致（「字段 | 类型 | 说明」三列表）；(d) 设计决策表格式一致（「决策 | 选项 | 选择 | 理由」四列表）；(e) 伪代码约定一致（Mermaid 语法、行内注释风格）；(f) 状态模型表示法一致（状态转换图 + 表格形式）。**应在需求要素对照表中将「设计风格一致性」从「部分覆盖」修正为「✓ 覆盖」**。 |
| **[新发现, 重要] `extractFromRequestContext()` 方法在 `AiOrchestrator.handle()` 中不可访问**：§3.10 将 `extractFromRequestContext()` 定义为 `AbstractCapabilityExecutor` 的 `protected` 方法，声明"仅在 AbstractCapabilityExecutor 及其子类中使用，不对外暴露"。但 §4.1 `AiOrchestrator.handle()` catch 块（第 1862-1868 行）直接调用了 `extractFromRequestContext("X-Department-ID")` 等方法。`AiOrchestrator` 既不继承 `AbstractCapabilityExecutor`，也不属于其子类层次。此调用在 Java 编译层面非法——从非继承关系的类中调用 `protected` 方法无法通过编译。**严重程度**：重要——编译错误级别，实现者编译时即刻受阻。**改进建议**：方案 A（推荐）：将 `extractFromRequestContext()` 提取为独立的工具类（如 `RequestContextUtils`）中的 `public static` 方法，`AbstractCapabilityExecutor` 和 `AiOrchestrator` 共同引用此工具类。方案 B：在 `AiOrchestrator` 中内联实现相同逻辑的私有方法，但导致两处维护重复代码。方案 C：在 `AiOrchestrator` 中注入 `HttpServletRequest` 直接提取 Header，但需在启动时验证 `RequestContextHolder` 可用性。 | — |
