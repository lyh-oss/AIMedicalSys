# Phase 5 包 G — AI 进阶底座 架构级 OOD 设计方案（v13）
...
- `orchestrator/impl/` 中的 6 个薄适配器型 CapabilityExecutor 各持有对应 Phase 4 业务服务的引用。ai-impl 模块在 Maven 层面声明对 Phase 4 业务服务模块的依赖（`test` 范围或 `compile` 范围）。为避免破坏包 G 内部的分层抽象，Phase 4 业务服务接口不在 ai-api 中重新定义——薄适配器直接引用 Phase 4 模块中的现有业务接口（如 `com.aimedical.modules.diagnosis.service.DiagnosisService`），通过构造器注入获取服务实例。依赖方向为 `ai-impl → Phase 4 modules`（出向依赖，包 G 依赖外部模块），不产生循环依赖
...
classDiagram
    ...
    class AbstractCapabilityExecutor~T, R~ {
        <<abstract>>
        #doDegrade(startTime, degradeReason, request, capabilityId, departmentId, callerRole, callerId, visitId, patientId, sessionId) AiResult~R~~
        +execute(T request, String capabilityId) CompletableFuture~AiResult~R~~ (template method)
        #doExecuteInternal(startTime, request, capabilityId, departmentId, userId, sessionId, callerRole, callerId, visitId, patientId) AiResult~R~~
...
// Phase 4 业务服务接口归属其原有模块（com.aimedical.modules.diagnosis），ai-impl 在 Maven pom.xml 中声明对 Phase 4 各业务模块的 compile 依赖
@Component
...
// inputSummary 已在 execute() 入口处定义并捕获至闭包（此处不再重复定义）
doExecuteInternal(startTime, request, capabilityId, departmentId, userId, sessionId, callerRole, callerId,
    ...
    try {
      ...
      CompletableFuture<R> delegateFuture = CompletableFuture.supplyAsync(
          () -> phase4ServiceDelegate.execute(request), llmCallExecutor)
      result = delegateFuture.get(thinAdapterTimeout.toMillis(), TimeUnit.MILLISECONDS)
    } catch (TimeoutException e):
      // 薄适配器委托超时：走降级路径
      delegateFuture.cancel(true)
      return doDegrade(startTime, DegradationReason.TIMEOUT + ":ThinAdapterTimeout", request, capabilityId, departmentId, callerRole, callerId, visitId, patientId, sessionId)
      ...
      return doDegrade(startTime, DegradationReason.INFRASTRUCTURE_ERROR + ":" + e.getClass().getSimpleName(), request, capabilityId, departmentId, callerRole, callerId, visitId, patientId, sessionId)
...
AbstractCapabilityExecutor.execute(request, capabilityId):
  ...
  // 防御性拷贝：若能力 DTO 尚未改为不可变，在容器线程（supplyAsync 之前）做深拷贝
  request = objectMapper.convertValue(request, request.getClass())

  return CompletableFuture.supplyAsync(() -> {
    // 在 lambda 入口处定义 inputSummary（降级路径和正常管线均需使用）
    ...
    for each strategy in this.degradationStrategies (sorted by getOrder() asc):
      if strategy.shouldDegrade(context):
        return doDegrade(startTime, strategy.getClass().getSimpleName(), request, capabilityId, departmentId, callerRole, callerId, visitId, patientId, sessionId)
...
// 子类需实现的步骤
abstract doExecuteInternal(startTime, request, capabilityId, departmentId, userId, sessionId, callerRole, callerId, visitId, patientId):
    // 模板渲染（完整管线）/ 直接委托（薄适配器）
    // 实验分流（完整管线）/ 跳过（薄适配器）
    // 模型路由
    // LLM 调用
...
- 超时阈值按能力独立配置，存储在 `capabilityTimeoutConfig`（`Map<String, Duration>`），通过 `AiPlatformConfig` 从 YAML 绑定，默认值 60 秒
...
**职责**：封装一次大模型调用的原始输出，包括文本内容、Token 用量、使用的模型标识与响应时间戳。Token 用量通过内嵌的 `TokenUsage` 值对象提供，`LlmClient` 实现类在收到 HTTP 响应后从响应体中解析 Token 用量字段并构造 `TokenUsage` 实例。
...
AiOrchestrator.handle(capabilityId, request):
  1. executor = executorMap.get(capabilityId)
  2. if executor == null: throw IllegalStateException("未注册能力标识: " + capabilityId)
  3. try:
  4.   result = executor.execute(request, capabilityId)
  5.   return result
  6. catch (Exception e):
  7.   // 捕获 executor.execute() 或下游管线中的不可预知异常
  8.   // 处理方式：记录错误日志 + 返回 AiResult.failure() 而非传播为 CompletionException
  9.   log.error("CapabilityExecutor 执行异常: capabilityId={}, error={}", capabilityId, e.getMessage(), e)
  10.  // 从 request 对象提取可用上下文，避免异常场景下丢失关键就诊信息（visitId/patientId/departmentId/sessionId）
  11.  inputSummary = request != null ? StringUtils.truncate(request.toString(), 500) : null
  12.  if request instanceof AiRequestBase:
  13.    departmentId = request.getDepartmentId()
  14.    visitId = request.getVisitId()
  15.    patientId = request.getPatientId()
  16.    sessionId = request.getSessionId()
  17.  else:
  18.    departmentId = null
  19.    visitId = null
  20.    patientId = null
  21.    sessionId = null
  22.  metricsCollector.record(AiCallRecord.failure(capabilityId, LocalDateTime.now(), 0L,
  23.      DegradationReason.INTERNAL_ERROR + ":" + e.getClass().getSimpleName(), e.getMessage(),
  24.      departmentId, inputSummary, visitId, patientId, sessionId, null, null, null))
  25.  slidingWindowMetricsStore.recordFailure(capabilityId)
  26.  return CompletableFuture.completedFuture(AiResult.failure("AI服务暂时不可用，请稍后重试"))
...
AbstractCapabilityExecutor.execute(request, capabilityId):
  ...
  for each strategy in this.degradationStrategies (sorted by getOrder() asc):
    if strategy.shouldDegrade(context):
      return CompletableFuture.completedFuture(doDegrade(startTime, strategy.getClass().getSimpleName(), request, capabilityId, departmentId, callerRole, callerId, visitId, patientId, sessionId))
  ...
  return future.orTimeout(capabilityTimeout.toMillis(), TimeUnit.MILLISECONDS)
    .exceptionally(ex -> {
      if (ex instanceof TimeoutException):
        ...
        return doDegrade(startTime, DegradationReason.TIMEOUT, request, capabilityId, departmentId, callerRole, callerId, visitId, patientId, sessionId)
...
// 注意：userId/sessionId/callerRole/callerId/visitId/patientId 已在 execute() 入口处提取并传入（线程安全），不再访问 SecurityContextHolder
// inputSummary 已在 execute() 入口处定义并捕获至闭包；outputSummary/retryCount/outputType 在方法体内定义
doExecuteInternal(startTime, request, capabilityId, departmentId, userId, sessionId, callerRole, callerId, visitId, patientId):
    ...
    if modelRoute == null:
      return doDegrade(startTime, DegradationReason.NO_AVAILABLE_ROUTE, request, capabilityId, departmentId, callerRole, callerId, visitId, patientId, sessionId)
    ...
    if endpointState == UNAVAILABLE:
      if not endpointHealthManager.tryProbe(modelRoute.getEndpointId()):
        return doDegrade(startTime, DegradationReason.ENDPOINT_UNAVAILABLE, request, capabilityId, departmentId, callerRole, callerId, visitId, patientId, sessionId)
    ...
    try:
      ...
      outputSummary = extractParsedSummary(parsedResult)
    catch (ParseException e):
      return doDegrade(startTime, DegradationReason.PARSE_FAILURE + ":" + e.getClass().getSimpleName(), request, capabilityId, departmentId, callerRole, callerId, visitId, patientId, sessionId)
    elapsedMs = System.currentTimeMillis() - startTime
...
// 参数说明：departmentId/callerRole/callerId/visitId/patientId/sessionId 由 execute() 入口处捕获传入，lambda 内不再访问 ThreadLocal 或 DTO getter
// inputSummary 由 execute() 入口处定义并通过闭包捕获，所有调用点均可访问（不通过参数传递）
doDegrade(startTime, degradeReason, request, capabilityId, departmentId, callerRole, callerId, visitId, patientId, sessionId):
  elapsedMs = System.currentTimeMillis() - startTime
