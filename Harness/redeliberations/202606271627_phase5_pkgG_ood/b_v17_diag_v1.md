...
### 问题 3【重要，逻辑矛盾】修订说明版本号与文档标题不一致

- **描述**：文档标题标注为 `（v17）`，但 `修订说明` 章节末尾包含 `修订说明（v18）` 的完整条目（§ 末尾，约 line 2625-2639）。v18 修订内容（如 `ModelRoute.parameters` 字段补充、`DegradationContext` 类图同步等）已实际体现在文档正文中，但标题仍为 v17。

- **位置**：文档第 1 行标题 vs. 末尾 `修订说明（v18）` 章节。

- **严重程度**：重要 — 版本号不一致导致版本管理混乱，在后续迭代中可能错误关联反馈与修改。

- **改进建议**：统一版本号为 v18（标题、文件名、修订说明标题），或确认 v18 修订是否已在当前版本中体现。若已体现在正文中，则标题应为 v18。

### 问题 4【一般，完整性】§1.3 核心抽象表遗漏 `CredentialProvider` 和 `EndpointRateLimiter`
...
### 问题 5【一般，完整性不足】`LlmChatOptions` 与 `ModelRoute.parameters` 的字段映射覆盖逻辑未定义
...
### 问题 6【一般，完整性】`PromptTemplateManager.getFallbackPrompt()` 的返回值约束未定义
...
2. **文档版本管理**（问题 3）和**概览层面组件列表的完整性**（问题 4-6）属于组织层面的改进项目。

以上 6 个问题修复后，产出将可直接指导编码实现。
