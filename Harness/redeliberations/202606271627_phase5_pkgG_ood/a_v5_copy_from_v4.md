...
**降级策略注入机制**（v4 新增，v5 细化装配路径）：每个 `CapabilityExecutor` 实现注入各自的 `List<DegradationStrategy>`，策略列表通过 Spring `@Qualifier("capabilityId")` 按能力标识注入，由 `AiPlatformConfig` 中的 `ai.degradation.strategies` 配置驱动。降级预检时，`execute()` 内部按 `getOrder()` 升序遍历策略链。

**YAML 配置到 Bean 引用的装配路径**（v5 新增）：
```
1. application.yml → ai.degradation.strategies:
       TRIAGE: [timeout, circuit-breaker]
       RX_AUDIT: [timeout, noop]
2. AiPlatformConfig 读取 YAML 列表字符串
...
4. 生成每个能力标识的策略 Bean 名称列表 → 使用 Spring ApplicationContext.getBeansOfType() 按名称匹配
5. AiPlatformConfig.defineStrategyBeans() 为每个能力标识生成 @Bean 方法返回 List<DegradationStrategy>：
   - @Bean("triageStrategies")
   - @Bean("rxAuditStrategies")
   - 每个方法中将 YAML 名称映射为实际 Bean 引用（通过 ApplicationContext 查找）
6. CapabilityExecutor 实现使用 @Qualifier("triageStrategies") 注入对应策略列表
```

此机制在 `AiPlatformConfig` 中集中实现，确保策略装配路径可测试、可追踪。
...
- **包含**：降级预检（同完整管线）、模型路由空值检查后直接委托 Phase 4 业务服务、指标采集与 `recordSuccess/Failure/Degraded`（同完整管线）
- **不包含**：模板渲染（`PromptTemplateManager.render()`）、实验分流（`ExperimentManager.assign()`）、模型路由到 LLM 调用的完整链路、结构化输出解析
...
getEffectiveFailureRate(capabilityId) → double
    返回有效失败率（recordFailure / (recordSuccess + recordDegraded + recordFailure)），供超时降级策略参考
...
AiOrchestrator.handle(capabilityId, request):
  ...
  9.   // 处理方式：记录错误日志 + 返回 AiResult.error() 而非传播为 CompletionException
  10.   log.error("CapabilityExecutor 执行异常: capabilityId={}, error={}", capabilityId, e.getMessage(), e)
  11.   metricsCollector.record(AiCallRecord.failure(capabilityId, System.currentTimeMillis(), "InternalError:" + e.getClass().getSimpleName(), ...))
  12.   slidingWindowMetricsStore.recordFailure(capabilityId)
  13.   return CompletableFuture.completedFuture(AiResult.error("AI服务暂时不可用，请稍后重试"))
...
CapabilityExecutor.execute(request, capabilityId):
  ...
  renderedPrompt = promptTemplateManager.render(capabilityId, request.getDepartmentId(), variables)  // departmentId 来自 AiRequestBase.departmentId
  userId = SecurityContextHolder.getContext().getAuthentication().getName()  // userId 从 Spring Security 上下文提取
  sessionId = request.getSessionId()              // sessionId 来自 AiRequestBase.sessionId
  ...
  if localRuleFallback != null:
    ...
    slidingWindowMetricsStore.recordDegraded(capabilityId, elapsedMs)  // 降级兜底成功，记入 degraded 类型
    return result
  else:
    metricsCollector.record(AiCallRecord.degraded(capabilityId, elapsedMs, degradeReason, ...))
...
- `ModelRouter`：路由表启动时从配置/DB 加载到 `ConcurrentHashMap`，支持运行时刷新。**为避免部分替换时读操作看到不一致的路由状态，刷新策略采用全量替换模式**：新路由表先在本地构造完整的 `ConcurrentHashMap` 副本（复制 + 增量变更），然后通过 `AtomicReference<Map<String, ModelRoute>>` CAS 一次性替换引用。读取路径通过 `AtomicReference.get()` 获取不可变的当前路由表快照，确保读取始终看到一致状态
...
| AiOrchestrator.handle() 异常处理（v5 新增） | 异常传播到 CompletableFuture vs 兜底捕获 | try-catch 捕获意外异常，返回 `AiResult.error()` | 防止不可预知异常（NPE、序列化失败）传播为 `CompletionException` 导致 HTTP 500；预期异常已在 CapabilityExecutor.execute() 内部以降级路径处理 |
| YAML 策略配置到 Bean 引用的转换（v5 新增） | 直接 @Qualifier 名称匹配 vs Factory 类集中转换 | `AiPlatformConfig` 集中转换 + `ApplicationContext.getBeansOfType()` 按名称查找 | 装配路径显式、可测试；避免 Bean 名称字符串散落在各 CapabilityExecutor 的 @Qualifier 注解中 |
...
| P12 [中等] AiOrchestrator.handle() 缺少异常捕获 | §4.1 handle() 伪代码新增 try-catch，捕获意外异常返回 `AiResult.error()` |
