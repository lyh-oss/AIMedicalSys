# Phase 5 包 G — AI 进阶底座 架构级 OOD 设计方案（v2）
...
classDiagram
    class AiService {
        <<interface>>
        +triage(TriageRequest) AiResult~TriageResponse~
        +prescriptionCheck(PrescriptionCheckRequest) AiResult~PrescriptionCheckResponse~
        +medicalRecordGen(MedicalRecordGenRequest) AiResult~MedicalRecordGenResponse~
        +prescriptionAssist(PrescriptionAssistRequest) AiResult~PrescriptionAssistResponse~
        +kbQuery(KbQueryRequest) AiResult~KbQueryResponse~
        +schedule(ScheduleRequest) AiResult~ScheduleResponse~
        +discussionConclusion(DiscussionConclusionRequest) AiResult~DiscussionConclusionResponse~
        +imageAnalysis(ImageAnalysisRequest) AiResult~ImageAnalysisResponse~
        +... (其余 5 项能力方法)
    }
    ...
    class FallbackAiService {
        <<decorator>>
        -AiService delegate
        +triage() AiResult~TriageResponse~
        +prescriptionCheck() AiResult~PrescriptionCheckResponse~
        +... (委托给 delegate + 降级兜底)
    }
    ...
    class CapabilityExecutor~T, R~ {
        <<interface>>
        +execute(T request, String capabilityId) AiResult~R~
...
7. **降级判定**：在尝试 LLM 调用之前先通过 `SlidingWindowMetricsStore` 构建 `DegradationContext`，循环委托 `DegradationStrategy` 链（按 `getOrder()` 升序排列），任一策略返回 true 则跳过 LLM 调用直接进入降级
...
**线程安全模型**：AiOrchestrator 内部持有可变状态（通过 `SlidingWindowMetricsStore` 维护的滑动窗口），其线程安全性取决于被编排组件的线程安全性。`AiOrchestrator` 本身通过 `synchronized` 或 `ReentrantLock` 保护关键路径的指标存储写入，同时确保 `CapabilityExecutor` 的查找与调用分离为两个原子操作。编码层面：`SlidingWindowMetricsStore` 内部使用 `ConcurrentHashMap` 和 `AtomicLong` 保证并发安全；`CapabilityExecutor[]` 在初始化后不再变更，读操作无竞争。
...
- `MockAiService`：标注 `@ConditionalOnProperty(name = "ai.platform.enabled", havingValue = "false", matchIfMissing = true)`
- `FallbackAiService` 不参与 `ai.platform.enabled` 条件，始终存在，且通过 `@Primary` 确保业务模块注入时优先选择

```yaml
ai:
  platform:
    enabled: true                     # true → AiOrchestrator 激活；false → MockAiService 激活
```
...
**方法签名**（v2 新增）：
```
AiResult<R> execute(T request, String capabilityId)
    入参: T request — 能力对应的业务请求 DTO（如 TriageRequest）
    入参: String capabilityId — 能力标识（如 "TRIAGE"），用于路由、模板检索和指标上报
    返回值: AiResult<R> — 包裹能力对应的业务响应 DTO（如 AiResult<TriageResponse>），失败时返回降级结果
    异常: 不抛出业务异常；LLM 调用失败、解析失败等均通过降级路径以 AiResult.degraded() 返回
...
**状态模型**：
```
每个模型端点（由 ModelRoute 的 endpoint 标识）维护独立的状态：
...
| call_time | LocalDateTime | DATETIME | 调用时间 |
...
CapabilityExecutor.execute(request):
  ...
  2. 遍历 DegradationStrategy 链（按 getOrder() 升序）:
...
CapabilityExecutor.execute(request, capabilityId):
  ...
  b. for each strategy in degradationStrategies (sorted by getOrder() asc):
  ...
  k. if localRuleFallback != null:
       return localRuleFallback.fallback(request)
     else:
       return AiResult.degraded(reason)
  l. metricsCollector.record(AiCallRecord.degraded(..., reason))
  m. slidingWindowMetricsStore.recordFailure(capabilityId)
```
...
ai:
  platform:
    enabled: true                     # true → AiOrchestrator；false → MockAiService
  router:
    routes:
