# Phase 5 包 G — AI 进阶底座 架构级 OOD 设计方案（v7）
...
**降级策略注入机制**（v4 新增，v5 细化装配路径，v6 统一命名约定）：每个 `CapabilityExecutor` 实现注入各自的 `List<DegradationStrategy>`，策略列表通过 Spring `@Qualifier("{capabilityId}Strategies")` 模式按能力标识注入（如 `@Qualifier("triageStrategies")`、`@Qualifier("rxAuditStrategies")`），由 `AiPlatformConfig` 中的 `ai.degradation.strategies` 配置驱动。降级预检时，`execute()` 内部按 `getOrder()` 升序遍历策略链。
  ...
  return CompletableFuture.supplyAsync(() -> {
    ...
    for each strategy in this.degradationStrategies:
      if strategy.shouldDegrade(context):
        return doThinDegrade(startTime, strategy.getClass().getSimpleName(), capabilityId)
    ...
    try {
      ...
      metricsCollector.record(AiCallRecord.failure(capabilityId, elapsedMs, e.getMessage(), ...))
      slidingWindowMetricsStore.recordFailure(capabilityId)
      return AiResult.failure(e.getMessage())
    } catch (Exception e):
      // 基础设施异常（网络超时、服务不可用）：走降级路径
      return doThinDegrade(startTime, "InfrastructureError:" + e.getClass().getSimpleName(), capabilityId)
...
doThinDegrade(startTime, degradeReason, capabilityId):
  elapsedMs = System.currentTimeMillis() - startTime
...
- `ModelRouter`：路由表启动时从配置/DB 加载到 `ConcurrentHashMap`，支持运行时刷新。**刷新触发机制**：采用三种触发方式——（1）`@Scheduled(fixedDelay = 60000)` 定时轮询配置源，检测路由表版本变更；（2）管理端 API 调用显式触发刷新；（3）Spring `ApplicationEvent` 事件驱动（配置变更时由管理端发布 `RouteConfigChangedEvent`，`ModelRouter` 作为 `@EventListener` 响应刷新）。三种触发方式均复用同一刷新方法，通过 `synchronized` 互斥防止并发刷新。**全量替换模式**：新路由表先在本地构造完整的 `ConcurrentHashMap` 副本（复制 + 增量变更），然后通过 `AtomicReference<Map<String, ModelRoute>>` CAS 一次性替换引用。读取路径通过 `AtomicReference.get()` 获取不可变的当前路由表快照，确保读取始终看到一致状态
