...
AbstractCapabilityExecutor.execute(request, capabilityId):
  ...
  for each strategy in this.degradationStrategies (sorted by getOrder() asc):
    if strategy.shouldDegrade(context):
      ...
      return CompletableFuture.completedFuture(doDegrade(startTime, strategy.getClass().getSimpleName(), request, capabilityId, departmentId, callerRole, callerId, visitId, patientId, sessionId, inputSummary, null, null))
