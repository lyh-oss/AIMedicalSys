# Phase 5 包 G — AI 进阶底座 架构级 OOD 设计方案（v12）
...
doExecuteInternal(startTime, request, capabilityId, departmentId, userId, sessionId, callerRole, callerId,
                   visitId, patientId):   // 新增参数：从 execute() 入口处独立提取
...
AiOrchestrator.handle(capabilityId, request):
  ...
  9.   // 处理方式：记录错误日志 + 返回 AiResult.failure() 而非传播为 CompletionException
    ...
    11.         metricsCollector.record(AiCallRecord.failure(capabilityId, LocalDateTime.now(), 0L, DegradationReason.INTERNAL_ERROR + ":" + e.getClass().getSimpleName(), e.getMessage(), null, null, null, null, null, null, null, null, null))
...
// 职责：在线程池外提取 ThreadLocal 上下文 → 防御性拷贝 → 降级预检 → 调用子类特化的 doExecuteInternal()
AbstractCapabilityExecutor.execute(request, capabilityId):
  // 线程池外提取 ThreadLocal 上下文，避免 supplyAsync 中 SecurityContextHolder / RequestContextHolder 丢失
  startTime = System.currentTimeMillis()
  auth = SecurityContextHolder.getContext().getAuthentication()
  userId = auth != null ? auth.getName() : "SYSTEM"
  departmentId = doExtractDepartmentId(request)   // 子类可重写：base 从 AiRequestBase.getDepartmentId() 获取；thin adapter 从 RequestContext/HTTP Header 获取
  visitId = doExtractVisitId(request)             // 子类可重写：base 从 AiRequestBase.getVisitId() 获取；thin adapter 从 RequestContext/HTTP Header 获取
  patientId = doExtractPatientId(request)         // 子类可重写：base 从 AiRequestBase.getPatientId() 获取；thin adapter 从 RequestContext/HTTP Header 获取
  sessionId = doExtractSessionId(request)         // 子类可重写：base 从 AiRequestBase.getSessionId() 获取；thin adapter 从 RequestContext/HTTP Header 获取
  callerRole = extractCallerRole()
  callerId = extractCallerId()

  // 防御性拷贝：若能力 DTO 尚未改为不可变，在容器线程（supplyAsync 之前）做深拷贝
  // 若 DTO 已为不可变（所有字段 final，无 setter），则跳过此步骤
  request = objectMapper.convertValue(request, request.getClass())
...
// inputSummary 已在 lambda 入口处定义；outputSummary/retryCount/outputType 在方法体内定义
doExecuteInternal(startTime, request, capabilityId, departmentId, userId, sessionId, callerRole, callerId, visitId, patientId):
    // 方法入口处定义局部变量
    outputType = getOutputType()                                  // 子类泛型令牌，由构造时传入的 Class<R> 提供
