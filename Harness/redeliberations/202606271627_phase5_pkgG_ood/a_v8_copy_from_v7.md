# Phase 5 包 G — AI 进阶底座 架构级 OOD 设计方案（v10）
...
classDiagram
    ...
    class AbstractCapabilityExecutor~T, R~ {
        ...
        #doExecuteInternal(startTime, request, capabilityId, departmentId) AiResult~R~~
...
- `userId` 由 `CapabilityExecutor.execute()` 内部从 Spring `SecurityContextHolder` 提取当前操作用户标识，采用 null-safe 操作：若 `getAuthentication()` 返回 null（如定时任务、匿名访问场景），回退使用 `"SYSTEM"` 作为 userId
...
doExecuteInternal(startTime, request, capabilityId, departmentId):
    ...
    try {
      ...
      return doDegrade(startTime, "InfrastructureError:" + e.getClass().getSimpleName(), request, capabilityId)
...
// 职责：降级预检 → 调用子类特化的 doExecuteInternal()
AbstractCapabilityExecutor.execute(request, capabilityId):
  return CompletableFuture.supplyAsync(() -> {
    startTime = System.currentTimeMillis()
...
doExecuteInternal(startTime, request, capabilityId, departmentId):
    ...
    auth = SecurityContextHolder.getContext().getAuthentication()  // null-safe: getAuthentication() 可能为 null
    ...
    try:
      ...
      // 结构化输出解析失败 → 进入降级路径（对应 §5.1 错误分类表）
      return doDegrade(startTime, "ParseFailure:" + e.getClass().getSimpleName(), request, capabilityId)
    elapsedMs = System.currentTimeMillis() - startTime
