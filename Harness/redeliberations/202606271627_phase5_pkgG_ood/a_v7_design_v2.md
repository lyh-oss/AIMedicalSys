...
CapabilityExecutorThinAdapter.execute(request, capabilityId):
  return CompletableFuture.supplyAsync(() -> {
    ...
    try {
      ...
      metricsCollector.record(AiCallRecord.failure(capabilityId, LocalDateTime.now(), elapsedMs, e.getClass().getSimpleName(), e.getMessage(), departmentId, ...))
    ...
    elapsedMs = System.currentTimeMillis() - startTime
    metricsCollector.record(AiCallRecord.success(capabilityId, LocalDateTime.now(), elapsedMs, departmentId, ...))
...
doThinDegrade(startTime, degradeReason, capabilityId, departmentId):
  elapsedMs = System.currentTimeMillis() - startTime
  metricsCollector.record(AiCallRecord.degraded(capabilityId, LocalDateTime.now(), elapsedMs, degradeReason, departmentId, ...))
...
AiOrchestrator.handle(capabilityId, request):
  ...
  9.   // 处理方式：记录错误日志 + 返回 AiResult.failure() 而非传播为 CompletionException
   10.   log.error("CapabilityExecutor 执行异常: capabilityId={}, error={}", capabilityId, e.getMessage(), e)
    11.   metricsCollector.record(AiCallRecord.failure(capabilityId, LocalDateTime.now(), 0L, "InternalError:" + e.getClass().getSimpleName(), e.getMessage(), ...))
...
doDegrade(startTime, degradeReason, request, capabilityId):
  ...
  if localRuleFallback != null:
    ...
    metricsCollector.record(AiCallRecord.degraded(capabilityId, LocalDateTime.now(), elapsedMs, degradeReason, ...))
    slidingWindowMetricsStore.recordDegraded(capabilityId, elapsedMs)
    return result
  else:
    metricsCollector.record(AiCallRecord.degraded(capabilityId, LocalDateTime.now(), elapsedMs, degradeReason, ...))
