# Phase 5 包 G — AI 进阶底座 架构级 OOD 设计方案（v19）
...
**`AiPlatformConfig` 类型定位**：使用 class 而非 interface，因配置装配为单一职责的具体实现，无多态替换需求。标注 `@Configuration` + `@ConfigurationProperties(prefix = "ai")` + `implements ApplicationContextAware`。不再实现 `EnvironmentPostProcessor`。
...
AiOrchestrator.handle(capabilityId, request):
  ...
  15.  inputSummary = request != null ? StringUtils.truncate(request.toString(), 500) : null
   ...
   24.    departmentId = extractFromRequestContext("X-Department-ID")
