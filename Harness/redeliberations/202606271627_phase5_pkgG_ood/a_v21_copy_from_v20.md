...
doExtractDepartmentId(request):
    ...
    return RequestContextUtils.extractFromRequestContext("X-Department-ID")

doExtractVisitId(request):
    // Phase 4 DTO 无 getVisitId()，从 RequestContext / HTTP Header / 当前就诊上下文独立提取
    return RequestContextUtils.extractFromRequestContext("X-Visit-ID")

doExtractPatientId(request):
    // Phase 4 DTO 无 getPatientId()，从 RequestContext / HTTP Header / 当前就诊上下文独立提取
    return RequestContextUtils.extractFromRequestContext("X-Patient-ID")
...
- **异常分类**（v21 新增）：`structuredChat()` 实现层应区分两类异常——(1) 模型不支持结构化输出时的 `StructuredOutputNotSupportedException`，语义为"回退到 `chat()` + `StructuredOutputParser.parse()`"；(2) 基础设施异常（HTTP 5xx、连接超时等）的 `LlmInfrastructureException`，语义为"直接降级，不尝试回退"。`CapabilityExecutor` 在 `doExecuteInternal()` 中根据异常类型分别处理
...
class AiPlatformConfig {
    ...
    LlmChatService delegatingLlmChatService(
            ...
            @Qualifier("springAiLlmChatService") LlmChatService springAi) {
        return new DelegatingLlmChatService(Map.of(
            ClientType.HTTP_API, httpApi,
            ClientType.SPRING_AI, springAi));
    }
}
```
