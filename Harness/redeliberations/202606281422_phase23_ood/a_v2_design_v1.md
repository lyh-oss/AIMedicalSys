...
三个新模块均为**扁平 Maven 模块**（不拆分为 api/impl 子模块），与 patient/doctor/admin 的结构一致。每个模块内部按职责分包：`api`（REST Controller）、`service`（业务接口与实现）、`repository`（JPA Repository）、`entity`（JPA 实体）、`dto`（请求/响应 DTO）、`converter`（实体/DTO 转换）。各模块依赖 `common`、`common-module-api` 和 `ai-api`，不依赖其他业务模块的 impl 层。
...
| `DialogueSession` | class（不可变） | 多轮对话会话不可变值对象，维护当前 session 的对话上下文、已收集的症状信息和当前轮次；通过 `withNewRound()` 返回包含新上下文的新实例 |
| `DialogueSessionManager` | class | 对话会话生命周期管理器，负责 session 的创建、查找（含过期判断）、更新和过期清理 |
| `TriageRuleEngine` | interface | 分诊规则引擎契约，根据症状匹配规则返回推荐科室；支持可配置规则源和运行期缓存刷新 |
...
| `TemplateConfigManager` | interface | 科室模板配置管理器契约，支持模板的运行时查询与缓存，科室标识不存在时回退 DEFAULT 模板 |
| `DepartmentTemplateAdminService` | interface | 科室模板管理契约，提供模板的 CRUD 操作接口，由管理员端调用 |
...
| `DosageThresholdService` | class | 剂量阈值校验服务，按药品、给药途径、年龄、体重维度检查剂量是否超限 |
...
│       ├── dto/
│       │   ├── TriageRequest.java           # 分诊请求（含主诉、sessionId，对话历史由服务端维护）
...
│       ├── dialogue/
│       │   ├── DialogueSession.java         # 多轮会话不可变值对象
│       │   └── DialogueSessionManager.java  # 会话生命周期管理（含过期清理）
│       ├── rule/
│       │   ├── TriageRuleEngine.java        # 分诊规则引擎接口
│       │   └── DefaultTriageRuleEngine.java # 默认规则引擎实现（基于数据库配置 + 定期刷新缓存）
│       ├── fallback/
...
│       │   └── assist/
│       │       └── PrescriptionAssistController.java  # 含 check-dose 及 AI 建议查询端点
│       ├── dto/
│       │   ├── audit/
...
│       │   └── assist/
│       │       ├── DosageCheckRequest.java  # 剂量检查请求（药品、给药途径、剂量、患者体征）
│       │       ├── DosageCheckResponse.java # 剂量检查响应（告警列表、AI 异步任务 taskId）
│       │       ├── DosageAlert.java         # 剂量告警值对象
│       │       └── AiSuggestionResponse.java # AI 建议查询响应（由 taskId 查询返回）
│       ├── service/
│       │   ├── audit/
...
│       │   └── DosageStandardRepository.java # 剂量标准 Repository（仅读操作）
│       ├── entity/
│       │   ├── AuditRecord.java            # 审核记录 JPA 实体
│       │   └── DosageStandard.java          # 剂量标准 JPA 实体（药品-给药途径联合定位剂量上限参考，处方模块仅读）
│       └── converter/
...
│       │   └── MedicalRecordAdminController.java  # 科室模板管理 REST 端点（管理员权限）
│       ├── dto/
...
│       │   ├── DatabaseTemplateConfigManager.java # 基于数据库的模板配置实现（含 DEFAULT 兜底）
│       │   └── DepartmentTemplateAdminService.java # 模板管理服务接口
│       ├── parser/
...
- 三个新模块均以 `compile` scope 依赖 `common`、`common-module-api` 和 `ai-api`；`ai-api` 和 `common-module-api` 中的依赖在 Maven `dependency:analyze` 中通过 `<ignoredUnusedDeclaredDependencies>` 豁免（与现有 patient/doctor/admin 模块的处理一致），直到业务代码产生真实引用
...
- **共享数据**：
  - `DosageStandard` 实体（药品剂量标准）由管理员模块作为唯一写入者，审核子域和辅助开方子域均以只读方式查询
  - `AuditRecord` 实体（审核记录）由审核子域写入，辅助开方子域读取以感知最新审核结果
...
#### `DialogueSession` — 多轮对话会话不可变值对象（immutable class）

**职责**：封装一次多轮分诊对话的完整上下文。核心状态包括：会话唯一标识、患者主诉、已收集的症状信息列表、当前追问轮次、AI 中间推理结果、当前会话状态（进行中/已完成/已超时）。提供 `withNewRound(question, answer)` 方法返回包含新增 QA 轮次的新实例。

**协作**：被 `DialogueSessionManager` 创建和持久化，被 `TriageServiceImpl` 在每轮迭代中通过 `withNewRound()` 生成新实例替换旧实例，实现无副作用的上下文演进。对话历史以服务端 `DialogueSession` 为单一真相来源，前端后续请求仅携带 sessionId。

**为何使用不可变 class + copy-on-write**：贯穿多个请求的会话状态需要明确的读写边界，不可变值对象避免引用逃逸导致的并发问题。copy-on-write 模式通过 `withNewRound()` 显式生成新实例，既保持不可变的安全性，又支持上下文积累。

#### `DialogueSessionManager` — 对话会话生命周期管理器（class）

**职责**：管理 `DialogueSession` 的创建、按 sessionId 查找（含过期判断）、更新和过期清理。

**协作**：
- 被 `TriageService` 在每次分诊请求时调用，用于创建新会话或恢复已有会话
- `findOrCreate(sessionId, chiefComplaint)` 行为：session 不存在或已过期时创建新会话；session 存在且未过期时返回已有会话
- 内部维护 `ConcurrentHashMap<String, DialogueSession>` 作为运行时存储，配合 `ScheduledExecutorService` 定期清理过期会话；每个 session 附带创建时间戳和 TTL（默认 30 分钟）
- session 过期后被清理，后续使用该 sessionId 的请求触发创建新会话（视作新一轮对话）
...
**职责**：根据症状-科室匹配规则，将主诉或已收集的症状列表映射到推荐科室集。规则以数据库表存储，支持运行期动态调整。

**协作**：
- 被 `TriageService` 在 AI 分诊结果为空或置信度不足时调用
- 内部持有规则配置数据源（数据库表），通过定时刷新或 Caffeine 缓存 `refreshAfterWrite` 机制实现规则变更的热加载，无需重启服务

**为何使用 interface**：规则源可能从数据库演化为更复杂的规则引擎，interface 隔离规则消费方与规则存储实现。
...
- 读取 `DosageStandard` 时仅通过 `DosageStandardRepository` 只读查询
...
- 内部委托 `TemplateConfigManager` 获取当前科室的生成规则（科室标识不存在时回退 DEFAULT 模板）
...
**职责**：封装单个科室的病历生成规则配置，包括：科室标识（含保留标识 `DEFAULT` 作为全局兜底）、模板版本号、字段映射规则、必填字段列表、Prompt 模板内容。
...
**职责**：按科室标识和版本号查询模板配置，支持运行期热加载。查询契约：优先返回科室专属模板；科室标识不存在时返回 `departmentId = DEFAULT` 的兜底模板；若 DEFAULT 模板也不存在，返回空模板结构。

**协作**：
- 被 `MedicalRecordService` 在每次生成请求开始时调用
- 内部维护 `LoadingCache`（Caffeine 或 Guava）缓存模板配置，通过 Caffeine 的 `refreshAfterWrite` 定时刷新或配置变更事件触发缓存失效

**为何使用 interface**：缓存策略可替换（本地缓存 → 分布式缓存），interface 隔离缓存实现与业务消费方。

#### `DepartmentTemplateAdminService` — 科室模板管理服务接口（interface）

**职责**：提供科室模板的 CRUD 操作接口，包括创建、更新、删除、查询科室模板。由管理员端（`MedicalRecordAdminController`）调用，确保只有授权管理员可修改模板。

**协作**：
- 被 `MedicalRecordAdminController` 调用
- 内部委托 `DeptTemplateConfigRepository` 持久化模板数据
- 模板变更后通知 `TemplateConfigManager` 失效对应缓存

**为何使用 interface**：管理端操作可能与业务端查询有不同的部署和权限要求，interface 隔离管理契约与底层存储。
...
**职责**：定义处方合理性辅助检查的业务边界：接收医生正在录入的药品和剂量 → 执行剂量阈值检查 → 返回告警列表和 AI 异步任务标识。

**协作对象**：
- 被 `PrescriptionAssistController` 调用（前端在医生输入药品和剂量时实时触发）
- 内部委托 `DosageThresholdService` 执行剂量校验（同步）
- 内部委托 `PrescriptionAuditService` 获取最新审核结果作为辅助参考（可选）
- 内部委托 `AiService.prescriptionAssist()` 获取 AI 合理性建议（在后端异步调用，不影响剂量阈值检查的即时响应）；异步调用完成后，结果存入内部存储（内存 Map 或数据库），关联 taskId 供后续查询

**为何使用 interface**：辅助开方未来可能扩展更多检查维度（药物-食物相互作用、肾功能调整剂量等），interface 保持契约稳定。

#### `DosageThresholdService` — 剂量阈值校验服务（class）

**职责**：按药品编码和给药途径联合查询剂量标准（`DosageStandard`），比较录入剂量是否在安全范围内。超限时生成 `DosageAlert`。`DosageStandard` 以 `drugCode + routeOfAdministration` 为联合键定位阈值记录。

**协作对象**：
- 被 `PrescriptionAssistService` 调用
- 内部以只读方式委托 `DosageStandardRepository` 查询标准剂量配置
- 剂量数据的写入由管理员模块的 `DosageStandardManagementService` 负责

**为何使用 class**：剂量阈值校验逻辑稳定，无多态变体需求；直接使用 class 降低不必要的抽象层次。

#### `DosageAlert` — 剂量告警值对象（class）

**职责**：封装一条剂量告警的完整信息：告警级别（WARN/BLOCK）、告警消息（如"xx 药品单次剂量超出成人最大推荐剂量"）、药品编码、给药途径、当前剂量、建议调整值。
...
```
POST /api/triage/consult
  请求体: { sessionId: "xxx", chiefComplaint: "胸痛" }
  → TriageController
    → DialogueSessionManager.findOrCreate(sessionId, chiefComplaint)
      → session 不存在或已过期 → 创建新会话（首次请求或会话超时）
...
正常路径（第一轮）:
  ...
  → AI 确认需要追问 → TriageResponse.requiresFollowUp = true
    ...
    → 前端展示追问并收集用户回复
    → DialogueSession.withNewRound(question, answer) 创建新实例
    → DialogueSessionManager 更新会话（以新实例替换旧实例）

正常路径（第二轮及以后）:
  → AiService.triage(sessionId, 累积对话上下文)
  → AI 结合全部上下文给出最终推荐科室
  → TriageResponse.requiresFollowUp = false
  → DialogueSession 标记为已完成

会话过期处理:
  → 前端使用已过期/不存在的 sessionId 请求
  → DialogueSessionManager.findOrCreate 检测过期
  → 返回 BusinessException(ErrorCode.TRIAGE_SESSION_EXPIRED)
  → 前端提示"对话会话已过期，请重新开始分诊"

超时/降级路径（同单轮分诊的规则/兜底回退）
```

**关键变更说明**：对话历史以服务端 `DialogueSession` 为单一真相来源，后续请求中前端仅携带 `sessionId`，不再发送完整 `history`。`TriageRequest` 不再包含 `history` 字段。
...
正常路径:
  → TemplateConfigManager.getTemplate("NEUROLOGY")
    → NEUROLOGY 模板存在 → 加载神经内科的病历模板配置
    → NEUROLOGY 模板不存在 → 加载 DEFAULT 模板
    → DEFAULT 模板也不存在 → 返回空模板结构
...
```
管理员操作:
  POST   /api/medical-record/admin/templates          → 创建科室模板
  PUT    /api/medical-record/admin/templates/{deptId}  → 更新科室模板
  DELETE /api/medical-record/admin/templates/{deptId}  → 删除科室模板
  GET    /api/medical-record/admin/templates           → 查询所有模板
  GET    /api/medical-record/admin/templates/{deptId}  → 查询指定模板

初始化数据:
  系统首次部署时，通过 SQL 脚本或数据迁移工具创建 DEFAULT 模板条目：
  departmentId = "DEFAULT", version = "1.0", 包含标准病历字段集
  （主诉、现病史、既往史、体格检查、辅助检查、初步诊断、治疗意见）

缓存一致性:
  模板创建/更新/删除操作完成后，DepartmentTemplateAdminService
  触发 TemplateConfigManager 的缓存失效，确保后续查询获取最新配置
```
...
正常路径:
  → DosageThresholdService.check(drugCode, routeOfAdministration, dosage, patientInfo)
    → DosageStandardRepository.findByDrugCodeAndRoute(drugCode, routeOfAdministration)
    → 比较剂量与标准阈值（单次上限、日上限、按体重调整值）
    → dosage 在安全范围内 → 返回空告警列表
    → dosage 超限 → 生成 DosageAlert(WARN/BLOCK, message, suggestedDosage)
  → 异步调用 AiService.prescriptionAssist() 获取 AI 合理性建议
    → 异步任务提交后，返回 taskId（不阻塞主响应）
    → AI 结果完成后按 taskId 存储
...
响应: Result<DosageCheckResponse> {
  alerts: [ { level: "WARN", message: "...", suggestedAdjustment: 250 } ],
  taskId: "async-task-uuid-xxx"  // AI 异步任务的标识，可用于查询 AI 建议
}
```
...
GET /api/prescription/assist/suggestion/{taskId}
  → PrescriptionAssistController
    → PrescriptionAssistService.getAiSuggestion(taskId)
    → 根据 taskId 查询 AI 异步调用结果
    → AI 结果已完成 → 返回 AiSuggestionResponse { suggestion: "...", status: "COMPLETED" }
    → AI 结果尚未完成 → 返回 AiSuggestionResponse { suggestion: null, status: "PENDING" }
    → taskId 不存在或已过期 → 返回 BusinessException(ErrorCode.RX_ASSIST_SUGGESTION_NOT_FOUND)
```

**给药途径枚举**：`routeOfAdministration` 定义为枚举，包含但不限于 `ORAL`（口服）、`INTRAVENOUS`（静脉注射）、`INTRAMUSCULAR`（肌肉注射）、`TOPICAL`（外用）、`SUBLINGUAL`（舌下）、`RECTAL`（直肠给药）。`DosageStandard` 实体增加该维度，以 `drugCode + routeOfAdministration` 为联合键定位剂量阈值记录。临床中口服与静脉的剂量阈值差异显著，该字段确保校验准确性。
...
|---------|-----------|---------|---------|
| 分诊错误 | `TRIAGE_` | 会话超时/已过期(`TRIAGE_SESSION_EXPIRED`)、分诊参数缺失 | `BusinessException` → `Result` |
| 审核错误 | `RX_AUDIT_` | 处方格式不合法、药品编码不存在 | `BusinessException` → `Result` |
| 病历错误 | `MR_` | 对话文本过短、科室模板不存在 | `BusinessException` → `Result` |
| 开方辅助错误 | `RX_ASSIST_` | 药品剂量标准不存在、剂量数据异常、AI 建议查询未找到(`RX_ASSIST_SUGGESTION_NOT_FOUND`) | `BusinessException` → `Result` |
...
- 会话过期清理使用 `ScheduledExecutorService` 定时任务（每 5 分钟检查一次），清理过期 session（TTL = 30 分钟），清理逻辑对运行时请求无影响
- 会话过期后，前端使用原 sessionId 发起请求时，`DialogueSessionManager.findOrCreate` 检测过期状态并抛出 `BusinessException(TRIAGE_SESSION_EXPIRED)`，引导前端创建新会话
...
`PrescriptionAssistService.checkDosage()` 在返回剂量阈值检查结果后，使用 `@Async` 或 `CompletableFuture.runAsync()` 异步调用 `AiService.prescriptionAssist()`。主响应不等待 AI 结果，直接返回包含 `taskId` 的 `DosageCheckResponse`。前端可通过 `GET /api/prescription/assist/suggestion/{taskId}` 查询 AI 建议的完成状态和内容。异步结果存储于内部的 `ConcurrentHashMap<String, AiSuggestionResult>` 中，关联 taskId 和创建时间戳，配合定时清理机制，避免内存泄漏。
...
`DosageStandard` 的初始数据通过 SQL 种子脚本加载，脚本路径约定为：
```
backend/modules/prescription/src/main/resources/db/seed/R__init_dosage_standard.sql
```
脚本采用可重复执行模式（`R__` 前缀），使用 `MERGE` 或 `INSERT ... ON CONFLICT` 语句确保幂等性。

### 7.2 药品编码规范

药品编码采用国家药品编码标准（国药准字号编码取其标准化形式），格式为 `国药准字{类别字母}{8位数字}` 的脱前缀编码，或采用医院内部统一的药品编码体系。编码规范在项目级的 `common` 模块中以常量或校验注解定义，各新模块引用该规范。

### 7.3 单位一致性校验

`DosageThresholdService` 在执行剂量比较前，校验传入剂量单位与 `DosageStandard` 中配置的单位是否一致。若单位不匹配，统一按 `DosageStandard` 中的基准单位进行换算（仅支持 SI 单位体系内的单位换算，如 mg ↔ g、mL ↔ L），无法换算时返回 `BusinessException(RX_ASSIST_UNIT_MISMATCH)`。
...
| 对话历史真相来源 | 前端维护 vs 服务端维护 | 服务端 `DialogueSession` 为单一真相来源 | 消除前端历史与服务端上下文的不一致风险。前端后续请求仅携带 `sessionId`，对话历史在服务端 `DialogueSession` 中以 copy-on-write 模式安全演进 |
| 分诊规则源与生效机制 | YAML 配置 vs 数据库 + 启动时缓存 vs 数据库 + 定时刷新 | 数据库（配置表）+ Caffeine `refreshAfterWrite` 定时刷新 | 分诊规则需要支持非开发人员（业务运营）动态调整。数据库存储配合定时刷新（如每 5 分钟）实现热加载，既满足动态调整需求，又避免频繁查库的性能开销 |
...
| 科室模板兜底策略 | 缺失返回空 vs 空白模板 vs DEFAULT 模板 | DEFAULT 模板兜底 | 系统预置 `departmentId = "DEFAULT"` 的模板条目，任何未单独配置的科室均可得到合理的病历生成模板。初始化时通过 SQL 种子脚本创建该条目 |
| 包E 的 AI 调用时机 | 同步（阻塞主响应） vs 异步（不阻塞） | 剂量阈值检查同步返回 + AI 建议异步调用 | 剂量阈值是即时性要求高的安全检查，必须同步返回结果阻塞前端操作。AI 合理性建议是增值信息，异步调用不增加主响应延迟。异步结果通过 `taskId` 关联，由前端按需查询 |
| 包E 与包D-AI1 的协作方式 | 事件驱动 vs 直接方法调用 | 同一模块内的直接方法调用 | 两者同属一个 Maven 模块，直接调用不产生循环依赖。如果未来分离为独立模块，则通过门面接口或 REST 调用替代 |
| `DosageStandard` 写权限 | 处方模块内多子域共同写入 vs 单一写入者 | 管理员模块 `DosageStandardManagementService` 作为唯一写入者 | 符合领域设计原则——数据维护职责应归属单一权威方。处方审核和辅助开方子域仅持有读取权限，避免多写入者导致的数据不一致和权限扩散 |
| 给药途径维度 | 药品编码唯一定位 vs `drugCode + routeOfAdministration` 联合定位 | `drugCode + routeOfAdministration` 联合定位 | 临床中口服与静脉的剂量阈值差异悬殊，同一药品不同给药途径的安全范围不同。联合键确保剂量校验准确 |
...
| 审查意见 | 修改措施 |
|---------|---------|
| DialogueSession 不可变声明与可变追加操作的逻辑矛盾 | 保留不可变 class 定位，增加 `withNewRound(question, answer)` copy-on-write 方法，协作描述中改为"生成新实例替换旧实例" |
| 包E 异步 AI 建议缺少消费路径 | 新增 `GET /api/prescription/assist/suggestion/{taskId}` 查询端点；`DosageCheckResponse` 增加 `taskId` 字段；后端异步结果以 `ConcurrentHashMap` 暂存，关联 taskId 供查询 |
| 多轮分诊中对话历史的维护责任与一致性不明确 | 以服务端 `DialogueSession` 为单一真相来源，前端后续请求仅携带 `sessionId`，`TriageRequest` 移除 `history` 字段 |
| DosageCheckRequest 缺少给药途径参数 | `DosageCheckRequest` 增加 `routeOfAdministration` 枚举字段；`DosageStandard` 增加该维度；`DosageThresholdService` 以 `drugCode + routeOfAdministration` 联合键查询 |
| prescription 模块内 DosageStandard 实体的写权限归属未定义 | 管理员模块 `DosageStandardManagementService` 作为唯一写入者；处方模块（D-AI1 和 E）仅持有只读权限；设计决策中增加该项 |
| 对话会话内存存储未覆盖服务重启场景 | 增加 `TRIAGE_SESSION_EXPIRED` 错误码；`findOrCreate` 明确过期处理策略（session 不存在或过期时视为新会话）；session TTL 设为 30 分钟 |
| 新模块依赖声明未包含 common-module-api | 三个新模块均补充 `common-module-api` 的 `compile` scope 依赖，更新模块依赖示意图和文本描述 |
| 分诊规则配置变更的生效机制未定义 | 规则引擎从"启动时缓存"改为"Caffeine `refreshAfterWrite` 定时刷新"（每 5 分钟），支持热加载；更新设计决策理由 |
| 科室模板配置的 CRUD 管理和默认兜底缺失 | 增加 `DepartmentTemplateAdminService` 接口和 `MedicalRecordAdminController` 端点；`TemplateConfigManager.getTemplate` 增加 DEFAULT 模板回落机制；补充初始 DEFAULT 模板数据集描述 |
| 剂量标准数据初始化方案和编码规范缺失 | 新增第 7 章描述初始化方案（`R__init_dosage_standard.sql` 种子脚本）、药品编码规范（国药准字号标准化）、单位一致性校验逻辑及 `RX_ASSIST_UNIT_MISMATCH` 错误码 |


<｜｜DSML｜｜parameter name="filePath" string="true">C:\Develop\Software\AIMedicalSys\Harness\redeliberations\202606281422_phase23_ood\a_v2_design_v1.md