...
| AllergyWarningItem | class（DTO） | 过敏冲突告警值对象，对齐需求文档 3.4.10 allergy_warnings 数组每项结构。包含 drugId（药品编码）、allergen（过敏原）、severity（AllergyWarningSeverity 枚举：CRITICAL/WARNING/INFO） |
...
职责：过敏告警严重程度枚举，定义 CRITICAL / WARNING / INFO 三个级别。CRITICAL 表示严重过敏冲突（语义对应 AuditRiskLevel.BLOCK 但不等同——辅助开方 allergyWarnings.severity=CRITICAL 为编辑阶段即时提示，处方审核的 BLOCK 为提交时正式阻断判定，二者独立执行不互斥），WARNING 表示中度冲突，INFO 表示轻微提示。此枚举与 DosageAlertLevel（INFO/WARNING/CRITICAL）和 AlertSeverity（INFO/WARNING/CRITICAL）分属不同告警维度，三者已统一为 INFO/WARNING/CRITICAL 命名约定。
...
- drugId（string，药品编码）
- allergen（string，过敏原）
- severity（AllergyWarningSeverity 枚举：CRITICAL / WARNING / INFO，告警严重程度）
...
规则变更后通过 ApplicationEventPublisher 发布对应变更事件（TriageRuleChangeEvent、DrugContraindicationChangeEvent、DrugAllergyMappingChangeEvent、DrugCompositionDictChangeEvent、DrugInteractionPairChangeEvent），消费端（如 DefaultTriageRuleEngine、LocalRuleEngine 中各规则类）监听后刷新缓存。TemplateConfigChangeEvent 与规则变更事件复用同一事件发布/监听框架。
