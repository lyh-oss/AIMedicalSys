...
- **底座直接落地**：四个包均直接以 Maven 模块形式落地在 AIMedical 后端底座上，遵循 common → modules → application 的分层架构，规避后续迁移至 Phase 5 AI 进阶底座的重构成本。业务模块仅依赖 ai-api 的 AiService 接口（编译期依赖）；若 Phase 5 保持 AiService 接口签名不变，业务模块的核心 Service 和 Controller 代码无须修改；Converter 层需随 ai-api DTO 字段变更同步更新
...
| DialogueSessionManager | class | 对话会话生命周期管理器，负责 session 的创建、查找、更新和过期清理，承担并发访问控制职责；接受前端传入的 UUID v4 并验证格式，首次使用时创建会话 |
...
| LocalRuleEngine | interface | 本地规则校验引擎契约，封装 6 条独立规则——DrugInteractionRule（DDI，Phase 2/3 预留骨架）、AllergyCheckRule（过敏检查）、ContraindicationCheckRule（合并症禁忌检查）、DuplicateCheckRule（重复用药检查）、DosageLimitRule（剂量范围检查）、SpecialPopulationDosageRule（特殊人群剂量检查）；AI 超时或不可用时作为降级回退。各规则独立实现/测试/启用/禁用，具体职责见 §3.2 实现范围表 |
...
| AiSuggestionResult | class | 异步 AI 结果值对象，封装 taskId、suggestion（String，COMPLETED 时填充）、status（AiSuggestionStatus 枚举：PENDING / COMPLETED / FAILED）、createTime、failReason（可选）、partialData（可选，AI 超时或部分生成时携带部分结果）；AiSuggestionResult 实例由 ConcurrentHashMap 存储，通过 compute() 原子替换整个实例保证并发安全，同一 taskId 的状态更新保证幂等 |
...
| DosageCheckRequest | class（DTO） | 剂量阈值即时校验请求值对象，对齐需求文档 3.4.10 check-dose 输入契约。包含处方标识（prescriptionId，必填，String——作为 PrescriptionDraftContext 写入键）、药品编码（drugCode）、剂量（dosage）、单位（unit）、给药途径（routeOfAdministration）、患者年龄（patientAge，可选）、患者体重（patientWeight，可选）、用药频率（frequency，可选，用于日剂量校验） |
...
| AllergyWarningItem | class（DTO） | 过敏冲突告警值对象，对齐需求文档 3.4.10 allergy_warnings 数组每项结构。包含 drugId（药品编码）、allergen（过敏原）、severity（AllergyWarningSeverity 枚举：INFO/WARNING/CRITICAL） |
...
**RegistrationEvent 消费失败补偿策略**：RegistrationEventListener 采用 @Retryable（Spring Retry，重试间隔 2s，最多 3 次，exponential backoff）处理事件消费失败。若重试仍失败，异常由 @Recover 方法（RetryOperationsInterceptor 的 recover 回调）接收，写入 dead_letter_event 表（含 eventPayload、failReason、failTime），由定时补偿任务（每 30 分钟扫描重试未成功的死信事件）兜底。

**dead_letter_event 表模块归属**：DeadLetterEvent 实体定义在 consultation 模块（com.aimedical.modules.consultation.entity.DeadLetterEvent），对应 Repository 为 DeadLetterEventRepository（com.aimedical.modules.consultation.repository.DeadLetterEventRepository）。定时补偿任务封装为 DeadLetterCompensationService（com.aimedical.modules.consultation.service.DeadLetterCompensationService），由 ScheduledExecutorService 每 30 分钟调度一次，扫描 state=FAILED 且 retryCount < maxRetryCount（默认 3 次）的死信事件重新投递，完成后更新 state=COMPENSATED 或递增 retryCount。补偿任务在 consultation 模块内部完成，不依赖外部模块。
...
AiService 接口定义在 ai-api 模块（com.aimedical.modules.ai.api.AiService），是各业务模块调用 AI 能力的统一入口。各方法使用泛型 `CompletableFuture<AiResult<T>>` 封装异步 AI 调用结果，具体方法签名及泛型绑定如下：
...
**AiResult<T> 泛型要点**：各方法的泛型 T 分别绑定为上述 4 个 ai-api 层 Response DTO；AiResult 统一包含 success（boolean）、data（T，成功时填充）、errorCode（String，可选）、degraded（boolean）、fallbackReason（String，可选）、partialData（T，可选，通过 failure()/degraded() 重载传入超时/降级部分结果）。超时/降级重载方法（AiResult.failure(String errorCode, T partialData) / AiResult.degraded(String fallbackReason, T partialData)）的泛型参数 T 绑定的 ai-api 层 DTO 当前为空壳类（仅含默认构造器，完整字段定义待 §10 扩展），使用前需先完成 ai-api 层 DTO 的字段扩展——此为前提条件，时序依赖已在 §10 首段标注。
...
**全量拼接 Token 超限风险评估**：全量拼接策略在多轮长对话场景下，累计对话文本长度可能超出 AI 模型的上下文窗口（如 4K/8K token 限制）。为控制此风险，DialogueSession 在追加每轮 QA 时维护累计字符数，当累计字符数超过预配置阈值（默认 3000 字符，可配置参数 `triage.max-context-chars`）时，触发"上下文截断"策略——保留首轮主诉 + 最近 N 轮 QA（窗口策略），丢弃中间轮次。截断后前端继续显式展示完整对话历史，AI 调用仅传入精简后的推理上下文。此截断风险在 Phase 2/3 分诊场景（通常 2-5 轮）下发生概率低，但设计层面做前瞻性备案，阈值参数预留可配置扩展。
...
**主诉修正支持**：DialogueSession 增加 correctedChiefComplaint 可选字段。首轮记录原始 chiefComplaint；多轮场景中若患者主动修正主诉（如"我之前说的不对，其实是头痛"），TriageServiceImpl 识别到并填充 correctedChiefComplaint。全量拼接时若 correctedChiefComplaint 存在则替换原始 chiefComplaint 作为推理上下文起点。此字段可通过前端显式传入的"主诉修正"标记触发更新，或由 AI 在追问回复中隐式识别。
...
并发安全策略：AiSuggestionResult 实例由 ConcurrentHashMap 存储，通过 compute() 原子替换整个实例保证并发安全。
...
职责：处方草稿上下文，在医生编辑处方期间暂存 CRITICAL 级别剂量告警标记。内部使用 ConcurrentHashMap\<String, List\<DosageAlert\>\> 存储，Key 为 prescriptionId（系统层 UUID 标识，草稿创建时分配——与 AuditRecord.prescriptionId 是同一标识，贯穿草稿→审核→提交全流程；prescriptionOrderId 是提交成功后分配的业务单据号，在草稿编辑阶段尚不存在，因此 PrescriptionDraftContext 以 prescriptionId 为键）。

生命周期管理：创建时机（首次 check-dose 请求时——通过 private final ConcurrentHashMap 字段声明即初始化（new ConcurrentHashMap<>()），不存在空引用风险；写入操作通过 compute() 或 put() 直接写入，由 ConcurrentHashMap 内部保证首次写入时 Entry 的原子创建）、写入时机（CRITICAL 告警时，每次 check-dose 重算后覆盖该 prescriptionId 下已有的所有 CRITICAL 标记——若新结果无 CRITICAL 则清除旧标记）、清理时机（处方提交成功/处方取消/TTL 过期 60 分钟）。**TTL 过期清理由 ScheduledExecutorService 定期扫描实现（每 5 分钟扫描一次，清理最后更新时间超过 60 分钟的条目）**，确保异常退出场景下 CRITICAL 标记不会无限残留。

**实例化保证**：PrescriptionDraftContext 作为 Spring @Component 单例 Bean，其内部 ConcurrentHashMap 在构造器执行时完成初始化（声明时 `new ConcurrentHashMap<>()`），不存在"写入前 Map 为 null"的空指针风险。所有写入操作在对应 Service 方法内同步执行，无需外部队列或回调保证。
...
职责：过敏告警严重程度枚举，定义 INFO / WARNING / CRITICAL 三个级别（从轻到重）。CRITICAL 表示严重过敏冲突（语义对应 AuditRiskLevel.BLOCK 但不等同——辅助开方 allergyWarnings.severity=CRITICAL 为编辑阶段即时提示，处方审核的 BLOCK 为提交时正式阻断判定，二者独立执行不互斥），WARNING 表示中度冲突，INFO 表示轻微提示。此枚举与 DosageAlertLevel（INFO/WARNING/CRITICAL）和 AlertSeverity（INFO/WARNING/CRITICAL）分属不同告警维度，三者已统一为 INFO/WARNING/CRITICAL 命名约定。
...
- drugId（string，药品编码）
- allergen（string，过敏原）
- severity（AllergyWarningSeverity 枚举：INFO / WARNING / CRITICAL，告警严重程度）
...
处方提交端点设计边界:
  ...
  行为:
    1. forceSubmit=false 时执行常规审核流程（若待提交处方无最新审核结果则先调用审核）；若已有 WARN 级别最新审核结果，对当前处方做轻量级内容变更检测（drugId+dose+frequency+duration+route 五字段结构化哈希比对，与 AuditRecord.originalPrescription 的哈希比较），若检测到内容变更则返回 RX_AUDIT_PRESCRIPTION_MODIFIED 错误码并提示"处方内容已变更，请重新审核"，防止返回基于旧处方的已过期审核结果；若哈希一致则直接返回当前 WARN 审核结果并提示医生选择强制提交或修改处方，避免无意义重复审核
    2. forceSubmit=true 时校验 auditRecordId 对应 AuditRecord.riskLevel=WARN 且 isLatest=true
       ...
       → 校验失败：返回错误码
     3. 阻断合并判定（路径 A——先完整收集后阻断）：同时检查最新审核结果是否为 BLOCK 和 PrescriptionDraftContext 中该 prescriptionId 是否存在 CRITICAL 级别剂量告警，合并产生单一 BlockResponse。若 BLOCK 和 CRITICAL 均命中，BlockResponse 同时包含审核阻断原因和剂量告警阻断原因；任一命中即拒绝提交，返回 HTTP 422。CRITICAL 阻断独立于审核 BLOCK 阻断——二者为不同判断维度，任一命中均拒绝提交
...
→ 生成 taskId（UUID v4，36 字符含连字符），作为异步 AI 建议查询标识
→ 以 PENDING 状态预创建 AiSuggestionResult（key=taskId，createTime=now）
...
→ 异步 AI 调用完成后更新 AiSuggestionResult 状态为 COMPLETED 或 FAILED
→ AiSuggestionResult TTL 30 分钟，由 ScheduledExecutorService 定期清理过期条目
→ 四级均未命中时 DosageAlert.errorCode=RX_ASSIST_DOSE_STANDARD_NOT_FOUND

[异步建议查询] GET /api/prescription/assist/suggestion/{taskId}
→ 不存在 / TTL 过期: RX_ASSIST_SUGGESTION_NOT_FOUND
...
| 审核（非AI） | RX_AUDIT_ | RX_AUDIT_PRESCRIPTION_MODIFIED（处方版本校验不一致） |
...
| 开方辅助（非AI） | RX_ASSIST_ | RX_ASSIST_DOSE_STANDARD_NOT_FOUND、RX_ASSIST_SUGGESTION_NOT_FOUND |
