# Phase 2/3 包C/D-AI1/D-AI2/E — 架构级 OOD 设计方案（v4）
...
| TriageService | interface | 智能分诊业务契约，封装单轮/多轮对话的分诊逻辑、对话状态管理和规则回退；协调三级降级链：AI → 规则引擎 → 兜底提供者 |
...
| DepartmentFallbackProvider | interface | 兜底科室列表提供者契约，当 AI 和规则引擎均无法决策时返回静态部门列表或基于简单规则的匹配结果 |
...
| PrescriptionAuditService | interface | 处方审核业务契约，协调 AI 审核调用与本地规则回退；审计结果为最终判定，调用方需据此执行阻断决策。正常路径和降级路径均须写入 AuditRecord，降级路径额外标记 fromFallback=true |
| AuditRiskLevel | enum | 风险等级枚举，定义 PASS（通过）、WARN（警告）、BLOCK（阻断）三个级别，决定前端行为；BLOCK 级别在后端具有强制阻断语义 |
| AuditRecord | JPA @Entity | 审核记录持久化实体，保存每次审核的原始处方、AI 结果、风险等级、时间戳和降级标记（fromFallback），以及处方单号、医生 ID、患者 ID 等业务关联标识 |
...
| MedicalRecordField | enum | 病历结构化输出顶层字段标识枚举，定义主诉、现病史、既往史、体格检查、辅助检查、初步诊断、治疗意见等标准字段；用作 DepartmentTemplateConfig.requiredFields 的类型和 RecordGenerateResponse 中字段标识 |
| DepartmentTemplateConfig | class | 科室模板配置值对象（科室标识、版本号、必填字段列表 List<MedicalRecordField>、Prompt 模板） |
| TemplateConfigManager | interface | 科室模板配置管理器契约，支持模板的运行时查询、缓存和热加载；科室标识不存在时兜底返回 DEFAULT 模板。模板变更时发布 TemplateConfigChangeEvent 并写入配置变更审计日志 |
...
| DosageThresholdService | class | 剂量阈值校验服务，按药品编码、给药途径、年龄范围、体重范围维度匹配剂量标准，检查剂量是否超限；内含单位一致性校验（DosageUnitGroup 分组匹配 + 自动换算），无法换算时返回错误码 |
| DosageUnitGroup | enum | 剂量单位兼容分组枚举，定义 MASS_GROUP（mcg↔mg↔g）、VOLUME_GROUP（ml↔L）等可换算分组；IU 等非质量/体积单位单独分组或标记为不可换算 |
...
│       ├── dto/
│       │   ├── TriageRequest.java
│       │   ├── TriageResponse.java          # departments, sessionId, needFollowUp, followUpQuestion, confidence
│       │   └── DialogueCreateRequest.java   # 首轮分诊请求 DTO：chiefComplaint, patientId
│       ├── service/
│       │   ├── TriageService.java
...
│       ├── dto/
│       │   ├── audit/
│       │   │   ├── AuditRequest.java, AuditResponse.java, AuditIssue.java, BlockResponse.java
│       │   │   └── AuditRecordQuery.java
│       │   └── assist/
...
│       │   └── assist/DosageThresholdService.java    # 含 DosageUnitGroup 枚举和换算常量
│       ├── rule/
...
│       ├── entity/
│       │   ├── AuditRecord.java              # 含 prescriptionOrderId, doctorId, patientId, fromFallback
│       │   └── ConfigChangeLog.java          # 配置变更审计日志实体
...
│       ├── dto/
│       │   ├── RecordGenerateRequest.java   # dialogText, departmentId, patientId
│       │   ├── RecordGenerateResponse.java  # fields: Map<MedicalRecordField, String>, missingFields: List<MedicalRecordField>
│       │   └── FieldMissingHint.java
│       ├── service/MedicalRecordService.java + impl/
│       ├── template/
│       │   ├── TemplateConfigManager.java   # getTemplate 兜底返回 DEFAULT
│       │   ├── DepartmentTemplateConfig.java # requiredFields: List<MedicalRecordField>
│       │   └── DatabaseTemplateConfigManager.java # 定时/事件驱动缓存刷新; 配置变更时写入 ConfigChangeLog
│       ├── parser/MissingFieldDetector.java
│       ├── repository/MedicalRecordRepository.java, DeptTemplateConfigRepository.java
│       ├── entity/
│       │   ├── MedicalRecord.java
│       │   ├── MedicalRecordField.java      # enum: CHIEF_COMPLAINT, HISTORY_OF_PRESENT_ILLNESS, ...
│       │   └── DeptTemplateConfig.java
...
职责：定义智能分诊的核心业务边界，覆盖单轮分诊、多轮追问和会话管理。实现三级降级链：AI 有结果 → 直接返回推荐科室；AI 无结果 → 委托 TriageRuleEngine.match() 规则匹配；规则匹配无结果 → 委托 DepartmentFallbackProvider 返回静态兜底列表。
...
协作：被 TriageService 在 AI 不可用或无结果时调用。配合 Caffeine refreshAfterWrite（默认 60 秒）定时刷新规则缓存。
...
职责：处方审核核心流程：接收处方→调用 AI→AI 不可用时回退本地规则→返回风险等级。无论正常路径还是降级路径，均须写入 AuditRecord（降级路径标记 fromFallback=true）。审计结果的 BLOCK 等级具有强制阻断语义，调用方（Controller 或 Orchestrator）必须根据 AuditRiskLevel 执行阻断决策，不得将 BLOCK 处方提交至下一环节。

协作：委托 AiService.prescriptionCheck()、LocalRuleEngine；写入 AuditRecord（含 fromFallback 标记）；返回结果中包含 riskLevel 和阻断原因集合。
...
- fromFallback：boolean 标记，true 表示该审核结果来源于本地规则降级（非 AI 结果），用于区分审计追溯时 AI 结果与本地规则结果的来源

这些关联标识使 AuditRecord 具备完整的业务可追溯性，支持按医生、患者、处方单号三个维度的查询和分析，fromFallback 标记进一步区分了结果来源。
...
| TREATMENT_ADVICE | 治疗意见 |

该枚举用作 DepartmentTemplateConfig.requiredFields 的元素类型（List<MedicalRecordField>），以及 RecordGenerateResponse 中字段标识的 key。后续可扩展科室专属字段（如 INTERNAL_MEDICINE 可新增 CARDIOVASCULAR_HISTORY），扩展时只需新增枚举值并配置对应科室模板即可。
...
- departmentId：科室标识
- version：版本号
- requiredFields：List<MedicalRecordField> 类型，定义该科室模板必填字段集合
- promptTemplate：Prompt 模板文本

requiredFields 的类型明确为 List<MedicalRecordField>，与 §9.1 中 DEFAULT 模板字段列表形成呼应，使字段标识符与模板内容一致。

#### TemplateConfigManager（interface）

getTemplate(departmentId)：优先返回指定科室模板，不存在时返回 DEFAULT 兜底。Caffeine refreshAfterWrite + 事件驱动缓存失效。模板配置变更时，除了发布 TemplateConfigChangeEvent 外，还写入配置变更审计日志（ConfigChangeLog），确保配置操作可追溯。
...
按 drugCode + routeOfAdministration + ageRange + weightRange 查询 DosageStandard，比较剂量阈值。匹配优先级策略：精确匹配（所有维度均匹配）→ 年龄范围匹配（年龄范围匹配 + 体重不限）→ 体重范围匹配（体重范围匹配 + 年龄不限）→ 无分级默认阈值。

内含单位一致性校验。校验时先通过 DosageUnitGroup 分组判断：同组内支持自动换算（如 mg↔g），跨组返回 RX_ASSIST_UNIT_MISMATCH。换算系数以常量形式固化在 DosageThresholdService 中，后续可扩展为数据库可配置。

为何使用 class 而非 interface：剂量阈值的查找与比较逻辑稳定，无多实现需求；部门内部方法较多，class 封装可隐藏内部细节。

#### DosageUnitGroup（enum）

剂量单位兼容分组定义：

| 枚举常量 | 分组单位 | 换算关系 |
|---------|---------|---------|
| MASS_GROUP | mcg, mg, g | 1mg = 1000mcg, 1g = 1000mg |
| VOLUME_GROUP | ml, L | 1L = 1000ml |
| IU_GROUP | IU | 单成员，不支持向质量/体积单位换算 |

MASS_GROUP 组内各单位的换算系数固化在 DosageThresholdService 中（mcg→mg: 1000, mg→g: 1000），换算公式为目标剂量 = 源剂量 × 源单位换算系数 ÷ 目标单位换算系数（以 mg 为中间基准）。VOLUME_GROUP 同理。IU_GROUP 当前仅 IU 单成员，无换算需要；若后续引入 ng/mL 或 mIU/mL 等复合单位，可在 IU_GROUP 内部扩展换算表或迁移至独立分组。

跨组剂量单位比对直接返回 RX_ASSIST_UNIT_MISMATCH 错误码，由 DosageThresholdService 统一处理。
...
线性降级链:
  1. AiService.triage(session context)
     ├── 有结果 → 返回推荐科室列表
     └── 无结果（空/超时/不可用） → 进入步骤 2
  2. TriageRuleEngine.match(chiefComplaint, symptoms)
     ├── 匹配到科室 → 返回规则匹配结果
     └── 无匹配（规则未命中） → 进入步骤 3
  3. DepartmentFallbackProvider.getFallbackDepartments() → 返回静态兜底科室列表

Session 超时: TRIAGE_SESSION_EXPIRED 错误
```

TriageServiceImpl 内部实现此线性降级链：先调用 AI，AI 返回无结果时调用规则引擎，规则引擎也未命中时调用兜底提供者。降级链在每一次 consult 请求中触发且仅触发一次，不进行多次重试。前端的响应体通过 fromFallback 标记区分结果来源。
...
正常路径:
  AiService.prescriptionCheck() → AuditRecord 持久化（fromFallback=false） → 返回 riskLevel + 阻断原因

降级路径:
  LocalRuleEngine.check()（DrugInteraction → AllergyCheck → DosageLimit）
  → AuditRecord 持久化（fromFallback=true，riskLevel 标记降级来源）
  → 返回 riskLevel + 阻断原因
...
无论正常路径还是降级路径，AuditRecord 均须持久化。降级路径的 AuditRecord 中 fromFallback=true，riskLevel 字段记录本地规则的判定结果。这一统一表述消除了正常/降级两条路径对 AuditRecord 持久化的歧义。
...
→ 单位一致性校验（DosageUnitGroup 分组匹配）
   ├── 同组: 自动换算（如 mg→g）
   └── 跨组: RX_ASSIST_UNIT_MISMATCH
...
| 分诊降级链 | AI → 规则引擎 → 兜底提供者（线性三级） | 避免 AI 空且规则也空时行为未定义，降级链各环节职责明确且有序 |
...
| AuditRecord 降级标记 | fromFallback 布尔字段 | 区分 AI 结果与本地规则结果，支持审计场景的精确溯源 |
| 处方审核降级 AuditRecord | 正常/降级路径均持久化 | 消除降级路径审计记录遗漏，保证审核可追溯一致性 |
...
| 剂量单位分组 | DosageUnitGroup 枚举 + MASS_GROUP/VOLUME_GROUP/IU_GROUP | 分组规则明确，同组自动换算，跨组返回错误码；IU 等特殊单位单独分组 |
| 配置变更审计 | ConfigChangeLog 实体 + 事件处理链写入 | 规则/模板配置变更可追溯，Phase 2 可迁移至数据库审计表 |
| TriageResponse 字段 | departments, sessionId, needFollowUp, followUpQuestion, confidence | 覆盖多轮分诊结果推荐、追问诱导、置信度评估等完整场景 |
| 病历字段标识 | MedicalRecordField 枚举 | 字段标识与模板 requiredFields 类型一致，消除字符串散落的二义性 |
...
DosageThresholdService 校验前通过 DosageUnitGroup 分组比对单位兼容性。

DosageUnitGroup 枚举定义：
- MASS_GROUP：mcg ↔ mg ↔ g，换算以 mg 为中间基准。1mg = 1000mcg，1g = 1000mg
- VOLUME_GROUP：ml ↔ L，换算以 ml 为中间基准。1L = 1000ml
- IU_GROUP：IU，仅单成员，不支持向质量/体积单位换算

校验流程：
1. 判断请求剂量单位与剂量标准单位的归属分组
2. 同组 → 按组内换算系数自动转换后比对
3. 跨组 → 返回 RX_ASSIST_UNIT_MISMATCH

换算系数以 private static final Map 形式固化在 DosageThresholdService 中：
```
MASS_CONVERSION: { mcg→mg: 0.001, mg→g: 0.001, mg→mcg: 1000, g→mg: 1000 }
VOLUME_CONVERSION: { ml→L: 0.001, L→ml: 1000 }
```

后续可扩展为数据库可配置的换算表，但 Phase 2/3 阶段以常量为准。
...
- DEFAULT：通用模板（CHIEF_COMPLAINT / HISTORY_OF_PRESENT_ILLNESS / PAST_MEDICAL_HISTORY / PHYSICAL_EXAMINATION / AUXILIARY_EXAMINATION / PRELIMINARY_DIAGNOSIS / TREATMENT_ADVICE）
- 最少一个示例科室模板（如 INTERNAL_MEDICINE，字段标识基于 MedicalRecordField 枚举常量）
...
模板变更的处理链：
1. admin 模块管理界面更新模板配置
2. 业务层通过 ApplicationEventPublisher 发布 TemplateConfigChangeEvent
3. 事件携带新旧值语义（oldConfig, newConfig）
4. TemplateConfigManager 监听事件并使缓存失效
5. 事件处理链中写入 ConfigChangeLog（配置变更审计日志），记录操作人、变更时间、变更前后的配置标识和版本号

### 9.3 配置变更审计

规则配置变更（如 TriageRuleEngine 规则源更新）和模板配置变更均通过统一的配置变更审计机制记录。

#### ConfigChangeLog（JPA @Entity）

| 字段 | 类型 | 说明 |
|------|------|------|
| id | Long | 主键，自增 |
| configType | String | 配置类型标识（RULE / TEMPLATE） |
| configKey | String | 配置键（如规则 ID、科室标识） |
| oldVersion | String | 变更前版本号（新增时为空） |
| newVersion | String | 变更后版本号 |
| operatorId | Long | 操作人用户 ID |
| operatorName | String | 操作人用户名 |
| changeTime | LocalDateTime | 变更时间 |
| changeSource | String | 变更来源标识（ADMIN_UI / EVENT / INIT） |

归属：ConfigChangeLog 实体定义在 common 模块（common/entity/ConfigChangeLog.java），使 prescription 和 medical-record 模块均可引用。也可定义在 admin 模块（admin/entity/ConfigChangeLog.java），由 admin 模块统一管理审计记录。本设计推荐归属 admin 模块，因为配置变更操作均由 admin 管理界面触发，审计日志的写入者也自然是 admin 模块。

#### 审计场景覆盖

| 变更类型 | 事件 | 审计内容 |
|---------|------|---------|
| 分诊规则更新 | 规则源数据变更 + Caffeine 缓存失效 | ConfigChangeLog configType=RULE，记录规则 ID 和版本 |
| 科室模板更新 | TemplateConfigChangeEvent | ConfigChangeLog configType=TEMPLATE，记录科室标识和版本号 |

Phase 2/3 阶段先以 JPA 实体方式写入数据库审计表。后续可扩展为独立审计服务或日志文件方式，但数据库写入已在当前阶段提供完整的可追溯能力。
...
ConfigChangeLog 实体归属 admin 模块（admin/entity/ConfigChangeLog.java），理由是配置变更操作均由 admin 管理界面触发，记录者也自然是 admin 模块。prescription 和 medical-record 模块在事件处理链中通过 admin 模块提供的 Service 或直接通过 ConfigChangeLogRepository（admin 模块 Repository）写入审计记录。

### 10.2 事件处理链中的审计写入

```
TemplateConfigChangeEvent / 规则变更事件
  → 监听器（位于 admin 模块）
  → 构造 ConfigChangeLog（提取事件中的新旧版本、操作人、变更时间）
  → ConfigChangeLogRepository.save()
  → 缓存失效
```

事件发布者需提供新旧值语义：变更事件应包含 oldConfig 和 newConfig 引用（或用 configKey + oldVersion + newVersion 标识），以便审计日志记录变更前后的状态。

### 10.3 审计查询

admin 模块提供审计日志查询接口，支持按 configType、configKey、changeTime 范围、operatorId 等条件过滤。Phase 2/3 阶段仅提供基础列表查询，不要求复杂审计报表。
...
| 审查意见 | 修改措施 |
|---------|---------|
| 1. AuditRecord 落库在降级路径中被遗漏，与 §3.2 行为契约矛盾 | 在 §3.2 PrescriptionAuditService 职责说明中明确"正常路径和降级路径均须写入 AuditRecord"；在 §4.2 降级路径中补充 AuditRecord 持久化步骤，与正常路径统一表述；AuditRecord 新增 fromFallback 布尔字段标记降级来源 |
| 2. 分诊降级链路不完整——AI 为空且规则也为空时的行为未定义 | §4.1 改为线性三级降级链：AI 无结果 → 规则匹配无结果 → DepartmentFallbackProvider；§3.1 TriageService 协作描述同步更新，明确三级降级链 |
| 3. §2.1 目录中存在 AgeBandedDosage.java 残留条目，与已确定的实现方案矛盾 | 从 §2.1 目录中移除 AgeBandedDosage.java 条目 |
| 4. TriageResponse DTO 字段结构未定义，无法支撑编码实现 | §3.1 补充 TriageResponse 字段说明（departments、sessionId、needFollowUp、followUpQuestion、confidence）；§2.1 dto 目录中补充字段注释；DialogueCreateRequest 说明为首轮分诊请求 DTO（chiefComplaint, patientId） |
| 5. 剂量单位转换规则集未定义，实现面临随意假设风险 | §3.4 新增 DosageUnitGroup 枚举定义；§8.3 扩展为单位兼容分组+换算系数的完整定义，含 MASS_GROUP/VOLUME_GROUP/IU_GROUP 三个分组及换算关系；§7 决策表中新增"剂量单位分组"决策条目 |
| 6. MedicalRecord 实体字段未定义，病历结构化输出模型缺失 | §3.3 新增 MedicalRecordField 枚举（CHIEF_COMPLAINT 等 7 个标准字段）；DepartmentTemplateConfig.requiredFields 类型明确为 List<MedicalRecordField>；RecordGenerateRequest/Response 补充字段定义（dialogText/departmentId/patientId，fields/missingFields）；与 §9.1 中字段列表形成呼应 |
| 7. 规则/模板配置变更缺少审计溯源能力 | 新增 §9.3 ConfigChangeLog 实体定义及字段说明；§3.3 TemplateConfigManager 职责中增加"配置变更时写入审计日志"；§9.2 补充模板变更的处理链（事件发布→审计写入→缓存失效）；新增 §10 配置变更审计与溯源章节；事件发布者提供新旧值语义 |

DESIGN_WRITTEN:C:\Develop\Software\AIMedicalSys\Harness\redeliberations\202606281422_phase23_ood\a_v4_design_v1.md
