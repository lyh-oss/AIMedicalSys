...
处方提交端点设计边界:
  ...
  行为:
    1. forceSubmit=false 时执行常规审核流程（若待提交处方无最新审核结果则先调用审核）；若已有 WARN 级别最新审核结果，不重复调用 AI 审核，直接返回当前 WARN 审核结果并提示医生选择强制提交或修改处方，避免无意义重复审核
...
五级匹配优先级（DosageThresholdService.check() 按优先级 1→2→3→4 依次尝试，命中即返回，不再继续检查更低优先级）：

1. **精确匹配**：ageRangeStart = ageRangeEnd = 患者年龄 AND weightRangeStart = weightRangeEnd = 患者体重
2. **年龄范围匹配**：ageRangeStart ≤ 患者年龄 ≤ ageRangeEnd，且 weightRange 均为 null
3. **体重范围匹配**：weightRangeStart ≤ 患者体重 ≤ weightRangeEnd，且 ageRange 均为 null
4. **无分级默认阈值**：ageRange 和 weightRange 均为 null（代表该药品/给药途径的通用剂量上限）
5. **标准不存在**：四级均未命中 → 降级返回 WARN 级 DosageAlert 并携带 errorCode=RX_ASSIST_DOSE_STANDARD_NOT_FOUND
