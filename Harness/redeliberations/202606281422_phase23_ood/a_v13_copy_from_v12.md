...
**AiResult<T> 泛型要点**：各方法的泛型 T 分别绑定为上述 4 个 ai-api 层 Response DTO；AiResult 统一包含 success（boolean）、data（T，成功时填充）、errorCode（String，可选）、degraded（boolean）、fallbackReason（String，可选）、partialData（T，可选，通过 failure()/degraded() 重载传入超时/降级部分结果）。
...
生命周期管理：创建时机（首次 check-dose 请求时——通过 private final ConcurrentHashMap 字段声明即初始化（new ConcurrentHashMap<>()），不存在空引用风险；写入操作通过 compute() 或 put() 直接写入，由 ConcurrentHashMap 内部保证首次写入时 Entry 的原子创建）、写入时机（CRITICAL 告警时，每次 check-dose 重算后覆盖该 prescriptionId 下已有的所有 CRITICAL 标记——若新结果无 CRITICAL 则清除旧标记）、清理时机（处方提交成功/处方取消/TTL 过期 60 分钟）。
