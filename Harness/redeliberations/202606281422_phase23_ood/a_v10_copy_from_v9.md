...
职责：病历生成请求值对象，对齐需求文档 3.4.3 输入契约。包含对话文本（dialogueText）、患者 ID（patientId）、就诊标识（encounterId，此字段映射为 MedicalRecord.visitId——Service 层在生成病历记录时执行 encounterId → visitId 转换，确保 encounter_id 与 visit 概念的一致性）、流式输出标记（stream，bool，可选，默认 false）。
