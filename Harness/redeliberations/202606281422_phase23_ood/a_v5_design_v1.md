...
| DialogueCreateRequest | class（DTO） | 分诊对话创建请求值对象，包含患者主诉（chiefComplaint）、患者基本信息（patientId、age、gender）、会话标识（sessionId，多轮场景复用时携带） |
...
| AuditRequest | class（DTO） | 处方审核请求值对象，包含处方药品列表、患者个体信息（含过敏史与合并症）、处方标识 |
| AuditResponse | class（DTO） | 处方审核响应值对象，包含风险等级（riskLevel，AuditRiskLevel）、审核问题列表（issues，List<AuditIssue>）、是否降级标记（fromFallback） |
...
| DosageAlertLevel | enum | 剂量告警级别枚举，定义 INFO（信息提示，不影响流程）、WARN（警告剂量，需确认）、CRITICAL（危险剂量，强制阻断）三个级别；CRITICAL 与 PrescriptionAuditService 的 BLOCK 审核联动 |
| AiSuggestionResult | class | 异步 AI 结果值对象，封装 taskId、suggestion、status（状态枚举：PENDING / COMPLETED / FAILED）、createTime、failReason（可选，FAILED 时填充） |
| PrescriptionAssistResponse | class（DTO） | 辅助开方响应值对象，包含告警列表（alerts，List<DosageAlert>）和异步任务标识（taskId） |
...
│       ├── dto/TriageResponse.java, DialogueCreateRequest.java
│       ├── service/
│       │   ├── TriageService.java
...
│       │       ├── AiSuggestionResult.java  # 含 FAILED 状态和 failReason
│       │       └── PrescriptionAssistResponse.java # 含 alerts 和 taskId
│       ├── service/
...
职责：封装分诊对话请求值对象。包含患者主诉（chiefComplaint）、患者基本信息（patientId、age、gender）、会话标识（sessionId，多轮追问场景复用已有 sessionId，通过 DialogueSessionManager 恢复上下文；首轮请求时 sessionId 为空，服务端新建会话后返回 sessionId）。
...
职责：封装处方审核请求值对象。包含处方药品列表（prescriptionItems，每项含药品标识、药品名称、剂量、频率、疗程、给药途径）、患者个体信息（patientInfo，含患者标识、年龄、性别、过敏史、合并症）、处方标识（prescriptionId）。

#### AuditResponse（class，DTO）

职责：封装处方审核响应值对象。包含风险等级（riskLevel，AuditRiskLevel 枚举）、审核问题列表（issues，List<AuditIssue>）、是否降级标记（fromFallback，boolean）。
...
职责：病历生成请求值对象。包含对话文本（dialogueText）、科室标识（departmentId）、患者 ID（patientId）、就诊标识（visitId，必填字段，用于将生成的病历关联至具体就诊记录）。
...
职责：封装单个必填字段缺失的提示信息。包含缺失字段标识（missingField，MedicalRecordField 枚举）、提示消息（promptMessage，面向医生的友好提示文案，如"主诉为必填字段，请补充患者主要症状"）、建议操作（suggestedAction，指导医生如何补全，如"请在主诉输入框中填写"或"请补充问诊记录后重新生成"）。
...
剂量阈值检查 + 异步 AI 建议查询。委托 DosageThresholdService、AiService.prescriptionAssist()（异步）。check-dose 响应返回 PrescriptionAssistResponse（含 alerts 和 taskId），前端通过 GET /api/prescription/assist/suggestion/{taskId} 查询 AI 建议。
...
- CRITICAL：危险样式（如红色阻断），强制阻断当前剂量提交，联动触发 PrescriptionAuditService 的 BLOCK 审核
...
- **CRITICAL**：危险级别，表示剂量严重超标或存在明确禁忌，前端强制阻断提交并展示危险标识；后端同步联动触发 PrescriptionAuditService 将该项标记为 BLOCK 审核结果，确保 CRITICAL 剂量告警在处方提交环节被二次强制阻断
...
**预创建→更新模式**：check-dose 响应返回 taskId 后，服务端即以 PENDING 状态预创建 AiSuggestionResult 条目并存入 ConcurrentHashMap。异步 AI 调用完成后更新为 COMPLETED 或 FAILED。CompletableFuture 的 exceptionally 处理器捕获异常时，将预创建条目更新为 FAILED 状态（填充 failReason），而非保留 PENDED 僵尸条目。

AiSuggestionResult 增加 TTL（同 suggestion TTL 30 分钟），由 ScheduledExecutorService 定期清理过期条目。

#### PrescriptionAssistResponse（class，DTO）

职责：封装辅助开方同步响应值对象。包含告警列表（alerts，List<DosageAlert>，由 DosageThresholdService 产出的剂量告警）和异步任务标识（taskId，用于前端后续查询 AI 建议）。
...
正常: AiService.prescriptionCheck() → AuditRecord 持久化（fromFallback=false） → 返回 AuditResponse（riskLevel + issues + fromFallback=false）
降级: LocalRuleEngine.check()（DrugInteraction → AllergyCheck → DosageLimit）
      ...
      → AuditRecord 持久化（fromFallback=true） → 返回 AuditResponse（riskLevel + issues + fromFallback=true）

审核结果处理:
  PASS（低风险） → 响应建议性审核意见，不影响提交
  WARN（中风险） → 响应警告信息，前端弹窗提示医生确认
                   医生三种可选操作：强制提交并留痕 / 修改后重新审核 / 撤销审核
...
{ dialogueText, departmentId, patientId, visitId }
...
{ drugCode, dosage, unit, routeOfAdministration, patientAge, patientWeight, frequency }
→ 单位一致性校验
...
→ 返回 PrescriptionAssistResponse: { alerts: [...DosageAlert], taskId }
→ 同时以 PENDING 状态预创建 AiSuggestionResult
→ 异步 AiService.prescriptionAssist() → 成功则更新为 COMPLETED，异常则更新为 FAILED
...
| 开方辅助 | RX_ASSIST_ | 剂量标准不存在（RX_ASSIST_DOSE_STANDARD_NOT_FOUND）、单位不匹配、AI 建议不存在、AI 建议生成失败 |
...
@Async / CompletableFuture.runAsync() 调用 AiService.prescriptionAssist()。主响应返回 taskId 时，服务端即以 PENDING 状态预创建 AiSuggestionResult 条目。异步调用完成后更新状态：
- 成功：更新为 COMPLETED，填充 suggestion
- 异常：CompletableFuture.exceptionally() 处理器捕获异常，更新为 FAILED，填充 failReason
...
| DosageAlert 告警级别 | DosageAlertLevel 枚举（INFO/WARN/CRITICAL） | 告警级别决定前端渲染策略和后端阻断行为，CRITICAL 联动 BLOCK 审核确保双重阻断 |
...
| MedicalRecord 就诊关联 | visitId 必填字段 | 病历可追溯至具体就诊记录，消除数据追溯缺口 |
