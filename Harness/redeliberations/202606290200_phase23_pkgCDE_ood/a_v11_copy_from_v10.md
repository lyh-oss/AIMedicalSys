...
处方提交端点设计边界:
   ...
   行为（执行顺序严格定义）:
    ...
    并发提交防护手段：RX_AUDIT_CONCURRENT_SUBMIT 错误码已在 §5.1 定义，本节补充其触发时机和具体防护机制。(a) **乐观锁防护**：AuditRecord 增加 `@Version` 乐观锁版本字段，forceSubmitted 写入时 JPA 乐观锁自动校验——若两并发线程同时读取同一条 AuditRecord 并通过版本校验，后提交线程的更新操作因版本号不匹配抛出 OptimisticLockException，Service 层捕获后转换为 RX_AUDIT_CONCURRENT_SUBMIT 错误码返回；(b) **数据库层唯一约束**：AuditRecord 实体上对 (prescriptionOrderId, forceSubmitted=true) 组合建立部分唯一索引（partial unique index，`WHERE forceSubmitted = TRUE`），确保同处方单仅能有一条强制提交记录；(c) **触发时机**：步③ forceSubmit=true 路径中，写入 forceSubmitted/forceSubmitTime 前执行乐观锁校验；两路径任意一类防护命中时返回 HTTP 409 + RX_AUDIT_CONCURRENT_SUBMIT 错误码，提示"处方正在被提交，请刷新后重试"。
...
| 审核（非AI） | RX_AUDIT_ | RX_AUDIT_PRESCRIPTION_MODIFIED（处方版本校验不一致）、RX_AUDIT_CONCURRENT_SUBMIT、RX_AUDIT_FORCE_SUBMIT_INVALID（forceSubmit=true 在无审核结果或 PASS 时无效） |
...
四处独立的定时清理任务（DialogueSessionManager 会话 TTL 30 分钟、AiSuggestionResult TTL 60 分钟、PrescriptionDraftContext TTL 60 分钟、DeadLetterCompensationService 死信补偿每 30 分钟）统一通过 Spring `@Scheduled` 注解集中调度，由 Spring 的 TaskScheduler 线程池统一管理线程资源。具体方案：
...
- 各清理逻辑封装为独立的 `@Component` Bean 方法（如 `DialogueSessionCleanupTask.cleanup()`、`SuggestionCleanupTask.cleanup()`、`DraftContextCleanupTask.cleanup()`、`DeadLetterCompensationService.compensate()`），通过 Spring 的默认 TaskScheduler（1 个线程）执行；若需自定义线程池，通过 `@Bean(TaskScheduler)` 覆盖默认配置为线程池大小 2-4
...
2. **同时范围匹配**：ageRange 与 weightRange 均非 null 但未达到精确匹配——检查 ageRangeStart ≤ 患者年龄 ≤ ageRangeEnd AND weightRangeStart ≤ 患者体重 ≤ weightRangeEnd，两项同时命中时使用该记录；若任一项未命中则继续检查下一优先级。**部分 null 边界规则**：若 ageRange 非 null 但 weightRange 部分为 null（如 weightRangeStart 非 null 但 weightRangeEnd 为 null，或相反），该记录不满足 Level 2 的"ageRange 与 weightRange 均非 null"先决条件，直接降级至 Level 3（年龄范围匹配）检查；若 ageRange 非 null 且 weightRange 完全为 null（start 和 end 均为 null），同样降级至 Level 3——此时该记录使用年龄维度匹配；若 ageRange 完全为 null 而 weightRange 部分为 null，降级至 Level 5（无分级默认阈值），因为 Level 3（年龄范围）和 Level 4（体重范围）的先决条件均不满足
