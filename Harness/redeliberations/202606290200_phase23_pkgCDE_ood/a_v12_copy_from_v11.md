...
### 1.1c 与 Phase 5 包G OOD 的兼容性分析

本设计参考 Phase 5 包G 初步 OOD（`Harness/redeliberations/202606271627_phase5_pkgG_ood/a_v22_copy_from_v21.md`），对以下三个关键兼容性约束进行分析：

**(a) AiService 接口签名可变性**：Phase 5 包G 声明 `AiService.java` 和 `AiResult.java` 均为"不变"（标注 `# 不变`）。本设计新增 `AiResult.failure(String errorCode, T partialData)` 和 `AiResult.degraded(String fallbackReason, T partialData)` 重载工厂方法与包G 的"AiResult.java 不变"假设存在潜在冲突。建议处理策略：将重载方法移至 ai-api 模块中独立的静态工厂类（如 `AiResultFactory`），保持 `AiResult.java` 的原始签名不变，通过新类型扩展而非修改现有类实现兼容。Phase 2/3 的 Converter 和 Service 实现依赖新工厂类，Phase 5 底座只需保留 `AiResult.java` 原有签名即可，双方互不冲突。AiService 接口的 4 个方法签名（triage/prescriptionCheck/generateMedicalRecord/prescriptionAssist）在包G 中同样存在且返回类型一致，业务模块代码在接口层面无迁移风险。

**(b) 底座分层架构与本设计假设的一致性**：Phase 5 包G 底座采用 `ai-impl` 内部多层管线（`AiOrchestrator` → `CapabilityExecutor` → `ModelRouter` / `LlmChatService`），`AiService` 接口作为业务模块的唯一编译期入口。本设计假设的"业务模块仅依赖 ai-api 的 AiService 接口、实现类替换对业务模块透明"与包G 底座的分层架构一致。Phase 2/3 的 `MockAiService` 在 Phase 5 替换为 `FallbackAiService` + `AiOrchestrator` 管线后，业务模块的 Service 和 Controller 代码无须修改。

**(c) Store 接口 package 路径一致性**：本设计的 Store 接口（`SessionStore`、`SuggestionStore`、`DraftContextStore`）定义在 `common-module-api` 的 `com.aimedical.modules.commonmodule.store` 包下。Phase 5 包G 未在 `common-module-api` 中定义类似的 Store 接口——包G 底座内部分布式缓存的存储接口（如 `SlidingWindowMetricsStore` 等）归属 `ai-impl` 模块内部。三项 Store 接口与本设计业务层强相关（会话存储、建议结果存储、草稿上下文存储），包G 底座不涉及同组接口定义，无 package 路径冲突风险。Phase 5 将 `ConcurrentHashMapStore` 替换为 `RedisStore` 实现时，仅需实现 `SessionStore` / `SuggestionStore` / `DraftContextStore` 接口，业务 Service 代码不感知存储变更。
...
| SubmitRequest | class（DTO） | 处方提交请求值对象，对齐处方提交端点输入契约。包含处方标识（prescriptionId，String，必填）、处方药品列表（prescriptionItems，List\<PrescriptionItem\>，每项含 drugId/drugName/dose/frequency/duration/route）、强制提交标记（forceSubmit，boolean，默认 false）、审核记录标识（auditRecordId，Long，可选——forceSubmit=true 时必填，用于定位 WARN 审核记录；forceSubmit=false 时可选；无审核结果或 PASS 场景不传） |
| SubmitResponse | class（DTO） | 处方提交响应值对象，包含提交结果状态（submitted，boolean）、处方单号（prescriptionOrderId，String，提交成功时填充）、阻断信息（blockInfo，BlockResponse，提交被阻断时填充）、错误码（errorCode，String，可选——如 RX_AUDIT_PRESCRIPTION_MODIFIED、RX_AUDIT_CONCURRENT_SUBMIT） |
...
| PrescriptionAssistService | interface | 辅助开方业务契约，覆盖两个子域：(a) AI 辅助开方完整流程——接收诊断结论/检查检验结果/患者信息/已有处方，委托 AiService.prescriptionAssist() 生成处方草案；包括剂量告警和过敏告警的本地即时校验作为 AI 调用的前置/补充，校验后将 CRITICAL 级别剂量告警同步写入 PrescriptionDraftContext（以 prescriptionId 为键全量覆盖）；(b) 剂量阈值即时校验——委托 DosageThresholdService |
...
| PrescriptionAssistRequest | class（DTO） | AI 辅助开方主端点请求值对象，对齐需求文档 3.4.10 输入契约。包含 diagnosis/examResults/patientInfo/existingPrescription/prescriptionId（可选，String——空值时由后端自动生成 UUID v4 并通过 PrescriptionAssistResponse 回写）/encounterId |
| PrescriptionAssistResponse | class（DTO） | AI 辅助开方主端点响应值对象，对齐需求文档 3.4.10 输出契约。新增 prescriptionId 可选回写字段（String，后端自动生成时填充）包含处方草案（prescriptionDraft，含 drugs 数组，每项含 drugId/drugName/dose/frequency/duration/route）、剂量告警列表（doseWarnings，List\<DoseWarning\>，每项含 drugId/warningType/message/severity）、过敏冲突告警（allergyWarnings，List\<AllergyWarningItem\>，每项含 drugId/allergen/severity）、顶层错误码（errorCode，可选，String——AI 返回无可推荐药品时填充 RX_ASSIST_AI_NO_RECOMMENDATION）、免责声明标记（disclaimerRequired） |
...
|------|---------|---------|
| DoctorFacade | interface | 跨模块医生信息查询门面，定义在 common-module-api 中，供 consultation 模块查询推荐医生列表和医生排班信息 |
| AvailableDoctor | class（DTO） | DoctorFacade.findAvailableDoctorsByDepartment() 返回值类型，定义在 common-module-api 中。包含 doctorId、doctorName、departmentId、availableSlotCount（可预约时段数）四个字段，为 TriageServiceImpl 生成推荐医生列表提供数据来源 |
| DrugFacade | interface | 跨模块药品信息查询门面，定义在 common-module-api 中（com.aimedical.modules.commonmodule.drug.DrugFacade），提供 findByDrugCode(drugCode) 等方法，供 prescription 模块查询药品名称/规格信息；与 DoctorFacade 一致的跨模块门面模式；配置独立超时阈值（默认 2s），调用超时或失败时返回空药品信息 + WARN 日志，不阻断辅助开方/处方审核主流程 |
| RegistrationEvent | class（事件） | 挂号事件契约，定义在 common-module-api 中。包含 registrationId、patientId、sessionId（可选，String——由前端在分诊结束后保留并传入挂号流程，registration 模块接收后填充至事件，供 RegistrationEventListener 通过 sessionId 精确关联 TriageRecord）、departmentId、departmentName（String，挂号科室名称——由 registration 模块在其领域内获取 departmentId 对应的科室名称后填充，与 TriageRecord.finalDepartmentName 映射）、doctorId、eventTime。事件发布端为 registration 模块，消费端为 consultation 模块，在 application 模块聚合后跨模块传播 |
...
│       ├── repository/
│   ├── TriageRecordRepository.java    # 含 findTopByPatientIdOrderByTriageTimeDesc(String patientId) 等查询方法
│   ├── TriageRuleRepository.java
│   └── DeadLetterEventRepository.java
│       ├── entity/TriageRecord.java         # 含 recordId（@Id）、AI 推荐科室快照、推荐医生快照、规则匹配科室、最终选择科室、置信度、降级标记等；按 sessionId 一对一映射，同 sessionId 重新分诊时覆盖已有记录
...
│       ├── api/
│       │   ├── PrescriptionAuditController.java  # 含 POST /api/prescription/audit + POST /api/prescription/submit
│       │   └── PrescriptionAssistController.java  # 含 POST /assist 主端点 + POST /assist/check-dose + GET /assist/suggestion/{taskId}
│       ├── dto/
...
│   ├── AiResult.java                # AI 调用结果封装（含 success/data/errorCode/degraded/fallbackReason 五字段；保留原始 failure()/degraded() 签名不变）
│   ├── AiResultFactory.java         # AiResult 重载工厂类——提供 failure(String errorCode, T partialData)、degraded(String fallbackReason, T partialData) 等重载静态方法，避免修改 AiResult.java 原始签名；供 Phase 2/3 Converter 和 Service 依赖
...
prescription 模块需获取药品名称和规格信息，通过 common-module-api 中定义的 DrugFacade 接口解耦。DrugFacade 由 drug 模块的 impl 层实现，遵循与 DoctorFacade 一致的跨模块门面模式。DrugFacade 配置独立超时阈值（默认 2s，通过 @Value 或 feign/restTemplate 超时配置注入）。调用超时或 DrugFacade 抛出异常时，调用方（PrescriptionAssistServiceImpl、PrescriptionAuditServiceImpl）捕获异常并返回空药品信息 + WARN 日志（含调用耗时、异常类型和药品编码），不阻断辅助开方/处方审核主流程。

#### 跨模块事件传递机制

RegistrationEvent 定义在 common-module-api 中（com.aimedical.modules.commonmodule.event.RegistrationEvent），包含 registrationId、patientId、sessionId（可选，String——由前端在分诊结束后保留，进入挂号界面时将 sessionId 作为请求参数传入 registration 模块，registration 模块接收后填充至事件，作为事件消费端定位 TriageRecord 的精确关联键）、departmentId、departmentName（String——由 registration 模块在其领域内获取 departmentId 对应的科室名称后填充，确保 RegistrationEventListener 可直接使用写入 TriageRecord.finalDepartmentName）、doctorId、eventTime。事件发布端为 registration 模块（患者在挂号模块完成挂号后发布事件），消费端为 consultation 模块的 RegistrationEventListener（接收事件后通过 sessionId 定位对应的 TriageRecord，补充写入 finalDepartmentId 和 finalDepartmentName）。事件在 application 模块聚合后通过 Spring ApplicationEvent 机制跨模块传播。

**sessionId 跨模块传播机制**：分诊流程结束后，前端保留 TriageResponse.sessionId。当患者进入挂号流程时，前端将 sessionId 作为挂号请求的扩展参数传入 registration 模块。registration 模块接收 sessionId 后填充至 RegistrationEvent.sessionId 字段。此机制不要求 consultation 模块与 registration 模块之间存在任何后端依赖——sessionId 通过前端侧完成跨模块传递。若前端未传递 sessionId（如分诊降级场景前端未保存），RegistrationEvent 中 sessionId 为空，RegistrationEventListener 降级为按 patientId 查询最近的分诊记录进行关联。**最近分诊记录选择标准**：按 TriageRecord.triageTime 降序排列取第一条记录作为"最近"记录，对应 Repository 查询方法 `findTopByPatientIdOrderByTriageTimeDesc(String patientId)`。triageTime 相同时按 recordId 降序选择（recordId 自增，后写入的记录 recordId 更大）。patientId 字段建立数据库索引以保障查询性能。手动选科路径（POST /api/triage/select-department）作为 RegistrationEvent 自动写入路径的补充——RegistrationEventListener 写入的 finalDepartmentId 来源于挂号模块的 departmentId，手动选科写入的 finalDepartmentId 来源于前端提交的 departmentId。覆盖优先级强制执行机制：RegistrationEventListener 在调用 selectDepartment 前先检查 TriageRecord.finalDepartmentId 是否为空，仅当为空时才写入（即事件路径仅在手动选择未发生时生效）；手动选科端点调用 selectDepartment 时始终覆盖写入。此机制确保手动选科值优先于 RegistrationEvent 写入值，因为手动选科代表患者的最新最终选择。

**RegistrationEvent 消费失败补偿策略**：RegistrationEventListener 采用 @Retryable（Spring Retry，重试间隔 2s，最多 3 次，exponential backoff）处理事件消费失败。**重试触发条件**：仅对 DataAccessException、TimeoutException 等可治愈临时异常触发重试；对 IllegalArgumentException、NullPointerException 等不可治愈异常（不可重试类型）直接进入 @Recover 回调，不浪费重试资源。不可治愈异常列表通过 @Retryable 的 `excludes` 或 `noRetryFor` 属性配置。若重试仍失败，异常由 @Recover 方法（RetryOperationsInterceptor 的 recover 回调）接收，写入 dead_letter_event 表（含 id、eventPayload、failReason、failTime、state、retryCount、maxRetryCount），由定时补偿任务（每 30 分钟扫描重试未成功的死信事件）兜底。
...
定时补偿任务封装为 DeadLetterCompensationService（com.aimedical.modules.consultation.service.DeadLetterCompensationService），由统一 Spring @Scheduled 每 30 分钟调度一次（详见 §6.1 定时任务集中管理），扫描 state=FAILED 且 retryCount < maxRetryCount 的死信事件，从事件载荷（eventPayload，JSON 序列化的 RegistrationEvent）中反序列化获取 departmentId 和 departmentName，直接调用 TriageService.selectDepartment(sessionId, departmentId, departmentName) 完成补偿，不重新发布 ApplicationEvent（避免与 @Retryable 形成无限循环）。补偿完成后更新 state=COMPENSATED 或递增 retryCount。状态迁移规则：FAILED（初始）→（补偿成功）→COMPENSATED；FAILED →（重试耗尽/超出最大重试次数）→EXPIRED。补偿任务在 consultation 模块内部完成，注入 TriageService 接口完成调用，不依赖外部模块。
...
本设计涉及的领域事件如下表，涵盖事件名称、定义位置、发布/消费模块、触发条件、事务边界及 Phase 范围：

| 事件名称 | 定义位置（包路径） | 发布模块 | 消费模块 | 触发条件 | 事务边界 | Phase 范围 |
|---------|------------------|---------|---------|---------|---------|-----------|
| RegistrationEvent | common-module-api/event/ | registration | consultation | 患者在挂号模块完成挂号后发布 | 发布端事务提交后（@TransactionalEventListener AFTER_COMMIT）；消费端独立事务 | Phase 2/3 |
| TemplateConfigChangeEvent | medical-record/event/ | admin | medical-record（TemplateConfigManager） | 管理员更新科室模板配置后 | admin 模块事务提交后发布；消费端刷新缓存（非事务） | Phase 2/3 |
| TriageRuleChangeEvent | consultation/event/ | admin | consultation（DefaultTriageRuleEngine） | 管理员更新/发布/回滚分诊规则后 | admin 模块事务提交后发布；消费端刷新缓存 | Phase 2/3 |
| DrugContraindicationChangeEvent | prescription/event/ | admin | prescription（ContraindicationCheckRule） | 管理员更新药品禁忌症映射后 | admin 模块事务提交后发布；消费端刷新缓存 | Phase 2/3 |
| DrugAllergyMappingChangeEvent | prescription/event/ | admin | prescription（AllergyCheckRule） | 管理员更新药物过敏映射后 | admin 模块事务提交后发布；消费端刷新缓存 | Phase 2/3 |
| DrugCompositionDictChangeEvent | prescription/event/ | admin | prescription（DuplicateCheckRule） | 管理员更新药品成分字典后 | admin 模块事务提交后发布；消费端刷新缓存 | Phase 2/3 |
| DrugInteractionPairChangeEvent | prescription/event/ | admin | prescription（DrugInteractionRule，Phase 4） | 管理员更新药物相互作用数据后 | admin 模块事务提交后发布 | Phase 4 预留 |
| DosageStandardChangeEvent | common/event/ | admin | prescription（DosageThresholdService，Phase 4 引入缓存时） | 管理员更新剂量标准后 | admin 模块事务提交后发布；消费端刷新缓存 | Phase 4 预留（当前无运行时缓存） |

**事件驱动缓存刷新与定时刷新的双重失效策略**：所有可缓存规则实体（TriageRule、DrugContraindicationMapping、DrugAllergyMapping、DrugCompositionDict、科室模板配置）均采用"事件驱动立即失效 + 定时刷新兜底"策略——事件通知丢失时，Caffeine refreshAfterWrite（默认间隔 60 秒）自动从数据库加载最新数据。

### 2.3 AiService 接口定义

AiService 接口定义在 ai-api 模块（com.aimedical.modules.ai.api.AiService），是各业务模块调用 AI 能力的统一入口。AiResult 类定义在同包（com.aimedical.modules.ai.api.AiResult），封装 AI 调用结果（含 success/data/errorCode/degraded/fallbackReason 五字段），保留原始 failure()/degraded() 签名不变。超时/降级重载工厂方法统一迁移至 AiResultFactory（同包，com.aimedical.modules.ai.api.AiResultFactory），提供以下静态工厂方法：
- `public static <T> AiResult<T> failure(String errorCode, T partialData)` — 超时失败路径，errorCode 如 "MOCK_AI_TIMEOUT"/"MR_GEN_AI_TIMEOUT"，partialData 写入 data 字段
- `public static <T> AiResult<T> degraded(String fallbackReason, T partialData)` — 降级路径，fallbackReason 如 "AI 服务不可用"，partialData 写入 data 字段
...
各方法使用泛型 `CompletableFuture<AiResult<T>>` 封装异步 AI 调用结果，具体方法签名及泛型绑定如下：
...
- 输入校验：AiService 实现层负责校验 TriageRequest 字段基本合法性（如 chiefComplaint 非空、sessionId 格式为 UUID v4），校验不通过时返回 AiResult.failure(TRIAGE_AI_INPUT_INVALID)
...
- 输入校验：AiService 实现层负责校验 PrescriptionCheckRequest 字段基本合法性（如 prescriptionItems 非空且至少一项、prescriptionId 非空），校验不通过时返回 AiResult.failure(RX_AUDIT_AI_INPUT_INVALID)
...
- 输入校验：AiService 实现层负责校验 MedicalRecordGenRequest 字段基本合法性（如 dialogueText 非空且长度 ≥ 50、patientId 非空），校验不通过时返回 AiResult.failure(MR_GEN_AI_INPUT_INVALID)
...
- 输入校验：AiService 实现层负责校验 PrescriptionAssistRequest 字段基本合法性（如 diagnosis 非空），校验不通过时返回 AiResult.failure(RX_ASSIST_AI_INPUT_INVALID)
...
**并发安全**：策略存储使用 `volatile` 关键字声明——`private volatile ResponseStrategy currentStrategy = ResponseStrategy.STATIC;`，确保单线程写入策略值后所有读取线程立即可见（happens-before 语义）。策略切换不影响正在执行的 AI 调用——正在执行的调用使用调用开始时的策略快照值（每个 AiService 方法在入口处将 currentStrategy 读入局部变量，后续逻辑使用该局部变量而非每次重新读取 currentStrategy），切换后的新策略仅在新调用入口处生效。此设计避免策略切换时对正在执行的线程造成不可预期的返回结果。若未来需要更细粒度的并发控制（如读写锁），可将策略存储升级为 `AtomicReference<ResponseStrategy>`。
...
方法签名：
- `TriageResponse triage(DialogueCreateRequest request)`：智能分诊入口，覆盖单轮/多轮场景，内部委托 AiService.triage() 进行 AI 分诊，AI 不可用时降级至 TriageRuleEngine 规则匹配 → DepartmentFallbackProvider 兜底；返回 TriageResponse 含推荐科室列表、推荐医生列表、追问标记、降级标记等
- `TriageResponse selectDepartment(String sessionId, String departmentId, String departmentName)`：手动选科写入，按 sessionId 查找 TriageRecord 并补充写入 finalDepartmentId 和 finalDepartmentName；记录不存在时返回 TRIAGE_SESSION_NOT_FOUND
...
**重新分诊生命周期行为**：TriageRecord 按 sessionId 一对一映射——sessionId 字段在 TriageRecord 实体上标注 `@Column(unique = true)` 确保数据库级唯一约束，JPA schema 自动推导生成唯一索引。同一 sessionId 第二次发起分诊时，TriageRecordRepository 通过 update 覆盖同 sessionId 记录（update 模式为推荐实现），或通过 delete+insert 模式（delete 后 insert）。前端负责在患者发起新会话时生成新的 sessionId，后端不假设 session 复用场景下的语义自动合并。此设计确保分诊记录的 sessionId 域内唯一，每次分诊对应一条最新记录。update 模式通过 `@Column(unique = true)` 唯一约束在 SessionStore 键值替换时结合 JPA merge() 或自定义 `@Modifying @Query` 实现，避免约束冲突。

**DialogueSession 与 TriageRecord 的事务一致性策略**：DialogueSession（内存 ConcurrentHashMap）的状态更新与 TriageRecord 数据库写入存在一致性问题——若 DialogueSession 先更新后 TriageRecord 数据库写入失败，用户重试时 DialogueSession 处于不一致状态。采用"先写数据库再更新内存"策略——所有分诊流程中，TriageRecord 的数据库写入操作先于 DialogueSession 的内存更新执行。具体而言：TriageServiceImpl 在 AiService 调用/降级完成后，先通过 TriageRecordRepository.save() 写入 TriageRecord（在 @Transactional 事务内），事务提交成功后，再更新 DialogueSession（aiFailCount 递增、QA 历史追加、correctedChiefComplaint 写入、对话轮次计数器递增等）。若数据库写入失败（抛异常），DialogueSession 保持之前未更新的状态，用户下次重试时一切从头执行。此策略接受如下数据丢失窗口：事务提交成功到内存更新之间的进程崩溃会导致 TriageRecord 已持久化但 DialogueSession 未同步更新。**受影响的可变状态范围及影响评估**：
| DialogueSession 可变状态 | 丢失影响 | 可接受性 |
|-------------------------|---------|---------|
| aiFailCount（AI 连续失败计数） | 丢失后重置为 0，连续失败兜底机制从当前会话重新计数 | 可接受（仅影响同一会话内兜底触发时机，最多延迟一次兜底判决） |
| QA 历史列表（additionalResponses 累积） | 丢失后下次请求时 TriageServiceImpl 因无法拼合完整上下文，向 AI 发送不完整的对话历史，可能导致推理结果偏离 | **有条件可接受**——多轮对话的 QA 历史通过前端保留完整 UI 展示；后端丢失后用户下一轮请求时，前端重新发送全部历史（DialogueCreateRequest.additionalResponses 含 full context）。受影响的仅为进程崩溃前尚未持久化的最近一轮 QA——该轮用户的回答尚未提交至 TriageRecord 即崩溃，用户需重新回答 |
| correctedChiefComplaint | 丢失后下次请求时主诉修正信息丢失，AI 推理使用原始 chiefComplaint | **可接受**——用户若确认主诉修正，可在后续轮次通过 correctedChiefComplaint 字段再次提交修正值 |
| 对话轮次计数器（roundCount） | 丢失后从 0 重新计数，上下文截断策略根据 roundCount 触发 | **有条件可接受**——丢失后截断策略以更少的轮次触发截断，仅影响极少数长对话场景（通常 2-5 轮），截断阈值宽松。影响仅限于提前触发截断，不影响已生成的推荐结果 |

**补充保障措施**：
- (a) **TriageRecord 存储关键上下文快照**：TriageRecord 实体增加 chiefComplaint 原始主诉快照字段（已有）和 correctedChiefComplaint 快照字段（新增，String 可选），确保修正主诉在数据库层有持久化副本。进程重启后若 session 超时重建，TriageServiceImpl 可从最近 TriageRecord 中恢复 correctedChiefComplaint 写入新 DialogueSession。补充措施：新增 `findTopBySessionIdOrderByTriageTimeDesc(String sessionId)` 查询方法用于恢复
- (b) **写入顺序调整不影响不可接受的丢失**：QA 历史列表的丢失通过"先写数据库再更新内存"策略保护——QA 追加操作在事务提交成功后执行，若崩溃发生在事务提交后、内存更新前，QA 已持久化至 TriageRecord（JSON TEXT 快照字段），下次请求时可通过快照恢复，不涉及丢失。若崩溃发生在事务提交前（用户提交 QA 后服务器崩溃），该轮 QA 未持久化也未更新内存，用户需重新提交，属于正常业务失败而非数据丢失
...
协作：被 TriageService 调用。内部依赖 SessionStore 接口（com.aimedical.modules.commonmodule.store.SessionStore）进行会话存储，Phase 2/3 由 ConcurrentHashMapStore 实现（com.aimedical.modules.commonmodule.store.impl.ConcurrentHashMapStore），Phase 5 替换为 RedisStore；会话 TTL 30 分钟，由统一 Spring @Scheduled 任务每 5 分钟扫描清理过期会话（详见 §6.1 定时任务集中管理）。
...
方法签名：
- `AuditResponse audit(AuditRequest request)`：处方审核入口，接收待审核处方和患者信息，委托 AiService.prescriptionCheck() 进行 AI 审核，AI 不可用时降级至 LocalRuleEngine 本地规则校验；返回含风险等级、风险提示列表、用药建议等的 AuditResponse
...
DrugInteractionRule（DDI）不在 Phase 2/3 本地规则范围内，DrugInteractionPair 数据实体在目录中预留骨架但不参与 Phase 2/3 运行时校验。DrugInteractionPair 实体标注为"Phase 4 预留，当前版本不建表"——在 @Entity 上增加 `@Table(schema = "PHASE4_PRELOAD")` 条件建表控制，或通过 Flyway/Liquibase 脚本按 Phase 版本控制建表，避免 Phase 2/3 产生空数据库表。
...
#### SubmitRequest（class，DTO）

职责：处方提交请求值对象，对齐处方提交端点 POST /api/prescription/submit 输入契约。包含以下字段：
- prescriptionId（String，必填——系统层 UUID 标识，贯穿草稿→审核→提交全流程）
- prescriptionItems（List\<PrescriptionItem\>，每项含 drugId/drugName/dose/frequency/duration/route，提交处方完整药品列表）
- forceSubmit（boolean，默认 false——是否强制提交；forceSubmit=true 仅当最新审核结果为 WARN 时有效，其他场景返回 RX_AUDIT_FORCE_SUBMIT_INVALID）
- auditRecordId（Long，可选——forceSubmit=true 时必填，标识医生确认的 WARN 级审核记录；forceSubmit=false 时可选，为空时后端自动查询最新审核记录；无审核结果或 PASS 场景不传）

**auditRecordId 必填/可选语义边界**：
| forceSubmit | 最新审核结果 | auditRecordId | 行为 |
|------------|-------------|--------------|------|
| false | 无审核结果 | 不传 | 先执行审核后提交 |
| false | PASS | 不传 | 直接常规提交 |
| false | WARN | 可选（为空时自动定位最新 WARN 记录） | 内容变更检测 → 提示医生选择 |
| false | BLOCK | 不传 | 步②阻断，返回 422 |
| true | WARN | 必填 | 校验 auditRecordId 对应 isLatest=true → 处方版本校验 → 强制提交 |
| true | 非 WARN | 无效 | 返回 RX_AUDIT_FORCE_SUBMIT_INVALID |

#### SubmitResponse（class，DTO）

职责：处方提交响应值对象，包含以下字段：
- submitted（boolean——true 表示提交成功，false 表示被阻断或校验失败）
- prescriptionOrderId（String，可选——提交成功时填充业务单据号；BLOCK 阻断或校验失败时不填充）
- blockInfo（BlockResponse，可选——步①/步②阻断时填充，含阻断原因列表、阻断码、阻断时间）
- errorCode（String，可选——步③校验失败时填充，如 RX_AUDIT_PRESCRIPTION_MODIFIED、RX_AUDIT_CONCURRENT_SUBMIT、RX_AUDIT_FORCE_SUBMIT_INVALID）
...
| TREATMENT_PLAN | treatment_plan | 治疗意见 |

#### MedicalRecordService（interface）

方法签名：
- `RecordGenerateResponse generate(RecordGenerateRequest request)`：病历生成入口，接收对话文本和患者/就诊信息，通过 VisitFacade.findVisitIdByEncounterId() 获取 visitId，委托 AiService.generateMedicalRecord() 进行 AI 结构化病历生成，对 AI 输出由 TemplateConfigManager 获取科室模板、MissingFieldDetector 检测缺失字段并生成提示；返回含结构化病历 fields、缺失字段提示列表、降级标记等的 RecordGenerateResponse
...
职责：病历生成请求值对象，对齐需求文档 3.4.3 输入契约。包含对话文本（dialogueText，必填，字符数 50–10000）、患者 ID（patientId）、就诊标识（encounterId，此字段映射为 MedicalRecord.visitId——Service 层在生成病历记录时通过 VisitFacade.findVisitIdByEncounterId(encounterId) 执行 encounterId → visitId 转换。VisitFacade 定义在 common-module-api 中（com.aimedical.modules.commonmodule.visit.VisitFacade），由就诊模块实现，返回 visitId（String）。MedicalRecordService 依赖 VisitFacade 接口而非实现，编译期不依赖就诊模块。**VisitFacade 调用降级策略**：VisitFacade 配置独立超时阈值（默认 `medical-record.visit-facade.timeout=2s`，通过 @Value 或 RestTemplate 超时配置注入）。调用超时或失败时，MedicalRecordService 依次尝试降级：(a) 将 encounterId 直接作为 visitId 的 fallback 写入（当 encounterId 非空时——直接使用 encounterId 字符串值作为 visitId，不校验额外格式约束，由 visit 模块在后续关联查询时验证有效性）；fallback 写入时在 MedicalRecord 增加 visitIdFallback=true 标记字段（Boolean，nullable，降级写入时设为 true），供 visit 模块后续 reconciled 任务识别并修复——修复完成后清除 visitIdFallback 标记并更新为正确的 visitId；(b) encounterId 为空或降级写入失败时，返回 MR_GEN_VISIT_NOT_FOUND 错误码并携带已生成的部分病历内容。降级场景下记录 WARN 级别日志（含调用耗时、异常类型）。此降级策略确保 VisitFacade 不可用时病历内容的已生成部分不丢失）、流式输出标记（stream，bool，可选，默认 false）。
...
方法签名：
- `PrescriptionAssistResponse assist(PrescriptionAssistRequest request)`：AI 辅助开方主入口，接收诊断结论、检查检验结果、患者信息、已有处方，委托 AiService.prescriptionAssist() 生成处方草案；对 AI 产出执行本地即时校验（DosageThresholdService 剂量校验 + AllergyCheckRule 过敏冲突检查），将 CRITICAL 级别告警写入 PrescriptionDraftContext；返回含处方草案、剂量告警、过敏冲突告警等的 PrescriptionAssistResponse
- `DosageCheckResponse checkDose(DosageCheckRequest request)`：剂量阈值即时校验子入口，接收单药品剂量检查参数，委托 DosageThresholdService 做即时阈值校验；同步将 CRITICAL 级别告警全量覆盖写入 PrescriptionDraftContext；触发异步 AI 建议生成（通过 DedupTaskScheduler 去重后创建 AiSuggestionResult）；返回即时剂量告警列表及 taskId

职责：**AI 辅助开方完整流程**——接收诊断结论、检查检验结果、患者信息、已有处方，委托 AiService.prescriptionAssist() 生成处方草案；本地即时校验（剂量阈值 DosageThresholdService + 过敏冲突本地检查）作为 AI 调用的前置补充，产出 doseWarnings 和 allergyWarnings；校验后将 alertLevel=CRITICAL 的剂量告警同步写入 PrescriptionDraftContext（以 prescriptionId 为键全量覆盖），写入逻辑与 check-dose 路径一致——覆盖当前 prescriptionId 下所有 CRITICAL 标记，无 CRITICAL 时清除对应条目。prescriptionId 的兜底规则：若 /assist 请求未携带 prescriptionId，后端自动生成 UUID v4 作为处方标识，通过 PrescriptionAssistResponse 新增 prescriptionId 字段回写前端。**过敏冲突本地检查实现路径**：PrescriptionAssistServiceImpl 直接复用同模块的 AllergyCheckRule 规则类（PrescriptionAssistServiceImpl 与 AllergyCheckRule 同属 prescription 模块，无跨模块依赖），对 AI 处方草案中每项药品执行过敏冲突匹配。复用逻辑包括结构化过敏信息精确匹配（allergyDetails 优先）和文本过敏史回退匹配。此复用确保辅助开方阶段与处方审核阶段共享同一套过敏匹配逻辑，避免逻辑分叉。**剂量阈值即时校验子域**——接收单药品剂量检查参数，委托 DosageThresholdService 做即时阈值校验。
...
**异步 AI 调用去重策略**：同一 prescriptionId 在 check-dose 高频调用下可能产生大量并发异步 AI 调用。定义 prescriptionId 级去重规则——(a) 创建新异步 AI task 前查询该 prescriptionId 下最近 AiSuggestionResult：若存在 PENDING 状态 task 则复用已有 taskId（不创建新 AI 调用）；若存在 COMPLETED 且未被前端消费的 task 则复用已有 taskId 与结果（不创建新 AI 调用）；仅当前次 task 为 FAILED 或 COMPLETED 已被消费时创建新 task。(b) **已消费判定**：GET /api/prescription/assist/suggestion/{taskId} 查询端点在返回 COMPLETED 结果时自动将 consumed 置为 true（PENDING/FAILED/NOT_FOUND 路径不设置）。前端崩溃后重新发起 check-dose 将创建新 task，属于可接受的边界行为。(c) 前端配合 300ms 防抖合并连续剂量调整请求。去重检查为同步前置操作（在主响应 DosageCheckResponse 返回前执行），不影响即时剂量校验的实时性。去重实现建议封装为 DedupTaskScheduler 辅助类（com.aimedical.modules.prescription.service.assist.DedupTaskScheduler）。

**去重原子性保障**：为消除"查询 AiSuggestionResult → 判断状态 → 按条件创建新 task"三步操作之间的 TOCTOU 竞态条件，在 SuggestionStore 接口新增原子性方法 `SuggestionStore.createIfNotExists(taskId, prescriptionId, supplier)`——该方法将去重判定（检查该 prescriptionId 下是否存在 PENDING/COMPLETED 未消费的 AiSuggestionResult）和条件创建合并为存储层原子操作。

`createIfNotExists` 精确契约：
- **参数**：`taskId`（String，新 task 的 UUID，去重判定未命中时创建），`prescriptionId`（String，作为存储键，去重判定的分组维度——同一 prescriptionId 下最多一个活跃 task），`supplier`（`Supplier<AiSuggestionResult>`，去重未命中时调用 supplier.get() 创建新实例，去重命中时 supplier 不被调用——惰性求值避免副作用）
- **返回值**：`AiSuggestionResult`——若去重命中（存在 PENDING 或 COMPLETED 未消费的 task）则返回已有的 AiSuggestionResult 实例；若去重未命中则调用 supplier.get() 创建新实例并存储，同样返回新实例
- **原子性**：检查已有 task 状态 + 条件创建新 task 在单个存储层原子操作内完成，无 TOCTOU 窗口

Phase 2/3 的 ConcurrentHashMapStore 实现通过 `ConcurrentHashMap.compute(prescriptionId, (key, existing) -> { ... })` 在锁保护的函数内完成：函数体内查询 existing（即当前 prescriptionId 对应的 AiSuggestionResult），若其为 PENDING 或 COMPLETED+!consumed 则返回 existing（去重命中），否则调用 supplier.get() 创建新实例后返回。Phase 5 的 RedisStore 实现通过 Lua 脚本 `redis.call('EXISTS', key) and redis.call('GET', key) or redis.call('SET', key, supplierValue)` 等原子命令完成。DedupTaskScheduler 不再自身维护去重逻辑，仅调用 SuggestionStore.createIfNotExists() 并处理返回结果。
...
AiSuggestionResult 增加 TTL（60 分钟），由统一 Spring @Scheduled 任务每 5 分钟扫描清理 TTL 超过 60 分钟的过期条目（详见 §6.1 定时任务集中管理）。

**TTL 过期与 consumed 标记的协调策略**：AiSuggestionResult 有两个释放条件——TTL 过期（60 分钟自动清理）和已消费（consumed=true）。二者协调规则如下：
- **已消费优先**：查询端点在返回 COMPLETED 结果时自动将 consumed 置为 true，此时即使 TTL 未过期，去重逻辑仍视其为"已消费"触发新 task 创建。
...
- 格式：JSON TEXT，结构为 PrescriptionAssistResponse 的 prescriptionDraft.drugs 字段的完整药品条目列表——partialData 语义为"已成功生成的完整药品条目列表"（部分药品而非部分字段）。每项含 drugId/drugName/dose/frequency/duration/route 完整六字段（不存在字段级缺失，只有药品级缺失——AI 要么完整生成某药品的所有字段，要么完全不生成该药品）。缺失字段不在 JSON 中出现。示例：`{"prescriptionDraft": {"drugs": [{"drugId": "DRG001", "drugName": "头孢克肟", "dose": 100, "frequency": "bid", "duration": 5, "route": "po"}]}}`。
...
生命周期管理：创建时机（首次 check-dose 或 /assist 请求时——DraftContextStore 通过 ConcurrentHashMapStore 的 computeIfAbsent() 惰性创建 Entry，不存在空引用风险；写入操作通过 Store 接口的 put() 方法委托给后端存储的原子写入）、写入时机（CRITICAL 告警时，每次 check-dose 或 /assist 重算后覆盖该 prescriptionId 下已有的所有 CRITICAL 标记——若新结果无 CRITICAL 则清除旧标记）、清理时机（处方提交成功/处方取消/TTL 过期 60 分钟）。**TTL 过期清理由统一 Spring @Scheduled 任务每 5 分钟扫描清理（遍历 DraftContextStore 清理最后更新时间超过 60 分钟的条目，详见 §6.1 定时任务集中管理）**，确保异常退出场景下 CRITICAL 标记不会无限残留。
  ...
  WARN（中风险） → 响应警告信息，前端弹窗提示医生确认
                    ...
                    1. 强制提交并留痕 → 前端调用处方提交端点携带 forceSubmit=true + auditRecordId
                       ...
                       → 校验处方版本：比较当前处方与 AuditRecord.originalPrescription 是否一致（防篡改）
                         一致性语义定义：按业务字段做结构化比较（drugId + dose + frequency + duration + route 五字段组合比对），忽略 JSON 文本级的格式差异（字段顺序、null 与缺失字段、数值精度等）。药品增删、剂量变化等业务实质变更判为"不一致"；仅 JSON 序列化格式差异判为"一致"。**边界规则**：(a) 双方均为 null 或空列表视为一致，一方为空另一方非空视为不一致；(b) null 与 0/空字符串按业务语义等价处理（建议统一要求非空校验在前，结构化比较阶段不应出现 null 字段）；(c) 字段顺序差异不视为不一致
...
处方提交端点设计边界:
    POST /api/prescription/submit（Phase 2/3 正式 DTO 定义见 §3.2 SubmitRequest/SubmitResponse）
    请求 DTO：SubmitRequest（prescriptionId/prescriptionItems/forceSubmit/auditRecordId）
    响应 DTO：SubmitResponse（submitted/prescriptionOrderId/blockInfo/errorCode）
    行为（执行顺序严格定义）:
      步① CRITICAL 阻断检查（最高优先级）：检查 PrescriptionDraftContext 中该 prescriptionId 是否存在 CRITICAL 级别剂量告警。若存在，立即返回 HTTP 422 + SubmitResponse(submitted=false, blockInfo=BlockResponse)，不执行后续任何步骤。
      步② 审核结果阻断检查：若步①通过（无 CRITICAL 阻断），检查该处方的最新 AuditRecord 是否为 BLOCK 级别。若为 BLOCK，返回 HTTP 422 + SubmitResponse(submitted=false, blockInfo=BlockResponse)，跳过步③。若最新审核结果为 WARN 或更低，进入步③。
      步③ forceSubmit 判定（仅在步①②均通过时生效）：
        - forceSubmit=false 且无最新审核结果：先调用 PrescriptionAuditService.audit() 执行审核（同步），根据审核结果走对应路径——BLOCK 返回阻断，PASS 直接常规提交，WARN 回到 forceSubmit=false+WARN 路径
        - forceSubmit=false 且最新审核结果为 PASS：直接执行常规提交流程——校验处方完整性后由处方落单模块完成提交，返回 SubmitResponse(submitted=true, prescriptionOrderId)
        - forceSubmit=false 且最新审核结果为 WARN：对当前处方做轻量级内容变更检测（drugId+dose+frequency+duration+route 五字段结构化哈希比对，与 AuditRecord.originalPrescription 的哈希比较），若检测到内容变更则返回 SubmitResponse(submitted=false, errorCode=RX_AUDIT_PRESCRIPTION_MODIFIED) 并提示"处方内容已变更，请重新审核"，防止返回基于旧处方的已过期审核结果；若哈希一致则直接返回当前 WARN 审核结果并提示医生选择强制提交或修改处方，避免无意义重复审核
        - forceSubmit=true：校验 auditRecordId 对应 AuditRecord.riskLevel=WARN 且 isLatest=true
          + 处方版本校验（当前处方与 originalPrescription 一致）
          → 校验通过：写入 forceSubmitted/forceSubmitTime → 处方落单 → 返回 SubmitResponse(submitted=true, prescriptionOrderId)
          → 校验失败：返回 SubmitResponse(submitted=false, errorCode=RX_AUDIT_PRESCRIPTION_MODIFIED)
        - 最新审核结果为 PASS 或无审核结果：直接执行常规提交流程——跳过审核结果校验，处方完整性校验后落单；forceSubmit=true 在此场景下无效（返回 SubmitResponse(submitted=false, errorCode=RX_AUDIT_FORCE_SUBMIT_INVALID)）
    ...
    并发提交防护手段：RX_AUDIT_CONCURRENT_SUBMIT 错误码已在 §5.1 定义，本节补充其触发时机和具体防护机制。(a) **乐观锁防护**：AuditRecord 增加 `@Version` 乐观锁版本字段，forceSubmitted 写入时 JPA 乐观锁自动校验——若两并发线程同时读取同一条 AuditRecord 并通过版本校验，后提交线程的更新操作因版本号不匹配抛出 OptimisticLockException，Service 层捕获后转换为 RX_AUDIT_CONCURRENT_SUBMIT 错误码返回；(b) **数据库层唯一约束**：AuditRecord 实体上对 (prescriptionOrderId, forceSubmitted=true) 组合建立部分唯一索引（partial unique index，`WHERE forceSubmitted = TRUE`），确保同处方单仅能有一条强制提交记录；(c) **触发时机**：步③ forceSubmit=true 路径中，写入 forceSubmitted/forceSubmitTime 前执行乐观锁校验；两路径任意一类防护命中时返回 HTTP 409 + SubmitResponse(submitted=false, errorCode=RX_AUDIT_CONCURRENT_SUBMIT)，提示"处方正在被提交，请刷新后重试"。

    阻断竞态防护手段：设计 SubmitContext 值对象（线程级闭包，在提交端点方法调用栈内传递，不跨线程共享），用于承载步①执行时的 CRITICAL 告警快照。SubmitContext 的定义如下：
    - **作用域**：提交端点方法调用栈内的线程局部值对象，方法返回后即销毁。
    - **数据结构**：含 snapshotCriticalAlerts（List\<DosageAlert\>，步①时从 PrescriptionDraftContext 读取的 CRITICAL 告警列表快照）和 submitStartTime（LocalDateTime，步①执行时间戳）两个字段。
    - **存储位置**：在提交端点 Controller 方法中创建 SubmitContext 实例，作为参数逐层传入 Service 方法栈，不涉及持久化。

    执行步骤增强：
    - 步① CRITICAL 阻断检查时，从 PrescriptionDraftContext 读取当前 CRITICAL 告警列表，存入 SubmitContext.snapshotCriticalAlerts。
    - 步② 审核结果阻断检查完成后（步③前），执行二次 CRITICAL 验证：重新查询 PrescriptionDraftContext 中该 prescriptionId 的 CRITICAL 告警列表，与 SubmitContext.snapshotCriticalAlerts 做集合比对。若存在新增 CRITICAL 告警（实时列表相对快照列表有增量），则立即中止提交流程并返回阻断响应。
    - 二次验证在 forceSubmit=false 路径下同样生效——哈希比对前先做二次 CRITICAL 检查，确保提交时刻的处方安全状态与步①检查时一致。
...
请求: { diagnosis, examResults, patientInfo, existingPrescription, encounterId, prescriptionId（可选，空值时后端自动生成） }
→ prescriptionId 兜底处理：若请求中未携带 prescriptionId，后端自动生成 UUID v4，将该 ID 贯穿当前请求的全量校验和写入路径，通过 PrescriptionAssistResponse 新增 prescriptionId 字段回写前端
...
→ CRITICAL 告警同步写入 PrescriptionDraftContext（以 prescriptionId 为键全量覆盖，写入逻辑与 check-dose 路径一致——覆盖当前 prescriptionId 下所有 CRITICAL 标记，无 CRITICAL 时清除对应条目）
→ 返回 PrescriptionAssistResponse: { prescriptionDraft, doseWarnings, allergyWarnings, disclaimerRequired, prescriptionId }
...
→ AiSuggestionResult TTL 60 分钟，由 ScheduledExecutorService 每 5 分钟扫描清理 TTL 超过 60 分钟的条目
...
→ COMPLETED: 返回完整 suggestion 内容，后端自动将 consumed 置为 true
...
{
  "submitted": true,
  "prescriptionOrderId": "RX202606280001",
  "blockInfo": null,
  "errorCode": null
}
```
...
请求 JSON 示例（forceSubmit=false + WARN 审核结果——内容未变更）：
```json
{
  "prescriptionId": "RX-001-20260628",
  "prescriptionItems": [
...
响应 JSON 示例（forceSubmit=false + WARN——返回当前审核结果，提示医生选择）：
```json
{
  "submitted": false,
  ...
  "warnResult": {
    "riskLevel": "WARN",
    "alerts": [
      {"alertCode": "ALLERGY_001", "alertMessage": "患者对阿莫西林有过敏史", "severity": "WARNING"}
    ],
    "auditRecordId": 1001,
    "prescriptionHash": "a1b2c3d4e5"
  }
}
```

响应 JSON 示例（步① CRITICAL 阻断——HTTP 422）：
```json
{
  "submitted": false,
  "prescriptionOrderId": null,
...
**处方提交端点的错误格式边界**（补充 §4.2 步①/步②/步③）：
- 步①（CRITICAL 剂量阻断）和步②（审核 BLOCK 阻断）使用 BlockResponse（HTTP 422）格式——对应 `SubmitResponse.blockInfo` 字段承载阻断详情，阻断码（blockCode）分别设置为 `RX_BLOCK_CRITICAL_DOSE`（步①）和 `RX_BLOCK_AUDIT_BLOCK`（步②）
- 步③ forceSubmit 校验失败（RX_AUDIT_PRESCRIPTION_MODIFIED、RX_AUDIT_CONCURRENT_SUBMIT、RX_AUDIT_FORCE_SUBMIT_INVALID）使用统一错误格式（HTTP 409），对应 `SubmitResponse.errorCode` 字段承载错误码，`SubmitResponse.submitted=false`；不经过 BlockResponse 路径
...
**错误码 → HTTP 状态码映射**：本设计的错误码统一通过 GlobalExceptionHandler 管理，遵循以下集中映射规则：

| 错误类别 | HTTP 状态码 | 说明 |
|---------|-----------|------|
| AI 超时（*_AI_TIMEOUT） | 504 Gateway Timeout | AI 调用超时，由 ai-impl 层超时拦截器自动触发 |
| AI 不可用（*_AI_UNAVAILABLE / *_AI_INPUT_INVALID / *_AI_NO_RECOMMENDATION） | 503 Service Unavailable | AI 服务不可用或输入校验失败，触发降级路径 |
| 输入校验违规（TRIAGE_FIELD_COMBINATION_INVALID） | 400 Bad Request | 字段组合不符合业务约束 |
| 会话/资源不存在（*_SESSION_EXPIRED / *_SESSION_NOT_FOUND / *_NOT_FOUND / *_VISIT_NOT_FOUND） | 404 Not Found | 请求的会话或资源不存在 |
| 并发冲突（*_CONCURRENT_SUBMIT / *_CONCURRENT_MODIFICATION） | 409 Conflict | 并发写入冲突，提示用户刷新重试 |
| 业务逻辑冲突（*_PRESCRIPTION_MODIFIED / *_FORCE_SUBMIT_INVALID / *_STREAM_NOT_SUPPORTED） | 409 Conflict | 前置条件不满足或业务约束冲突 |
| 跨模块门面不可用（*_FACADE_UNAVAILABLE / *_TEMPLATE_LOAD_FAILED / *_UNIT_MISMATCH） | 502 Bad Gateway | 依赖的跨模块门面不可用或数据格式不匹配 |
| BLOCK 阻断（步①/步②，CRITICAL 剂量或 BLOCK 审核） | 422 Unprocessable Entity | 使用 BlockResponse 格式返回，不经过统一错误包装 |

GlobalExceptionHandler 实现者按上表进行错误码到 HTTP 状态码的集中转换，无需在 §4.x 每个端点重复标注映射关系。

| 错误类别 | 前缀 | HTTP 状态码 | 代表场景 |
|---------|------|-----------|---------|
| 分诊（AI） | TRIAGE_ | 504/503 | TRIAGE_AI_TIMEOUT、TRIAGE_AI_UNAVAILABLE、TRIAGE_AI_INPUT_INVALID |
| 分诊（非AI） | TRIAGE_ | 400/404 | TRIAGE_SESSION_EXPIRED、TRIAGE_FIELD_COMBINATION_INVALID、TRIAGE_SESSION_NOT_FOUND、TRIAGE_DOCTOR_FACADE_UNAVAILABLE |
| 审核（AI） | RX_AUDIT_ | 504/503 | RX_AUDIT_AI_TIMEOUT、RX_AUDIT_AI_UNAVAILABLE、RX_AUDIT_AI_INPUT_INVALID |
| 审核（非AI） | RX_AUDIT_ | 409/422 | RX_AUDIT_PRESCRIPTION_MODIFIED（处方版本校验不一致）、RX_AUDIT_CONCURRENT_SUBMIT、RX_AUDIT_FORCE_SUBMIT_INVALID（forceSubmit=true 在无审核结果或 PASS 时无效） |
| 病历（AI） | MR_GEN_ | 504/503 | MR_GEN_AI_TIMEOUT、MR_GEN_AI_UNAVAILABLE、MR_GEN_AI_INPUT_INVALID、MR_GEN_AI_OUTPUT_INCOMPLETE |
| 病历（非AI） | MR_GEN_ | 404/409/502 | MR_GEN_STREAM_NOT_SUPPORTED、MR_GEN_VISIT_NOT_FOUND、MR_GEN_CONCURRENT_MODIFICATION、MR_GEN_TEMPLATE_LOAD_FAILED |
| 开方辅助（AI） | RX_ASSIST_ | 504/503 | RX_ASSIST_AI_TIMEOUT、RX_ASSIST_AI_UNAVAILABLE、RX_ASSIST_AI_INPUT_INVALID、RX_ASSIST_AI_NO_RECOMMENDATION、RX_ASSIST_AI_SUGGEST_CREATE_FAILED |
| 开方辅助（非AI） | RX_ASSIST_ | 404/502 | RX_ASSIST_DOSE_STANDARD_NOT_FOUND、RX_ASSIST_SUGGESTION_NOT_FOUND、RX_ASSIST_UNIT_MISMATCH |
...
| DoctorFacade 超时 | consultation.doctor-facade.timeout | 2s | 跨模块门面默认超时，调用超时或异常时 TriageResponse.doctors 置为空列表不阻断分诊主流程 |
| VisitFacade 超时 | medical-record.visit-facade.timeout | 2s | 跨模块门面默认超时，调用超时或异常时降级为 encounterId fallback 或返回 MR_GEN_VISIT_NOT_FOUND |
| DrugFacade 超时 | prescription.drug-facade.timeout | 2s | 跨模块门面默认超时，调用超时或异常时返回空药品信息 + WARN 日志不阻断主流程 |
...
四处独立的定时清理任务（DialogueSessionManager 会话 TTL 30 分钟、AiSuggestionResult TTL 60 分钟、PrescriptionDraftContext TTL 60 分钟、DeadLetterCompensationService 死信补偿每 30 分钟）以及 VisitIdReconciledTask（visitIdFallback 修复每 30 分钟）统一通过 Spring `@Scheduled` 注解集中调度，由 Spring 的 TaskScheduler 线程池统一管理线程资源。具体方案：

- 在 consultation 模块和 prescription 模块的 `config/` 包下各定义一个 `ScheduledTaskConfig` 配置类，使用 `@EnableScheduling` + `@Scheduled` 注解替代手动创建的 ScheduledExecutorService
- 各清理逻辑封装为独立的 `@Component` Bean 方法（如 `DialogueSessionCleanupTask.cleanup()`、`SuggestionCleanupTask.cleanup()`、`DraftContextCleanupTask.cleanup()`、`DeadLetterCompensationService.compensate()`、`VisitIdReconciledTask.reconcile()`），通过 Spring 的默认 TaskScheduler（1 个线程）执行；若需自定义线程池，通过 `@Bean(TaskScheduler)` 覆盖默认配置为线程池大小 2-4
- 统一任务执行异常处理：通过 `TaskSchedulerCustomizer` 或 `ErrorHandler` 注册全局异常处理，记录错误日志并触发告警，避免单次任务异常导致后续调度中断
- 优雅关闭：通过 Spring `@PreDestroy` 或 `@EventListener(ContextClosedEvent.class)` 确保应用关闭时任务线程池优雅终止完成已触发的清理操作
...
**VisitIdReconciledTask 定义**：medical-record 模块的 VisitIdReconciledTask（com.aimedical.modules.medicalrecord.task.VisitIdReconciledTask）为定时 reconciled 任务，每 30 分钟由 Spring @Scheduled(cron = "0 */30 * * * ?") 调度。核心职责：扫描 MedicalRecord 表中 visitIdFallback=true 且 visitId 与 encounterId 一致的记录（表明上次降级写入时未找到正确 visitId），通过 VisitFacade.findVisitIdByEncounterId(encounterId) 反查正确 visitId。若反查成功（返回非空 visitId），更新 MedicalRecord.visitId 为正确值并将 visitIdFallback 重置为 false；若反查仍为空（visit 模块仍无对应记录），保持 visitIdFallback=true 不下次轮任务继续尝试。**并发安全**：每次读取一条待修复记录后立即锁定（通过 `@Lock(LockModeType.PESSIMISTIC_WRITE)` 或 SQL `SELECT ... FOR UPDATE` 确保单实例内同一记录不被并发修复）；`visitIdFallback` 字段在 MedicalRecord 实体中定义（Boolean，nullable，默认 false），建立数据库索引以加速扫描查询。
...
**AiSuggestionResult TTL 清理**：AiSuggestionResult 由统一 Spring @Scheduled 任务每 5 分钟扫描清理 TTL 超过 60 分钟的过期条目，通过 §6.1 定义的集中调度框架统一线程管理。
...
| MissingFieldDetector | 差集比对检测模式（提示用户补全而非自动补全） | 避免自动补全引入错误信息，保留医生判断权——关键字段的医学信息应由医生命令式确认而非 AI 自动填充，差集比对保证已提取字段完整性，缺失字段以 FieldMissingHint 提示用户手动补充；与 RecordGenerateResponse.degraded 标记配合，degraded=true 时显示降级提示，degraded=false 时显示字段补全提示 |
...
DosageStandard.drugCode 采用国药准字号，关联药品基础信息实体（drug_base_info，位于 drug/master 模块）。关联方式：DosageStandard.drugCode 作为逻辑外键引用 drug_base_info.drugCode，不建立 JPA @ManyToOne 物理关联以避免 prescription 模块对 drug 模块的编译期依赖。跨模块的药品名称/规格信息查询通过 Service 层按需调用 DrugFacade（定义在 common-module-api 中）获取。DrugFacade 配置独立超时阈值（默认 2s），调用超时或不可用时返回空药品信息 + WARN 日志，不阻断辅助开方/处方审核主流程。DosageStandard 的写入端（admin 模块）和读取端（prescription 模块）均通过 drugCode 与药品基础信息实体做业务关联。
...
**DosageStandard 数据库索引**：为加速 DosageThresholdService 按优先级查询匹配，在 DosageStandard 实体上建立以下复合索引：
- `idx_dosage_drug_route`：(drugCode, routeOfAdministration)——覆盖按药品编码+给药途径维度匹配的主要查询路径，避免全表扫描
- `idx_dosage_drug_route_age_weight`：(drugCode, routeOfAdministration, ageRangeStart, ageRangeEnd, weightRangeStart, weightRangeEnd)——覆盖各级优先级匹配的复合查询，由 JPA `@Table(indexes = @Index(...))` 或 Flyway/Liquibase DDL 定义
...
六级匹配优先级（DosageThresholdService.check() 按优先级 1→2→3→4→5→6 依次尝试，命中即返回，不再继续检查更低优先级）：

1. **精确匹配**：ageRangeStart = ageRangeEnd = 患者年龄 AND weightRangeStart = weightRangeEnd = 患者体重
2. **同时范围匹配**：ageRange 与 weightRange 均非 null 但未达到精确匹配——检查 ageRangeStart ≤ 患者年龄 ≤ ageRangeEnd AND weightRangeStart ≤ 患者体重 ≤ weightRangeEnd，两项同时命中时使用该记录；若任一项未命中则继续检查下一优先级。**部分 null 边界规则**：ageRange 和 weightRange 各有 2 字段（start/end），null/非 null 组合产生 16 种状态。以下决策表列出关键路径，DosageThresholdService 实现时应按此表编写分支逻辑并覆盖对应测试用例：

| # | ageRangeStart | ageRangeEnd | weightRangeStart | weightRangeEnd | 匹配优先级 | 说明 |
|---|-------------|-----------|-----------------|---------------|-----------|------|
| 1 | 非 null | 非 null | 非 null | 非 null | Level 1（精确）或 Level 2（同时范围） | 全字段非 null——先尝试精确匹配，未命中则进入 Level 2 |
| 2 | 非 null | 非 null | 非 null | null | Level 3（年龄范围） | weightRange 部分 null，不满足"均非 null"先决条件 |
| 3 | 非 null | 非 null | null | 非 null | Level 3（年龄范围） | 同上 |
| 4 | 非 null | 非 null | null | null | Level 3（年龄范围） | weightRange 完全 null，使用年龄维度匹配 |
| 5 | 非 null | null | 非 null | 非 null | Level 5（默认阈值） | ageRange 部分 null——Level 3 先决条件不满足（ageRange 字段需"均为 null"或"均非 null"，部分 null 视为无效）；weightRange 均非 null 但 ageRange 无效 → Level 5 |
| 6 | 非 null | null | null | null | Level 5（默认阈值） | ageRange 部分 null + weightRange 完全 null |
| 7 | null | 非 null | 非 null | 非 null | Level 5（默认阈值） | 同#5 |
| 8 | null | 非 null | null | 非 null | Level 5（默认阈值） | ageRange 部分 null + weightRange 部分 null |
| 9 | null | null | 非 null | 非 null | Level 4（体重范围） | ageRange 完全 null，使用体重维度匹配 |
| 10 | null | null | null | null | Level 5（默认阈值） | 均完全 null——无分级默认阈值 |
| 11 | null | null | 非 null | null | Level 5（默认阈值） | weightRange 部分 null——Level 4 先决条件（均非 null）不满足 |
| 12 | null | null | null | 非 null | Level 5（默认阈值） | 同上 |

**需覆盖的边界测试用例**：
- 精确匹配命中（Level 1）：ageRangeStart=ageRangeEnd=患者年龄，weightRangeStart=weightRangeEnd=患者体重
- 同时范围匹配命中（Level 2）：ageRange 包含患者年龄，weightRange 包含患者体重，均非 null
- 同时范围匹配部分命中（Level 2→Level 3）：ageRange 包含但 weightRange 超出→降级至 Level 3（若 ageRange 均非 null）
- 精确匹配宽边界：ageRangeStart=ageRangeEnd=患者年龄，weightRangeStart=weightRangeEnd=患者体重=null→无法精确匹配，降级至 Level 2
- 部分 null #5 场景：ageRangeStart 非 null、ageRangeEnd null、weightRange 均非 null→直接降级至 Level 5
- 降级链全路径：1→2→3→4→5 全部未命中→Level 6 标准不存在
...
DosageStandard 当前无运行时缓存（每次校验直接查询数据库），因此不需要 DosageStandardChangeEvent。若未来引入缓存，需同步定义 DosageStandardChangeEvent 并遵循与 DrugContraindicationMapping 等实体一致的"事件驱动刷新 + 定时刷新"双重失效策略。
...
**跨阶段兼容风险**：本设计为 AiResult 新增了 failure(String errorCode, T partialData) 和 degraded(String fallbackReason, T partialData) 重载工厂方法（见 §2.3），这与 Phase 5 包 G OOD 中"AiResult.java 不变"的假设存在潜在冲突。若 Phase 5 团队保持 AiResult.java 不变，本设计的重载方法将在编译期冲突。**本阶段的兼容方案**（详见 §1.1c）：将重载方法移至 ai-api 模块中独立的静态工厂类 `AiResultFactory`，保持 `AiResult.java` 的原始签名不变——Phase 2/3 的 Converter 和 Service 实现依赖新工厂类，Phase 5 底座只需保留 AiResult.java 原有签名即可，双方互不冲突。
...
| 审查意见 | 修改措施 |
|---------|---------|
| 1.[一般] DrugFacade 同步跨模块调用缺失降级保护——既无超时配置边界，也无错误处理或降级策略定义 | §1.3 DrugFacade 核心抽象条目补充超时阈值（默认 2s）和降级描述（调用超时/失败时返回空药品信息 + WARN 日志，不阻断辅助开方/处方审核主流程）；§2.2 跨模块数据获取机制中 DrugFacade 段落补充超时配置注入方式和降级行为定义；§8.2 药品编码规范段同步补充 DrugFacade 降级声明 |
| 2.[一般] §8.4 六级匹配优先级文本编号与实际层级数不符——文本"1→2→3→4→5"与实际 6 级优先级不一致 | §8.4 将文本"按优先级 1→2→3→4→5"修正为"按优先级 1→2→3→4→5→6"以与实际列出的 6 级优先级匹配 |
| 3.[一般] RX_AUDIT_FORCE_SUBMIT_INVALID 错误码缺失于 §5.1 错误码表——该错误码在 §4.2 处方提交端点行为描述中使用但未列入错误码表 | §5.1 审核（非AI）错误码行补充 RX_AUDIT_FORCE_SUBMIT_INVALID，标注使用场景（forceSubmit=true 在无审核结果或 PASS 时无效） |
| 4.[轻微] AiSuggestionResult TTL 清理扫描间隔未显式定义——仅描述"由 ScheduledExecutorService 定期清理"，未给出具体清理间隔参数 | §3.4 AiSuggestionResult 条目补充清理间隔（每 5 分钟扫描清理 TTL 超过 60 分钟的条目）；§4.4 check-dose 流程同步更新 TTL 描述；§6.3 异步 AI 建议与去重段新增"AiSuggestionResult TTL 清理"段落，标注与 DialogueSessionManager 和 PrescriptionDraftContext 一致的 5 分钟扫描间隔 |

## 修订说明（v26）

| 审查意见 | 修改措施 |
|---------|---------|
| 1.[严重] /assist 主端点 CRITICAL 告警未写入 PrescriptionDraftContext——医生经 /assist 获取处方草案后直接提交，步①判定无 CRITICAL 放行 | §1.3 PrescriptionAssistService 和 PrescriptionAssistRequest 条目补充 CRITICAL 同步写入 PrescriptionDraftContext 及 prescriptionId 兜底；§3.4 PrescriptionAssistService 职责描述补充 CRITICAL 写入逻辑和 prescriptionId 兜底规则；§4.4 /assist 流程增加 prescriptionId 兜底处理步骤和 CRITICAL 写入步骤；§3.4 PrescriptionDraftContext 生命周期管理中创建时机扩展为"首次 check-dose 或 /assist 请求时" |
| 2.[严重] DedupTaskScheduler 去重逻辑存在 TOCTOU 竞态条件——查询→判断→创建三步非原子，两并发请求可同时通过"无 PENDING task"检查 | §3.4 去重原子性保障：在 SuggestionStore 接口新增原子性 createIfNotExists(taskId, prescriptionId, supplier) 方法，将去重判定（检查 PENDING/COMPLETED 未消费）和条件创建合并为存储层原子操作；DedupTaskScheduler 不再维护去重逻辑，仅调用 SuggestionStore.createIfNotExists()；Phase 2/3 ConcurrentHashMapStore 通过 compute() 实现，Phase 5 RedisStore 通过 Lua 脚本实现 |
| 3.[一般] 二次 CRITICAL 验证缺少快照保存机制——步①检查时未定义快照存储位置和格式 | §4.2 阻断竞态防护改为 SubmitContext 值对象方案——线程级闭包值对象，含 snapshotCriticalAlerts（List\<DosageAlert\>）和 submitStartTime；在 Controller 方法中创建，作为参数逐层传入 Service 方法栈，方法返回后销毁；步③前执行二次 CRITICAL 验证（实时列表与快照比较） |
| 4.[一般] DeadLetterCompensationService "重新投递"投递目标不明确——若重新发布事件会形成 @Retryable 无限循环 | §2.2 DeadLetterCompensationService 明确采用直接调用 TriageService.selectDepartment() 的补偿模式，不重新发布 ApplicationEvent；补充状态机迁移规则：FAILED →（补偿成功）→ COMPENSATED；FAILED →（重试耗尽）→ EXPIRED |
| 5.[一般] AiResult.prescriptionAssist() 超时场景 partialData 语义歧义——部分字段还是部分药品 | §3.4 partialData 格式定义明确为"已成功生成的完整药品条目列表"（部分药品而非部分字段）；每项含 drugId/drugName/dose/frequency/duration/route 完整六字段，不存在字段级缺失；补充 JSON 示例 |
| 6.[一般] MissingFieldDetector 在 §1.3 定义为 interface 但 §2.1 归属在 parser/ 包下 | §2.1 将 MissingFieldDetector.java 从 parser/ 子包调整至独立 detector/ 子包下 |
| 7.[轻微] TriageRecord 未定义"重新分诊"生命周期行为——同 sessionId 重新分诊的写入行为未定义 | §1.3 TriageRecord 目录注释补充"按 sessionId 一对一映射，同 sessionId 重新分诊时覆盖已有记录"；§3.1 TriageRecord 新增"重新分诊生命周期行为"段落：delete+insert 模式或 update 模式覆盖同 sessionId 记录，前端负责在发起新会话时生成新 sessionId |

## 修订说明（v27）

| 审查意见 | 修改措施 |
|---------|---------|
| P1.[严重] 事实错误：DeadLetterCompensationService 补偿路径缺少 departmentName | RegistrationEvent 补充 departmentName 字段（String，由 registration 模块在其领域内获取后填充），确保 DeadLetterCompensationService 从 eventPayload 反序列化后可提供 departmentName；§2.2 DeadLetterCompensationService 补偿路径更新为从事件载荷反序列化获取 departmentName |
| P2.[严重] 逻辑矛盾：AI 输入校验错误码无消费路径 | §2.3 AiService 各方法契约补充输入校验职责归属——AiService 实现层负责校验请求字段基本合法性，校验不通过时返回对应 `_AI_INPUT_INVALID` 错误码；为四个 AiService 方法分别标注校验范围及对应错误码 |
| P3.[严重] 深度不足：业务 Service 接口缺少显式方法签名 | §3.1 TriageService 补充 triage(DialogueCreateRequest)、selectDepartment() 方法签名；§3.2 PrescriptionAuditService 补充 audit(AuditRequest) 方法签名；§3.3 MedicalRecordService 补充 generate(RecordGenerateRequest) 方法签名；§3.4 PrescriptionAssistService 补充 assist(PrescriptionAssistRequest)、checkDose(DosageCheckRequest) 方法签名；各方法签名包含参数、返回类型和职责描述 |
| P4.[严重] 完整性缺口：patientId 兜底查询"最近分诊记录"缺少选择标准 | §2.2 sessionId 降级路径段补充最近分诊记录选择标准：按 triageTime 降序取第一条，triageTime 相同时按 recordId 降序；TriageRecordRepository 补充 findTopByPatientIdOrderByTriageTimeDesc 查询方法；TriageRecord 补充 patientId 索引说明 |
| P5.[一般] 事实错误：RegistrationEvent 缺少 departmentName，TriageRecord.finalDepartmentName 无法赋值 | RegistrationEvent 在 §1.3 核心抽象表和 §2.2 事件定义中同步补充 departmentName 字段，与 P1 复用同一修改 |
| P6.[一般] 深度不足：@Retryable 配置未定义重试触发条件 | §2.2 RegistrationEventListener @Retryable 描述补充重试触发条件——仅对 DataAccessException、TimeoutException 等可治愈临时异常重试；不可治愈异常（IllegalArgumentException、NullPointerException）通过 excludes/noRetryFor 直接进入 @Recover 回调 |
| P7.[一般] 完整性缺口：TriageRecord.sessionId 缺少唯一约束定义 | §3.1 TriageRecord 实体定义补充 `@Column(unique = true)`；重新分诊生命周期行为中明确 update 模式为推荐实现，唯一约束确保一对一映射的数据库级保障 |
| P8.[一般] 完整性缺口：多处 ScheduledExecutorService 独立运行，缺少集中管理 | §6.1 重写为定时任务集中管理方案——使用 Spring @Scheduled + TaskScheduler 统一管理四处定时任务；DialogueSessionManager、AiSuggestionResult、PrescriptionDraftContext、DeadLetterCompensationService 的调度描述全部更新为"统一 Spring @Scheduled 任务"；补充自定义线程池、全局异常处理和优雅关闭策略 |
| P9.[轻微] 概念歧义：api/ 子包同时放置 Controller 和 DTO | §2.1 prescription 模块目录结构调整：PrescriptionAuditController 和 PrescriptionAssistController 从 api/audit/ 和 api/assist/ 子包提升至 api/ 直接包下，与 consultation 模块的 api/TriageController.java 风格一致；dto/ 子包结构保持不变（DTO 按 audit/assist 分包属职责合理划分） |
| P10.[轻微] 边界条件遗漏：SuggestionStore.createIfNotExists 的 Phase 2/3 实现细节 | §3.4 createIfNotExists 补充精确契约——三参数（taskId/prescriptionId/supplier）及原子性语义、返回值语义、惰性求值保障；ConcurrentHashMapStore 通过 compute() 实现路径补充伪代码描述 |

## 修订说明（v28）

| 审查意见 | 修改措施 |
|---------|---------|
| 1.[一般] consumed 标记设置职责在 §3.4 和 §4.4 之间矛盾——§3.4 陈述"查询端点返回结果时将 consumed 置为 true"（后端自动设置），§4.4 陈述"前端标记 consumed=true 通知后端"（前端显式通知） | 统一为后端自动设置——§3.4 异步 AI 去重策略中"已消费判定"改为"查询端点在返回 COMPLETED 结果时自动将 consumed 置为 true（PENDING/FAILED/NOT_FOUND 路径不设置）"，并补充"前端崩溃后重新发起 check-dose 将创建新 task，属于可接受的边界行为"；§3.4 TTL 协调策略中"前端通过 GET ... 标记 consumed=true"改为"查询端点在返回 COMPLETED 结果时自动将 consumed 置为 true"；§4.4 异步建议查询端点中"前端标记 consumed=true 通知后端"改为"后端自动将 consumed 置为 true" |
| 2.[一般] DosageStandard 缺少变更事件通知，与同级配置实体的事件驱动刷新机制不一致 | §8.5 新增"缓存策略与变更事件"段落，显式声明"DosageStandard 当前无运行时缓存（每次校验直接查询数据库），不需要 DosageStandardChangeEvent"；§9.3 事件声明表中将 DosageStandardChangeEvent 标注为 Phase 2/3 预留——"DosageStandard 当前无缓存，变更事件推迟至引入缓存时实现" |
| 3.[轻微] AvailableDoctor 作为 DoctorFacade.findAvailableDoctorsByDepartment() 的返回值类型被引用但未在 §1.3 核心抽象表中作为正式 DTO 列出 | §1.3 跨模块门面与事件表中新增 AvailableDoctor 条目（class DTO，定义在 common-module-api 中，含 doctorId/doctorName/departmentId/availableSlotCount 四个字段） |
| 4.[轻微] VisitFacade fallback 路径中"visitId 格式约束"未定义——§3.3 RecordGenerateRequest 中有条件约束但无具体规则 | §3.3 RecordGenerateRequest fallback 路径移除格式约束条件，改为"直接使用 encounterId 字符串值作为 visitId，不校验额外格式约束，由 visit 模块在后续关联查询时验证有效性" |

## 修订说明（v11）

| 审查意见 | 修改措施 |
|---------|---------|
| R1 [一般] 需求文档引用的 Phase 5 包G OOD 参考文档未充分分析 | 新增 §1.1c 系统分析 Phase 5 包G OOD 的三个关键兼容性约束：(a) AiService 接口签名可变性——提议将新增重载方法移至独立静态工厂类 AiResultFactory 以兼容包G "AiResult.java 不变"约束；(b) 底座分层架构一致性——确认 AiService 接口作为唯一编译期入口与包G 底座架构一致；(c) Store 接口 package 路径——确认无冲突 |
| R2 [轻微] "关键字段缺失提示补全"中"补全"语义缺显式解释 | §7 设计决策表中 MissingFieldDetector 条目补充"差集比对检测模式（提示用户补全而非自动补全）"的选择理由——避免自动补全引入错误信息，保留医生判断权 |
| L1 [一般] MedicalRecordField.TREATMENT_ADVICE 与需求文档 3.4.3 输出字段名 treatment_plan 不匹配 | 将 MedicalRecordField 枚举值 TREATMENT_ADVICE 统一改为 TREATMENT_PLAN，同步更新 §1.3 核心抽象映射表、§4.6 JSON 示例、§9.1 初始模板数据集共 3 处引用 |
| L2 [一般] DrugInteractionPair 实体在 Phase 2/3 范围内无消费者 | §3.2 DrugInteractionRule 描述段补充 DrugInteractionPair 实体标注——增加 `@Table(schema = "PHASE4_PRELOAD")` 条件建表控制或通过版本控制脚本管理，避免 Phase 2/3 产生空数据库表 |
| L3 [轻微] §8.4 六级匹配优先级 Level 2 缺少"ageRange 或 weightRange 部分为 null"的边界规则 | §8.4 Level 2 补充"部分 null 边界规则"——ageRange 非 null 但 weightRange 部分为 null 时降级至 Level 3；ageRange 完全 null 而 weightRange 部分为 null 时降级至 Level 5 |
| D1 [严重] 处方提交端点缺少并发提交防护机制的具体实现位置 | §4.2 步③前新增"并发提交防护手段"段落——(a) AuditRecord 增加 @Version 乐观锁；(b) (prescriptionOrderId, forceSubmitted=true) 组合建立部分唯一索引；(c) RX_AUDIT_CONCURRENT_SUBMIT 触发时机与 HTTP 409 返回行为 |
| D2 [一般] 非AI跨模块门面超时配置未在 §5.5 超时配置表中集中收集 | §5.5 超时配置表新增三行：consultation.doctor-facade.timeout=2s、medical-record.visit-facade.timeout=2s、prescription.drug-facade.timeout=2s |
| D3 [一般] 处方版本校验的"结构化比较"未定义全 null/空列表等边界行为 | §4.2 一致性语义定义后补充边界规则——(a) 双方均为 null 或空列表视为一致，一方为空另一方非空视为不一致；(b) null 与 0/空字符串按业务语义等价处理；(c) 字段顺序差异不视为不一致 |
| D4 [轻微] RecordGenerateRequest 的 encounterId → visitId fallback 路径缺少数据完整性说明 | §3.3 RecordGenerateRequest fallback 路径补充 visitIdFallback=true 标记字段及 visit 模块 reconciled 任务修复说明 |

## 修订说明（v29）

| 审查意见 | 修改措施 |
|---------|---------|
| D1 [严重] 处方提交端点缺少正式 DTO 定义和完整 API 契约——(a) POST /api/prescription/submit 的请求/响应 DTO 无正式定义；(b) §4.6 缺少非 forceSubmit 场景和正常提交场景的完整 JSON 示例；(c) forceSubmit=false 且无最新审核结果时的"常规审核流程"具体路径未定义；(d) "常规提交流程"在无审核结果或 PASS 时的 DTO 层面定义缺失 | §1.3 包D-AI1 核心抽象表新增 SubmitRequest/SubmitResponse 条目；§3.2 新增 SubmitRequest 详细定义（含 auditRecordId 必填/可选语义边界表）和 SubmitResponse 定义；§4.2 处方提交端点重写——定义为 SubmitRequest/SubmitResponse DTO 格式，步③ forceSubmit=false 分支分解为无审核结果→先调用 PrescriptionAuditService.audit()、PASS→直接常规提交、WARN→内容变更检测三条子路径；§4.6 补充正常提交/forceSubmit=false+WARN/步①阻断三组 JSON 示例和 SubmitRequest/SubmitResponse 字段说明 |
| D2 [严重] visitIdFallback reconciled 任务仅有概念引用，无实现定义 | §6.1 新增 VisitIdReconciledTask 定义——每 30 分钟 Spring @Scheduled 调度，扫描 visitIdFallback=true 记录，通过 VisitFacade.findVisitIdByEncounterId() 反查正确 visitId 并修复；字段表和并发安全定义（@Lock PESSIMISTIC_WRITE）；§3.3 RecordGenerateRequest fallback 路径指向 §6.1 reconciled 任务 |
| D3 [严重] DialogueSession 事务一致性策略未覆盖 QA 历史等状态丢失——仅评估 aiFailCount 丢失的影响，未覆盖 (a) QA 历史列表 (b) correctedChiefComplaint (c) 对话轮次计数器 | §3.1 DialogueSession 一致性策略扩展为全过程可变状态影响评估表——覆盖 aiFailCount/QA 历史/correctedChiefComplaint/roundCount 四项；逐项评估可接受性；补充保障措施——(a) TriageRecord 增加 correctedChiefComplaint 快照 + 恢复查询方法；(b) QA 历史丢失通过"先写数据库再更新内存"保护 |
| D4 [严重] AiResultFactory 类未在目录结构中注册——未出现在 §2.1 目录结构、无包路径声明、无具体方法签名定义 | §2.1 ai-api 目录新增 AiResultFactory.java 条目（含包路径）；§2.3 AiService 接口定义段补充 AiResultFactory 四个静态工厂方法的完整签名（`failure(String, T)`、`degraded(String, T)`、`failure(String)`、`success(T)`） |
| I1 [一般] 缺少领域事件统一目录——10+ 种领域事件无集中目录，新开发者需全文检索了解事件驱动全貌 | §2.2 末尾新增"领域事件统一目录"表格——列出 8 种事件（RegistrationEvent/TemplateConfigChangeEvent/TriageRuleChangeEvent/DrugContraindicationChangeEvent/DrugAllergyMappingChangeEvent/DrugCompositionDictChangeEvent/DrugInteractionPairChangeEvent/DosageStandardChangeEvent）的事件名称、定义位置、发布/消费模块、触发条件、事务边界、Phase 范围；补充事件驱动刷新与定时刷新双重失效策略 |
| I2 [一般] 错误码→HTTP 状态码映射缺少集中声明——仅在 §4.x 散落标注部分映射关系，GlobalExceptionHandler 实现者需全文搜集 | §5.1 错误码命名规则后新增"错误码 → HTTP 状态码映射"集中声明表——覆盖 AI 超时(504)/AI 不可用(503)/输入校验违规(400)/会话资源不存在(404)/并发冲突(409)/业务逻辑冲突(409)/跨模块门面不可用(502)/BLOCK 阻断(422)共 8 类映射；错误码表每行新增 HTTP 状态码列 |
| I3 [一般] 六级匹配优先级 Level 2 边界规则的实现复杂度已升高——多字段 null/非 null 组合产生 2^4 = 16 种状态 | §8.4 Level 2 补充 12 行关键路径决策表（覆盖 ageRangeStart/ageRangeEnd/weightRangeStart/weightRangeEnd 各字段 null 与非 null 组合的降级路径），以及需覆盖的边界测试用例清单（精确匹配命中/同时范围匹配命中/部分命中降级/部分 null 降级/全路径降级链） |
| I4 [轻微] DosageStandard 数据库索引未定义——缺少 (drugCode, routeOfAdministration) 复合索引，大规模数据下可能全表扫描 | §8.4 DosageStandard 实体分级字段定义前新增复合索引说明——`idx_dosage_drug_route`（drugCode, routeOfAdministration）覆盖主要查询路径；`idx_dosage_drug_route_age_weight`（全字段复合）覆盖各级匹配；通过 JPA @Index 或 Flyway DDL 定义 |
| I5 [轻微] MockAiService 运行时策略切换的并发安全边界未定义——未定义切换时正在执行的 AI 调用使用旧策略还是新策略、策略存储结构、多线程影响 | §2.3 MockAiService 切换机制段补充并发安全定义——策略使用 `volatile` 声明确保可见性；正在执行调用不受策略切换影响（入口处读入局部变量快照）；声明可升级为 `AtomicReference` |
| I6 [轻微] 统一错误格式与 BlockResponse 的边界未覆盖所有提交端点错误——步①/步②阻断使用 BlockResponse（422），步③ forceSubmit 校验失败使用统一错误格式 | §4.6 HTTP 错误响应包装格式段新增"处方提交端点的错误格式边界"说明——步①/步②阻断使用 BlockResponse（HTTP 422，对应 SubmitResponse.blockInfo），步③ 校验失败使用统一错误格式（HTTP 409，对应 SubmitResponse.errorCode） |
