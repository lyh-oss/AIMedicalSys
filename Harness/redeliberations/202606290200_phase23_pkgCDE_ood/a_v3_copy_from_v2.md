...
|---------|---------|---------------|--------------|---------|
| registration 模块 | RegistrationEvent 的发布能力 + sessionId 填充逻辑 | 发布 RegistrationEvent（定义在 common-module-api），需在事件中填充 sessionId 字段 | 与 consultation 模块同步开发，最迟在挂号功能上线前完成 | sessionId 填充逻辑缺失将导致事件-driven finalDepartmentId 写入路径断裂，不影响手动选科路径 |
| visit 模块 | VisitFacade 接口的实现 | 实现 VisitFacade（定义在 common-module-api/visit/），提供 findVisitIdByEncounterId(encounterId) 等方法 | 与 medical-record 模块同步开发，最迟在病历生成功能上线前完成 | VisitFacade 实现缺失时 MedicalRecordService 无法完成 encounterId→visitId 转换，病历存储失败 |

**时间线约束**：两个外部依赖模块需在本设计对应的四个业务模块上线前完成接口实现，否则对应功能需以降级模式上线（如挂号事件缺失时降级为仅支持手动选科，病历生成缺失 visitId 时降级返回错误码）。
