...
| 抽象 | 类型形态 | 职责定位 |
|------|---------|---------|
| RegistrationEvent | class（事件） | 挂号事件契约，定义在 common-module-api 中。包含 registrationId、patientId、sessionId（可选，String——由 registration 模块从分诊上下文获取并填充，供 RegistrationEventListener 通过 sessionId 精确关联 TriageRecord）、departmentId、doctorId、eventTime。事件发布端为 registration 模块，消费端为 consultation 模块，在 application 模块聚合后跨模块传播 |
...
| 底座落地与 Phase 5 迁移兼容性 | 业务模块仅依赖 ai-api 的 AiService 接口（编译期依赖）；若 Phase 5 保持 AiService 接口签名不变，业务模块的核心 Service 和 Controller 的业务逻辑代码无须修改；但内存存储依赖部分需引入 Store 抽象层以隔离存储实现，迁移时替换存储实现而非修改业务代码 | 接口不变是迁移关键前提——AiService 接口定义在 ai-api 模块，业务模块通过接口调用 AI 能力，实现类替换对业务模块透明；三项内存储存（DialogueSessionManager、AiSuggestionResult、PrescriptionDraftContext）建议引入 Store 抽象层（如 SessionStore、SuggestionStore、DraftContextStore），Phase 2/3 用 ConcurrentHashMap 实现，Phase 5 替换为 Redis 实现，业务 Service 代码不感知存储变更；若 Phase 5 需扩展 DTO 字段，业务模块的 Converter 需同步更新 |
