...
**重新分诊生命周期行为**：TriageRecord 按 sessionId 一对一映射——同一 sessionId 第二次发起分诊时，TriageRecordRepository 先删除该 sessionId 的已有记录再插入新记录（delete+insert 模式），或通过 update 覆盖同 sessionId 记录（update 模式）。前端负责在患者发起新会话时生成新的 sessionId，后端不假设 session 复用场景下的语义自动合并。此设计确保分诊记录的 sessionId 域内唯一，每次分诊对应一条最新记录。
...
**去重原子性保障**：为消除"查询 AiSuggestionResult → 判断状态 → 按条件创建新 task"三步操作之间的 TOCTOU 竞态条件，在 SuggestionStore 接口新增原子性方法 `SuggestionStore.createIfNotExists(taskId, prescriptionId, supplier)`——该方法将去重判定（检查该 prescriptionId 下是否存在 PENDING/COMPLETED 未消费的 AiSuggestionResult）和条件创建合并为存储层原子操作。Phase 2/3 的 ConcurrentHashMapStore 实现通过 `ConcurrentHashMap.compute()` 在锁保护的函数内完成检查+创建；Phase 5 的 RedisStore 实现通过 Lua 脚本 `redis.call('EXISTS', key) and 0 or redis.call('SET', key, value)` 等原子命令完成。DedupTaskScheduler 不再自身维护去重逻辑，仅调用 SuggestionStore.createIfNotExists() 并处理返回结果。
...
AiSuggestionResult 增加 TTL（60 分钟），由 ScheduledExecutorService 每 5 分钟扫描清理 TTL 超过 60 分钟的条目。
...
生命周期管理：创建时机（首次 check-dose 或 /assist 请求时——DraftContextStore 通过 ConcurrentHashMapStore 的 computeIfAbsent() 惰性创建 Entry，不存在空引用风险；写入操作通过 Store 接口的 put() 方法委托给后端存储的原子写入）、写入时机（CRITICAL 告警时，每次 check-dose 或 /assist 重算后覆盖该 prescriptionId 下已有的所有 CRITICAL 标记——若新结果无 CRITICAL 则清除旧标记）、清理时机（处方提交成功/处方取消/TTL 过期 60 分钟）。**TTL 过期清理由 ScheduledExecutorService 定期扫描实现（每 5 分钟扫描一次，遍历 DraftContextStore 清理最后更新时间超过 60 分钟的条目）**，确保异常退出场景下 CRITICAL 标记不会无限残留。
...
**AiSuggestionResult TTL 清理**：AiSuggestionResult 由 ScheduledExecutorService 每 5 分钟扫描清理 TTL 超过 60 分钟的过期条目，与 DialogueSessionManager（§6.1）和 PrescriptionDraftContext（§6.4）的清理机制一致。
