...
| `ClientType` | enum | 模型客户端类型枚举，`HTTP_API`（直连 HTTP 调用）/ `SPRING_AI`（Spring AI ChatModel 封装），用于 `ModelRoute` 字段和 `DelegatingLlmChatService` 分发决策。⚠️ **枚举值绑定约束**：YAML 配置中 `client: "HTTP_API"` 字符串值通过 Spring Boot Relaxed Binding 绑定到 `ClientType` 枚举，绑定区分大小写且要求字符串值与枚举常量名完全一致。未匹配值时抛出 `ConversionFailedException`。建议实现层在 `ModelRoute` 的 `clientType` setter 中增加防御性字符串转换（如 `ClientType.valueOf(value.toUpperCase())`），或注册 `@Converter(autoApply = true)` 提供宽松匹配。底座切流初期若 YAML 配置中的 `client` 值拼写错误，`DelegatingLlmChatService` 分发层将回退到默认实现（`HttpApiLlmChatService`），日志 WARN 提示转换失败 |
    ...
    try:
      ...
      try:
        ...
        if chatAiResult.isSuccess():
          ...
          endpointHealthManager.recordCallResult(endpointId, true, System.currentTimeMillis() - startTime)
          try:
            // parse() 有独立 5s 超时控制（见 §3.2 StructuredOutputParser 契约），使用
            // CompletableFuture.supplyAsync() + .get(5s) 包裹，避免 JSON 解析步骤无限挂起
