...
DiscussionConclusionCapabilityExecutor.doExecuteInternal(startTime, request, capabilityId,
    ...
    if tokenizerAvailable:
        ...
        // 为缓解假阳性影响，回退分支的阈值从 3000 提升至 4000 tokens，
        // 使字符估算路径的触发条件更保守，减少不必要的前置压缩调用。
        // 同时每触发一次回退分支估算，记录 WARN 日志 + Micrometer Counter 指标
        ...
        if estimatedTokens > 3000:
            log.warn("estimateTokens() 回退到字符估算: capabilityId={}, estimatedTokens={}, 假阳性风险 30%~70%",
                capabilityId, estimatedTokens)
            // 回退路径阈值取更保守值（4000），减少误触发压缩
            if estimatedTokens <= 4000:
                estimatedTokens = 2000  // 标记为 2000，确保不触发前置压缩（低于 3000 阈值）
