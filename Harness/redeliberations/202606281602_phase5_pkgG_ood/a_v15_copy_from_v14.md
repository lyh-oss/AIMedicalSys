...
| DIAGNOSIS | `DiagnosisRequest` | 1（symptomDescription）+ 4 AiRequestBase 继承字段 | 同 TRIAGE | 否（当前为空类，新增字段仅底座使用） | ~60 行 | 薄适配器 DTO；由 Phase 4 模块自行决定是否继承 AiRequestBase |
| ANALYSIS_REPORT_INSPECTION | `AnalysisReportForInspectionRequest` | 1（reportData）+ 4 AiRequestBase 继承字段 | 同 TRIAGE | 否 | ~60 行 | 薄适配器 DTO |
| ANALYSIS_REPORT_LABTEST | `AnalysisReportForLabTestRequest` | 1（reportData）+ 4 AiRequestBase 继承字段 | 同 TRIAGE | 否 | ~60 行 | 薄适配器 DTO |
| IMAGE_ANALYSIS | `ImageAnalysisRequest` | 2（imageData/analysisType）+ 4 AiRequestBase 继承字段 | 同 TRIAGE | 否 | ~70 行 | 薄适配器 DTO |
| RECOMMEND_EXAM | `RecommendExaminationRequest` | 2（diagnosisCode/patientInfo）+ 4 AiRequestBase 继承字段 | 同 TRIAGE | 否 | ~70 行 | 薄适配器 DTO |
| RECOMMEND_EXEC_ORDER | `RecommendExecutionOrderRequest` | 2（examinationList/patientInfo）+ 4 AiRequestBase 继承字段 | 同 TRIAGE | 否 | ~70 行 | 薄适配器 DTO |
...
|-------|-------------|-----------|---------|
| `execution.timeout.*` | `AiExecutionProperties` | 自定义定时刷新（基于 `@Scheduled` 轮询配置源）+ `@RefreshScope` 兼容性兜底 | 在 `AiPlatformConfig` 中新增 `scheduledConfigRefresh()` 定时任务，每 60 秒重新读取 `@Value` 注入的配置属性（通过 `Environment` 直接查询），若检测到版本变更则重新构建 `capabilityTimeoutConfig` 和 `thinAdapterPerCapabilityConfig` 的 `@Bean` 缓存 |
