...
- **注入方式伪代码**（AbstractCapabilityExecutor 构造器，v4 迭代修正——移除 @Qualifier 策略注入，改为 Map 统一注入）：
   ...
   **⚠️ 构造器参数数量说明**：当前构造器包含 13 个参数（不含 `@Qualifier` 和 `@Value` 注解参数），`doDegrade()` 方法包含 14 个参数，`AiCallRecord` 工厂方法包含 15~16 个参数。大量参数构成编码实施风险（参数顺序依赖、调用点复杂度）。此问题为已知设计约束，建议在 Phase 6 或实施期引入上下文参数对象聚合通用参数：(a) 抽取 `ExecutionContext` 聚合 `departmentId/callerRole/callerId/visitId/patientId/sessionId/inputSummary/outputSummary` 等 8 个业务上下文参数，将 `doDegrade()` 从 14 参数降维至 7 参数；(b) `AiCallRecord` 工厂方法引入 `CallRecordContext` 聚合 `visitId/patientId/sessionId/callerRole/callerId/inputSummary/outputSummary` 等参数。当前设计保留显式参数传递以保证上下文更新的可追踪性，实现者可选择实施此优化。
...
**工厂方法签名**（`sentinelReason` 为可选参数，位于参数列表末尾，默认 null。当 `sentinelReason="EXPERIMENT_ASSIGN_ERROR"` 时内部将 `promptVersion` 覆写为 -1 哨兵值）：
```java
static AiCallRecord success(String capabilityId, LocalDateTime callTime, long elapsedMs,
    ...
    @Nullable String sentinelReason)
...
| `sliding-window.window-seconds` | `AiSlidingWindowProperties` | `@RefreshScope`（类型为基本类型 `long`，兼容性良好） | 在 `SlidingWindowMetricsStore` 的构造器中通过 `@Value` 注入 `windowSeconds`，该 Bean 标注 `@RefreshScope` 并实现 `RefreshScope` 刷新回调 |
  ...
  if localRuleFallback != null:
    ...
    if inputType != null && !inputType.isInstance(request):
      ...
      metricsCollector.record(AiCallRecord.failure(capabilityId, LocalDateTime.now(), elapsedMs,
          ...
          departmentId, inputSummary, visitId, patientId, sessionId, callerRole, callerId, promptVersion, sentinelReason))
      slidingWindowMetricsStore.recordFailure(capabilityId)
    ...
    metricsCollector.record(AiCallRecord.degraded(capabilityId, LocalDateTime.now(), elapsedMs,
        ...
        callerRole, callerId, promptVersion, sentinelReason))
...
ai:
  ...
  sliding-window:
    # [动态] 滑动窗口时间宽度，默认 60 秒
    # 热加载支持：@RefreshScope（基本类型 long，兼容性良好）+ SlidingWindowMetricsStore 实现 RefreshScope 刷新回调
    # 重启要求：修改后调用 /actuator/refresh 触发刷新，无需重启实例
...
| `sliding-window.window-seconds` 刷新 | 标注 `SlidingWindowMetricsStore` 为 `@RefreshScope`，通过 `ContextRefresher.refresh()` 模拟刷新 | 刷新后 `windowSeconds` 字段更新为新值，惰性淘汰阈值 `System.currentTimeMillis() - windowSeconds * 1000` 同步生效 |
