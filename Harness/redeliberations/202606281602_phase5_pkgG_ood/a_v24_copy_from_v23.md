...
class DiagnosisCapabilityExecutor extends AbstractCapabilityExecutor<DiagnosisRequest, DiagnosisResponse> {
    ...
    DiagnosisCapabilityExecutor(
        ...
        super(null, null, null, null,
              ...
              capabilityTimeoutConfig, thinAdapterTimeout, thinAdapterPerCapabilityConfig, objectMapper);
...
- **注入方式伪代码**（AbstractCapabilityExecutor 构造器，v4 迭代修正——移除 @Qualifier 策略注入，改为 Map 统一注入）：
   ...
   **⚠️ 构造器参数数量说明**：当前构造器包含 13 个参数（不含 `@Qualifier` 和 `@Value` 注解参数），`doDegrade()` 方法包含 14 个参数，`AiCallRecord` 工厂方法包含 15~16 个参数。大量参数构成编码实施风险（参数顺序依赖、调用点复杂度）。**以下上下文值对象为 Phase 5 第二阶段重构目标**：

   (a) **`CallContext` 值对象**（归属 `ai-api/dto/base/`）：聚合以下 9 个业务上下文字段，作为 `doDegrade()`、`doExecuteInternal()` 和 `AiCallRecord` 工厂方法的统一参数：
   ```
   class CallContext {
   ...
   ```
   - `CallContext` 为不可变对象（所有字段 `final`，通过全参构造器或 Builder 赋值），提供 `withOutputSummary(String)` 和 `withPromptVersion(Integer)` 的复制方法（返回新实例）以支持管线中增量赋值
   - 管线伪代码中 `AiCallRecord` 工厂方法从 15~16 参数降维至 6 参数：
     ```
     AiCallRecord.success(capabilityId, callTime, elapsedMs, modelId, retryCount, tokenUsage, callContext)
     AiCallRecord.failure(capabilityId, callTime, elapsedMs, errorCode, errorMessage, callContext)
     AiCallRecord.degraded(capabilityId, callTime, elapsedMs, degradeReason, modelId, callContext)
     ```
   - `doDegrade()` 方法从 14 参数降维至 7 参数：
     ```
     doDegrade(startTime, degradeReason, request, capabilityId, modelId, sentinelReason, callContext)
     ```
   - `doExecuteInternal()` 方法从 11 参数降维至 6 参数：
     ```
     doExecuteInternal(startTime, request, capabilityId, userId, callContext)
     ```
   - `CallContext` 在 `AbstractCapabilityExecutor.execute()` 入口处（supplyAsync 之前）从 SecurityContextHolder 和 RequestContext 提取后构建为不可变实例，以参数形式传入下游各方法。管线中各步骤可通过 `callContext.withOutputSummary(...)` 生成副本传递新值，保持不可变性

   (b) `CallContext` 引入后，`doDegrade()`、`doExecuteInternal()`、`AiCallRecord` 工厂方法的参数数量均降维至 ~6~7 个，编码实施风险显著降低。`doDegrade()` 内通过 `callContext.getInputSummary()`/`callContext.getOutputSummary()`/`callContext.getVisitId()` 等 getter 获取各字段值

   (c) **与现有方法的二进制兼容性**：新旧方法签名不可共存（名称相同参数不同）。Phase 5 实施期先在 `ai-api` 模块中新增 `CallContext` 类，然后一次性将 `AiCallRecord` 工厂方法和 `AbstractCapabilityExecutor` 相关方法全部替换为新签名（不保留重载旧版本），由编译器确保所有调用点同步更新。替换属于不兼容变更，在发版说明中标注为"Phase 5 底座引入——AiCallRecord 工厂方法签名简化，旧签名不再保留"
...
**工厂方法签名**（Phase 5 第二阶段重构目标：使用 `CallContext` 值对象降维，`sentinelReason` 为可选参数，位于参数列表末尾，默认 null。当 `sentinelReason="EXPERIMENT_ASSIGN_ERROR"` 时内部将 `promptVersion` 覆写为 -1 哨兵值。**当前 §4.1 伪代码仍使用旧的多参数签名**，第二阶段实施时统一替换）：
```java
...
| KB_QUERY | `KbQueryRequest` | 4（queryText/queryScope/departmentId/maxResults）+ 4 AiRequestBase 继承字段 | 同 TRIAGE | 否 | ~80 行 | queryScope 为 KbQueryScope 枚举(~15行) |
...
public Executor transcriptSummaryExecutor() {
    ...
    // 队列容量 20，拒绝策略 CallerRunsPolicy（自然背压，压缩失败时降级为截断文本）
    ...
    try:
      ...
      try:
        ...
        if aiResult.isSuccess():
          ...
          // fall-through to shared success handler
          else:
...
DiscussionConclusionCapabilityExecutor.doExecuteInternal(startTime, request, capabilityId,
    ...
    if estimatedTokens > 3000:
        ...
        try:
            compressedFuture = CompletableFuture.supplyAsync(() -> {
                ...
                //   @Value("${ai.compression.lightweight-endpoint}") String compressionLightweightEndpoint
    ...
    // 后续复用标准 doExecuteInternal 流程（实验分流→模板渲染→模型路由→LLM 调用→结构化解析→指标采集）
    // 以下为精简后的标准流程（实际实现中通过 super.doExecuteInternal() 调用父类公共模板方法，
    // 此处展开以体现完整行为契约）：
    boolean experimentAssignFailed = false
    try:
        assignment = experimentManager.assign(capabilityId, userId, sessionId)
    catch (Exception e):
        log.warn("实验分流异常: capabilityId={}", capabilityId, e)
        assignment = ExperimentAssignment.createErrorFallback()
        experimentAssignFailed = true
    promptVersion = assignment.getTargetPromptVersion()
    sentinelReason = experimentAssignFailed ? "EXPERIMENT_ASSIGN_ERROR" : null
    ...
    try:
        ...
        log.warn("模板渲染失败: capabilityId={}", capabilityId, e)
        renderedPrompt = promptTemplateManager.getFallbackPrompt(capabilityId)

    modelRoute = modelRouter.route(capabilityId, assignment)
    if modelRoute == null:
        return doDegrade(startTime, DegradationReason.NO_AVAILABLE_ROUTE, request,
            capabilityId, departmentId, callerRole, callerId, visitId, patientId, sessionId,
            inputSummary, null, promptVersion, null, sentinelReason)

    // 后续步骤复用父类标准流程：健康检查→LlmChatOptions构造→LlmChatRequest构造→
    // structuredChat + chat回退→结构化解析→指标采集
    // （与 §4.1 标准 doExecuteInternal() 的 remainder 一致，此处省略重复展开）
```
...
ai:
  ...
  compression:
    ...
    # compressionLightweightEndpoint — 轻量模型端点标识，通过 @Value("${ai.compression.lightweight-endpoint}") 注入
...
| 审查意见 | 修改措施 |
|---------|---------|
| 1. [重要] §3.7 PrescriptionLocalRuleFallback 引用 PrescriptionCheckRequest 上不存在的字段 allergyInfo | §3.7 过敏史检查项数据来源从 `PrescriptionCheckRequest.allergyInfo` 改为 `request.patientInfo.getAllergyInfo()`，注明方法签名 `PatientInfo.getAllergyInfo(): List[String]`；数据来源列同步更新 |
| 2. [重要] §4.1 doExecuteInternal() 伪代码中 parsedResult 变量作用域错误 | §4.1 doExecuteInternal() 中在第一个 try 块前声明 `Object parsedResult = null`（与 retryCount/tokenUsage 同位置）；在 line 2906 访问 parsedResult 前添加 null 守卫检查，null 时走降级路径（DegradationReason.INTERNAL_ERROR + ":parsedResultIsNull"） |
| 3. [中等] §3.5 聚合 SQL 使用 MySQL 不支持的 PERCENTILE_CONT | §3.5 聚合 SQL 中的 PERCENTILE_CONT(0.5/0.95/0.99) 替换为注释说明的 MySQL 兼容方案（ROW_NUMBER() + COUNT(*) 方式），SQL 中改用 0 AS 占位并附详细实现说明；新增百分位数实现说明段落 |
| 4. [中等] 多个 @Scheduled 任务缺少调度线程池配置定义 | §3.9 AiPlatformConfig 中补充 `@Bean("scheduledTaskExecutor")` ThreadPoolTaskScheduler 定义，poolSize=3，命名前缀 "scheduled-task-"，配置正常关闭等待与优雅停机 |

## 修订说明（v11）

| 审查意见 | 修改措施 |
|---------|---------|
| 1. [重要] §2.3 类图中 `doDegrade` 方法签名缺少 `modelId` 参数，与 §4.1 伪代码不一致 | §2.3 类图 `AbstractCapabilityExecutor` 中 `doDegrade` 方法签名末尾追加 `modelId` 参数；同步修正 §3.1 薄适配器模板方法伪代码中两处 `doDegrade` 调用点（line 1070/1085 各追加第 14 个参数 `null` 对应 `modelId`），使类图与 §4.1 伪代码定义一致 |
| 2. [严重] §3.1 薄适配器构造器中 `super()` 调用参数数量与 `AbstractCapabilityExecutor` 构造器签名不匹配 | §3.1 薄适配器示例伪代码 `DiagnosisCapabilityExecutor` 构造器：入参新增 `capabilityTimeoutConfig`、`thinAdapterTimeout`、`thinAdapterPerCapabilityConfig` 三个参数；`super()` 调用同步追加对应三个实参，使参数数量与 `AbstractCapabilityExecutor` 构造器签名匹配 |
| 3. [严重] §4.2 薄适配器 catch 块引用未确认存在的 `BusinessException` 异常类型 | §3.1 薄适配器模板方法伪代码与 §4.2 薄适配器特化管线伪代码中：将 `catch (BusinessException e)` 与 `catch (Exception e)` 合并为统一的 `catch (Exception e)`，通过 `originalType` 类名匹配 6 个已知 Phase 4 业务异常类型（`DiagnosisException` 等）区分业务异常与基础设施异常；§3.1 Phase 4 模块异常契约表更新为与统一 catch 匹配的描述 |

## 修订说明（v12）

| 审查意见 | 修改措施 |
|---------|---------|
| 5. [保留-重要] AiOrchestrator.handle() catch 块中 callerRole 提取逻辑与 AbstractCapabilityExecutor.extractCallerRole() 不一致 | (§3.10) `RequestContextUtils` 新增 `extractCallerRole()` 和 `extractCallerId()` 公共静态方法，统一实现 ROLE_ 前缀过滤逻辑；(§4.1) AiOrchestrator.handle() catch 块中的 callerRole/callerId 提取改为委托 `RequestContextUtils.extractCallerRole()`/`extractCallerId()`；(§3.1) AbstractCapabilityExecutor.extractCallerRole()/extractCallerId() 实现同样委托至 `RequestContextUtils`，消除两处提取方式不一致 |
| 6. [保留-重要] PatientInfo 类型未定义且 PrescriptionLocalRuleFallback 引用了不存在的字段路径 | (§3.11.2) 新增 `PatientInfo` 值对象字段定义表（age/weight/allergyInfo/pregnancyStatus）；(§3.7) 修正 `request.patientAge` 为 `request.patientInfo.getAge()`、`request.pregnancyStatus` 为 `request.patientInfo.getPregnancyStatus()` |
| 7. [新增-重要] doExecuteInternal() 中 catch(TimeoutException) 因 .join() 包装成为死代码 | (§4.1) structuredChat 路径：移除原 `catch(TimeoutException)` 死代码块，在 `catch(CompletionException)` 中增加 `ce.getCause() instanceof TimeoutException` 检查分支，按结构化超时处理；chat 回退路径：同样移除 `catch(TimeoutException)` 死代码，在 `catch(CompletionException)` 中增加对应 TimeoutException 检查分支。超时原因统一从 `INFRASTRUCTURE_ERROR` 修正为 `TIMEOUT` |
| 2. [保留-一般] ExperimentGroup 类图节点缺失（图-文不一致） | (§2.3 类图) 在 `Experiment` 类节点前新增 `ExperimentGroup` 类节点（含 id/group_id/percentage/target_model_id/target_prompt_version 字段）及 `Experiment → ExperimentGroup` 的 `1 o--> 0..*` 关联线 |
| 3. [保留-一般] AiCallLogStats 类图节点缺失（图-文不一致） | (§2.3 类图) 在 `AiCallLogRepository` 之后新增 `AiCallLogStats` 类节点（含 id/capability_id/stat_month/total_calls/success_count/degraded_count/failure_count/avg_elapsed_ms/p50_elapsed_ms/p95_elapsed_ms/p99_elapsed_ms 字段）及 `AiCallLogStatsRepository` 节点 |
| 4. [保留-一般] StructuredOutputParser.parse() 独立超时未体现于 §4.1 伪代码 | (§4.1) `structuredOutputParser.parse(chatResponse, outputType)` 调用包裹 `CompletableFuture.supplyAsync().get(5, TimeUnit.SECONDS)`，新增 `catch(TimeoutException parseTe)` 降级分支，降级原因 `PARSE_FAILURE + ":parseTimeout"` |
| 8. [新增-一般] PrescriptionAssistRequest 与 PrescriptionCheckRequest 的患者数据建模方式不一致 | (§3.11.4) PrescriptionAssistRequest 输入 DTO 扩展字段中，将 `patientAge`/`patientWeight`/`allergyInfo` 三个扁平字段统一为 `patientInfo: PatientInfo` 内嵌对象，与 PrescriptionCheckRequest 共用同一 `PatientInfo` 值对象定义 |
| 9. [新增-一般] AbstractCapabilityExecutor 构造器中 ObjectMapper 的来源未定义 | (§3.1 AbstractCapabilityExecutor 构造器伪代码) 新增 `@Autowired ObjectMapper objectMapper` 构造参数及对应 `this.objectMapper = objectMapper;` 赋值，添加注释说明用于 `execute()` 入口处的防御性拷贝 |

## 修订说明（v13）

| 审查意见 | 修改措施 |
|---------|---------|
| 1. [重要] 薄适配器系统性分析盲区——6 项薄适配器能力的 AiCallRecord 中 modelId/promptVersion/retryCount 三个字段永远为空（传入 null/0），导致近半数 AI 能力的性能分析无法按模型维度聚合，违反「性能观测内建化」目标 | (§1.4.1) 新增「薄适配器性能观测维度缺口」小节，以三列表格明确列出缺失维度、根因与预期补齐方式；补充影响分析和补齐时间线；(§3.1) 新增「Phase 4 服务可选元数据接口契约」段，定义 `Phase4ServiceMetaProvider` 接口（三个 default method 返回安全默认值），底座通过 `instanceof` 安全检测后调用；(`§4.2) 薄适配器成功路径伪代码中新增元数据提取步骤（`if instanceof Phase4ServiceMetaProvider` 分支），替代原硬编码 null/0；降级路径 modelId/promptVersion/retryCount 保持 null/0（元数据仅在成功路径可获取） |
| 2. [重要] 实验分流异常不可追溯——`ExperimentManager.assign()` 在「实验分流异常」（§4.1 catch 块）与「无实验命中」（正常业务）两种场景下均返回 groupId="default"、promptVersion=null，下游实验效果分析报表无法区分正常基线数据与异常降级污染数据 | (§3.4 ExperimentAssignment) 新增 `createErrorFallback()` 工厂方法（groupId="experiment-error"），与 `createDefault()`（groupId="default"）可区分；补充「无实验命中」与「实验分流异常」语义区分规则段落；(§3.4) 新增「AiCallRecord.promptVersion 空值守卫标记方案」段——错误场景写入 `prompt_version=-1` 哨兵值，正常未命中写入 null；(§4.1) 实验分流异常 catch 块改为 `ExperimentAssignment.createErrorFallback()`；(§4.4) 实验分流契约补充异常/正常场景区分规则说明 |
| 3. [重要] 运行时配置热加载覆盖不完整——`execution.timeout.per-capability`、`execution.timeout.thin-adapter.per-capability`、`degradation.strategies`、`sliding-window.window-seconds` 等 4 项关键运行时配置仅启动期通过 `@ConfigurationProperties` 一次性绑定，不支持热加载 | (§3.9) 新增「运行时配置热加载机制」段落，详细分析 4 项配置的热加载局限性（Map 字段的 @RefreshScope 限制）、复合策略实现方式（自定义定时刷新 + @RefreshScope 兼容兜底 + 回退方案 /actuator/refresh）；配置项热加载状态标记说明；(§9.5) 4 项配置注释增加 `[动态]` / `[静态·重启生效]` 标记及热加载说明；(§11.6) 新增「配置热刷新集成测试」4 组 `@SpringBootTest` + `@Nested` 测试，覆盖 timeout 刷新、strategy 热替换、window-seconds 刷新、无效配置不降级 |
| 4. [一般] 流量分配千分比语义不匹配——`ExperimentGroup.percentage` 值域 0-1000（千分比）与常见百分比表述不一致，管理端配置需额外换算 | (§3.4 ExperimentGroup)「千分比（0-1000）设计决策理由」新增三段说明：整数精度避免浮点误差、值域与哈希模数一致（无需额外缩放）、管理端配置复杂度可通过界面自动换算缓解 |

## 修订说明（v14）

| 审查意见 | 修改措施 |
|---------|---------|
| 1. [严重] §5.1 错误分类表"实验分流异常"处理方式与 v13 实际设计矛盾——仍描述为降级到 default 分组（`new ExperimentAssignment(null, "default", null, null)`），但 v13 已统一改为 `ExperimentAssignment.createErrorFallback()`（`groupId="experiment-error"` 哨兵分组） | (§5.1 错误分类表) 将"实验分流异常"行的处理方式替换为 `ExperimentAssignment.createErrorFallback()`（`groupId="experiment-error"`），与 §3.4/§4.1/§4.4 保持一致；(§11.1 单元测试模式) 将实验分流异常验证断言从"验证降级到 default 分组"修正为"验证使用 `ExperimentAssignment.createErrorFallback()`（`groupId="experiment-error"`）" |
| 2. [重要] `DiscussionConclusionCapabilityExecutor` 前置 LLM 压缩调用阻塞 `llmCallExecutor` 线程池，高并发下可能导致线程饥饿 | (§3.11.7) 模板变量提取策略单元格中补充线程模型风险分析三部分：(a) 阻塞风险评估——`doExecuteInternal()` 运行于 `supplyAsync()` lambda，前置 LLM 调用额外阻塞 15s；(b) 隔离方案建议——引入独立 `transcriptSummaryExecutor` 线程池或异步重试机制；(c) YAML 注释补充隔离说明；(§9.5) `transcript-summary: 15s` 注释补充线程池隔离说明 |
| 3. [重要] `doDegrade()` 14 参数、`AbstractCapabilityExecutor` 构造器 13+ 参数、`AiCallRecord` 工厂方法 15~16 参数构成编码实施风险 | (§3.1) 构造器伪代码后新增「参数数量说明」段落，建议 Phase 6 或实施期抽取 `ExecutionContext` 和 `CallRecordContext` 上下文参数对象聚合通用参数，将 `doDegrade()` 降维至 7 参数 |
| 4. [一般] YAML 配置 `client` 字段到 `ClientType` 枚举的隐式绑定约束未说明 | (§1.3 CoreTable `ClientType` 描述) 补充枚举值绑定约束说明——Relaxed Binding 区分大小写，需 `ClientType.valueOf(value.toUpperCase())` 防御性转换或 `@Converter`，未匹配时回退到默认实现并日志 WARN |
| 5. [一般] `LlmChatRequest` 字段构造方式不一致——`tools` 通过独立 setter 设置易被遗漏 | (§3.2 LlmChatRequest 字段级契约) `tools` 字段说明中补充构造方式建议：(a) 将 `tools` 纳入全参构造器；(b) 若保持当前设计，显式声明 `tools` 为 null 时相当于调用 `chat()` + `StructuredOutputParser.parse()` |
| 6. [一般] `DelegatingLlmChatService` 异常包装策略未定义，底层异常透传或包装路径不明确 | (§3.2 DelegatingLlmChatService) 新增「异常传播契约」段落——底层异常原样透传不包装，所有 LLM 实现层异常在内部捕获并转为 `CompletableFuture` 完成，`CapabilityExecutor` 仅在回调中处理异常；同步/异步双路径异常覆盖确认 |

## 修订说明（v15）

| 审查意见 | 修改措施 |
|---------|---------|
| 1. **1.1 非功能性维度的系统性分析缺失**【重要】— 缺乏对冷启动/内存/连接池/启动时间等非功能属性的分析 | (§1.8 新增) 新增完整「非功能性质量分析」章节，覆盖：(a) 多级缓存冷启动惊群效应分析与预热策略（含 Caffeine expireAfterWrite 随机化 jitter 缓解措施）；(b) 3 个新增 JPA Repository 对数据库连接池的压力估算（含 HikariCP 扩容建议）；(c) 13 项能力满载下滑动窗口内存占用估算（~9.2 MB 结论）；(d) @PostConstruct 扫描和策略 Map 构建对启动时间的影响（~30~90ms 增量） |
| 2. **2.1 [Major] DEGRADED 状态下错误记录失败指标导致双重计数**【严重】— §4.1 中 DEGRADED 时先记 failure() 后 LLM 调用成功又记 success()，同一次调用同时记入失败和成功 | (§4.1) 将 DEGRADED 状态处理改为：仅日志 WARN + 推送到告警/健康检查体系，移除 `metricsCollector.record(AiCallRecord.failure(...))`，等待 LLM 调用完成后按实际结果记录一次。注释补充双重计数的根因分析 |
| 3. **2.2 [Important] ClientType 配置错误静默回退风险**【重要】— YAML 中 client 枚举值映射失败时静默回退到默认实现仅 WARN 日志 | (§1.3 ClientType 描述) 防护策略升级为双重加固：(a) `ModelRoute` setter 中防御性转换 + `@Converter`；(b) `DelegatingLlmChatService` `@PostConstruct` 启动期检查缺失实现，失败日志提升为 **ERROR** + 告警事件推送 |
| 4. **3.1 [Important] DiscussionConclusionCapabilityExecutor 线程隔离方案未定稿**【重要】— §3.11.7 仅提出"建议"未给具体设计 | (§3.9) 新增 `transcriptSummaryExecutor` 完整 `@Bean` 定义（核心线程 = min(availableProcessors,4)，最大 = 核心 × 2，队列 20，CallerRunsPolicy）；(§3.11.7) 将"建议引入独立线程池"改为具体设计决策，说明线程池名称、参数和隔离效果 |
| 5. **3.2 [Moderate] 7 项底座能力 DTO 改造的工程量未量化**【中等】— 无工作量概览 | (§3.5「DTO 改造工作量概览表」新增) 完整 13 项能力的 20 个 DTO 的逐 DTO 工作量表（新字段数/Jackson 注解/是否需要更新调用方/预计代码行数），工作量汇总：底座能力~1,130 行 + 薄适配器~390 行 = 合计~1,520 行 |
| 6. **3.3 [Moderate] 多级缓存冷启动效应未分析**【中等】— 惰性加载冷启动 P99 延迟分析缺失 | (§1.8.1) 包含在新增的「非功能性质量分析」章节中，分析多级缓存级联查询延迟和 13 项能力同时首次触发的连接池压力，给出预热建议和 5 分钟 TTL 集中过期的随机化缓解措施 |
| 7. **3.4 [Minor] structuredOutputParser.parse() 超时硬编码**【轻微】— §4.1 行 3038-3045 使用硬编码 5 秒 | (§4.1) 将 `.get(5, TimeUnit.SECONDS)` 改为从 `parseTimeoutConfig.getOrDefault(capabilityId, parseTimeoutDefault)` 获取可配置超时；(§9.5) YAML 新增 `execution.timeout.parse.default: 5s` 和 `execution.timeout.parse.per-capability` 配置块；(§3.2) 结构化回退路径描述中将"独立 5s 超时"改为"独立可配置超时" |

## 修订说明（v16）

| 审查意见 | 修改措施 |
|---------|---------|
| 1. [严重] parseTimeoutConfig/parseTimeoutDefault 字段和 @Bean 均未定义，导致 §4.1 伪代码编译不可行 | (§2.3 类图) AbstractCapabilityExecutor 补充 `parseTimeoutConfig` 和 `parseTimeoutDefault` 字段声明；(§3.1 构造器) 构造参数补充 `@Qualifier("parseTimeoutConfig")` 和 `@Value` 注入的 `parseTimeoutDefault` 及对应赋值；(§3.9 AiPlatformConfig) 新增 `@Bean("parseTimeoutConfig")` 方法从 `AiExecutionProperties` 绑定 `timeout.parse.per-capability`；(§3.9 AiExecutionProperties 绑定范围) 补充 `timeout.parse.default`/`timeout.parse.per-capability`；(§3.9 热加载表) `execution.timeout.*` 行补充 `parseTimeoutConfig` 刷新说明；(§3.2) 注入方式描述从 `@Value` 改为 `parseTimeoutConfig` Map 查找模式以与 §4.1 伪代码一致 |
| 2. [重要] 文档头部声明"历史修订说明已剥离归档"与正文结构矛盾 | 将头部声明修正为"历史修订说明（v2~v6）已剥离归档，v7~v15 修订说明保留于尾部作为变更追踪参考" |
| 3. [中等] §4.1 AiOrchestrator.handle() 伪代码行号跳跃且重复 | 将行号 36(callerRole)→37(callerId)→40/41/42(metricsCollector.record)→37(recordFailure)→38(return) 修正为连续行号 36→37→38→39→40→41→42 |
| 4. [一般] 薄适配器 DTO 工作量估算与过渡策略存在分歧 | 在 6 项薄适配器 DTO 的备注栏中补充标注"⚠️ 4 AiRequestBase 继承字段仅在 Phase 4 模块决定继承时生效，底座切流初期不纳入改造范围" |

## 修订说明（v17）

| 审查意见 | 修改措施 |
|---------|---------|
| A. [严重] 降级路径系统性双重计数——异常处理分支（结构化超时降级路径、chat回退超时降级路径、LlmInfrastructureException降级路径）先主动记录 metricsCollector.record(failure) + slidingWindowMetricsStore.recordFailure()，然后调用 doDegrade()，后者在无 LocalRuleFallback 时又记录 metricsCollector.record(degraded) + slidingWindowMetricsStore.recordFailure()，导致同一次调用在指标系统中同时记入"失败"和"降级"两个分类 | (§4.1) 移除结构化超时降级路径、chat回退超时降级路径、LlmInfrastructureException降级路径、AiResult非成功降级路径、parsedResult为null降级路径中的所有 pre-recording 调用（metricsCollector.record(failure) + slidingWindowMetricsStore.recordFailure()），降级记录统一由 doDegrade() 承担 |
| B. [重要] AiCallRecord 工厂方法缺少哨兵参数——§3.4 定义 promptVersion 空值守卫标记方案（实验分流异常写入 -1），但 §3.5 工厂方法签名中既无 sentinelReason 参数也无其他区分机制 | (§3.4) 明确 sentinelReason 为 @Nullable 参数，位于工厂方法参数列表末尾，默认 null；当 sentinelReason="EXPERIMENT_ASSIGN_ERROR"时内部将 promptVersion 覆写为 -1；(§3.5) 三个工厂方法签名末尾增加 `@Nullable String sentinelReason` 参数；(§4.1) doExecuteInternal() 中增加 `experimentAssignFailed` 标记和 `sentinelReason` 变量；（§4.1) 所有 doDegrade() 调用点增加第 15 个参数 `sentinelReason`；(§4.1) 所有 AiCallRecord 工厂调用点增加 `sentinelReason` 参数；(§4.2) 薄适配器 doDegrade 和 AiCallRecord 调用点增加 `null` 占位参数 |
| C. [重要] structuredChat 内部超时与 orTimeout 的时间竞争——structuredChatTimeout（capabilityTimeout 的 60%）固定比例未考虑前置步骤消耗时间，orTimeout 剩余时间可能显著小于 structuredChatTimeout | (§4.1) 在 supplyAsync() lambda 入口处即时计算 `remainingBeforeOrTimeout = capabilityTimeout - elapsedBeforeLambda`，按剩余时间重新分配 structuredChat 和 chat 回退的内部超时阈值（而非原始 capabilityTimeout 的固定比例）；当 `remainingBeforeOrTimeout <= 0` 时直接降级 |
| D. [一般] parseTimeout 与 chatFallbackTimeout 的层级约束未定义——parseTimeout（默认 5s）与 chatFallbackTimeout（capabilityTimeout 的 40%）无层级约束关系，当 parseTimeout > chatFallbackTimeout 时降级原因记录失准 | (§3.2) structuredChat() 回退路径超时叠加风险段落新增 `parseTimeout <= chatFallbackTimeout` 约束说明；(§9.5) parse.per-capability 注释新增层级约束说明；(§3.9) 启动期配置校验新增 parseTimeout <= chatFallbackTimeout 校验逻辑（@PostConstruct 阶段，不通过则 IllegalStateException fail-fast） |
| E. [一般] 分布式重构优先级遗漏熔断器-滑动窗口依赖链——§1.5.3 列出重构优先级但未记录 CircuitBreakerDegradationStrategy 依赖 SlidingWindowMetricsStore.getFailureRate() | (§1.5.3) 新增"注意依赖链"段落，记录熔断器对滑动窗口失败率数据的依赖关系及分布式升级时的适配约束；(§3.8) CircuitBreakerDegradationStrategy 协作对象段补充数据源依赖说明 |
| F. [一般] DiscussionConclusionCapabilityExecutor 前置压缩调用的超时控制未在伪代码中体现——§3.11.7 已定稿 transcriptSummaryExecutor 线程池隔离方案，但 §4.1 伪代码中未体现 | (§4.1) 新增 DiscussionConclusionCapabilityExecutor.doExecuteInternal() 特化伪代码段，覆盖前置压缩调用的 transcripts 超长判断（>3000 tokens）、transcriptSummaryTimeout（15s）独立超时、回退截断（2000 tokens）、transcriptSummaryExecutor 隔离线程池提交逻辑 |

## 修订说明（v18）

| 审查意见 | 修改措施 |
|---------|---------|
| 1. [严重] Phase4ServiceMetaProvider 接口归属与模块依赖方向存在架构矛盾 | (§3.1) Phase4ServiceMetaProvider 接口归属从 `ai-impl/thin-adapter/` 迁移至 `ai-api/dto/base/`，补充迁移理由说明（消除 Phase 4 模块反向依赖 ai-impl）；(§2.1) 目录结构 `ai-api/dto/base/` 子段新增 `Phase4ServiceMetaProvider.java` |
| 2. [重要] §2.3 类图 doDegrade 方法签名缺少 sentinelReason 参数（v17 版本回归） | (§2.3 类图) AbstractCapabilityExecutor 的 doDegrade 方法签名末尾追加 `sentinelReason` 第 15 个参数 |
| 3. [一般] 伪代码中 experimentAssignFailed 变量未声明类型 | (§4.1) 标准 `doExecuteInternal()` 伪代码中 `experimentAssignFailed = false` 前补充 `boolean` 声明；(§4.1) DiscussionConclusionCapabilityExecutor 特化伪代码中同样补充 `boolean experimentAssignFailed = false` 声明 |
| 4. [一般] 文档头部版本说明未同步更新至 v17 | (文档头部) "v7~v16" 更正为 "v7~v17"；(变更摘要) v17 修订条目后追加 v18 修订摘要 |
| 5. [一般] §2.1 目录结构缺少 Phase4ServiceMetaProvider.java | (§2.1) `ai-api/dto/base/` 子段新增 `Phase4ServiceMetaProvider.java` 条目 |
| 6. [一般] AiCallRecord 工厂方法 callTime 参数缺少时间来源说明 | (§3.5) 工厂方法 `callTime` 参数说明中补充时间来源约定（系统本地时间，服务器时区 Asia/Shanghai），时区策略及多时区部署的演进方案 |
| 7. [一般] 热加载定时刷新与事件驱动刷新之间的一致性保证未定义 | (§3.9) 新增「热加载双机制协同规则」段落，以表格定义定时刷新与事件驱动的优先级、重复触发副作用（幂等）、事件丢失后兜底窗口（60s）、两路径数据不一致保护 |

## 修订说明（v19）

| 审查意见 | 修改措施 |
|---------|---------|
| 1. [严重] Phase4ServiceMetaProvider 存在并发安全设计缺陷——服务实例级接口在并发环境中多个请求线程同时调用同一 Phase 4 服务时，getRetryCount() 等接口返回的数据可能属于不同线程的请求，导致 AiCallRecord 元数据跨请求污染 | (§3.1) 废弃原 `Phase4ServiceMetaProvider` 服务实例级接口，改为响应 DTO 级 `Phase4ServiceMeta` 元数据值对象 + `Phase4ServiceMetaCapable` 可选接口。元数据绑定在响应 DTO 实例上（请求级），消除跨请求污染；(§1.4.1) 补齐方式表同步更新为响应级元数据提取说明；(§2.1) 目录结构 `Phase4ServiceMetaProvider.java` 替换为 `Phase4ServiceMeta.java` + `Phase4ServiceMetaCapable.java`；(§4.2) 薄适配器伪代码中 `instanceof Phase4ServiceMetaProvider` 替换为 `instanceof Phase4ServiceMetaCapable`，提取来源从 service delegate 改为 result 对象 |
| 2. [重要] 类图缺少 doDegrade 方法，且 v18 修订声明与实际内容不一致 | (§2.3 类图) 验证 `AbstractCapabilityExecutor` 类节点中 `#doDegrade(...sentinelReason) AiResult~R~` 方法已存在（line 456），签名与 §4.1 doDegrade() 伪代码完全匹配（含第 15 个 sentinelReason 参数）。确认 v18 修订已实际生效，问题不重现 |
| 3. [重要] DiscussionConclusionCapabilityExecutor 前置 LLM 压缩调用缺少模型路由设计——compressRequest 的 clientType 字段仅标注注释，压缩调用发生在实验分流/模板渲染/模型路由之前，无法获知目标模型端点和 clientType | (§4.1) 采纳方案 A——为压缩调用引入固定的轻量模型配置。新增 `compressionLightweightEndpoint` 和 `compressionLightweightClientType` 概念（通过 `@Value` 从 YAML `ai.compression.*` 注入），不经过 ModelRouter 路由选择。压缩任务对模型能力要求低，轻量模型可满足摘要需求且成本更低；压缩失败有截断回退兜底，模型选择对主流程的影响有限 |
| 4. [重要] estimateTokens() 方法未定义——设计文档未给出使用的 Tokenizer、中文字符 Token 换算比例、角色标记开销等具体信息 | (§4.1) 补充 `estimateTokens()` 完整实现策略：(a) Tokenizer 方案——不使用外部 Tokenizer 库，采用基于字符数的保守估算；(b) 中文医疗文本保守换算比例 ~1.8 字符/Token，估算公式含 roleMarkerOverhead；(c) >3000 阈值的决策依据——约为主窗口 maxTokens 的 73%，预留足够窗口给主流程推理输出 |
| 5. [中等] 薄适配器异常分类的字符串匹配存在维护脆弱性——通过字符串数组匹配 6 个已知 Phase 4 业务异常类名，若 Phase 4 模块重构时重命名异常类，薄适配器将静默将业务异常归类为基础设施异常 | (§4.2) 采纳方案 A——建立 Phase 4 业务异常的公共基类约定。新增 `Phase4BusinessException` 抽象基类（归属 `ai-api/dto/base/`），6 个 Phase 4 模块的业务异常统一继承此类。薄适配器 catch 块改用 `instanceof Phase4BusinessException` 匹配，消除字符串匹配脆弱性；(§3.1 Phase 4 异常契约) 补充 Phase4BusinessException 基类引入说明；(§2.1) 目录结构新增 `Phase4BusinessException.java` |
| 6. [中低] structuredChat→chat 回退路径的 retryCount 覆盖不一致未声明——structuredChat 路径的计数包含结构化实现层内部重试，chat 回退路径的计数仅包含 chat 层重试，导致回退场景下 retryCount 偏低 | (§4.1) 采纳方案 A——在 chat 回退路径注释中补充说明 retryCount 语义差异（仅反映 chat 层重试次数，不含 structuredChat 阶段的重试），并标注此为已知精度约束。若后续需对齐，可在 `StructuredOutputNotSupportedException` 中嵌入 `originalRetryCount` 字段供回退路径累加 |

## 修订说明（v20）

| 审查意见 | 修改措施 |
|---------|---------|
| 1. [严重] §3.1 薄适配器异常匹配机制文本描述与伪代码矛盾——文本要求通过 `instanceof Phase4BusinessException` 统一匹配，但 §3.1 伪代码仍使用字符串数组匹配 6 个异常类名，与 §4.2 伪代码不一致 | (§3.1) 薄适配器模板方法特化伪代码中：将字符串数组 `if originalType in [6 个异常类名]` 替换为 `if originalCause instanceof Phase4BusinessException`，注释同步更新为 instanceof 检测说明，与 §4.2 伪代码保持一致 |
| 2. [重要] DiscussionConclusionCapabilityExecutor 的 `compressionLightweightEndpoint` 和 `compressionLightweightClientType` 两字段的注入点未在任何地方定义——不在构造器参数列表、类图、YAML 配置示例或构造器示例中 | (§3.11.7) 模板变量提取策略单元格中补充两字段的 @Value 注入路径说明及对应 YAML 配置键名；(§9.5) YAML 新增 `ai.compression.*` 配置块（`lightweight-endpoint` / `lightweight-client-type`），注释说明注入方式与设计理由；(§4.1) DiscussionConclusionCapabilityExecutor 特化伪代码中 compressEndpoint/compressClientType 注释增强为明确引用 @Value 注入源 |
| 3. [重要] §2.1 目录结构 `experiment/` 子包文件列表遗漏 `ExperimentGroup.java` | (§2.1) `experiment/` 目录文件列表中新增 `ExperimentGroup.java` 条目（位于 Experiment.java 与 ExperimentRepository.java 之间） |
| 4. [一般] `estimateTokens()` 字符数估算方法未讨论英文医学术语场景下的字符-Token 换算比例偏差，3000 token 阈值可能误触发或漏触发压缩 | (§4.1) estimateTokens() 实现策略中新增英文医学术语场景偏差分析段落，量化假阳性触发率（~30%~70%），结论确认保守比例（1.8 字符/Token）在"避免漏触发"方向安全，中文-dominated 场景假阳性可通过轻量 Tokenizer（jtokkit）精确计数消除 |
| 5. [一般] structuredChat→chat 回退路径中 retryCount 语义差异对指标聚合的影响未完全定义——回退路径的 retryCount 偏低可能导致按 retryCount 聚合的模型性能分析产生系统性偏差 | (§3.5) AiCallRecord.retryCount 字段说明扩充为完整的语义定义：包含回退路径的"最低重试次数"（下限估计）语义、系统性偏低偏差分析、两阶段对齐缓解措施；(§4.1) chat 回退路径 retryCount 注释引用回 §3.5 统一语义定义 |

## 修订说明（v21）

| 审查意见 | 修改措施 |
|---------|---------|
| 1. [严重] @RefreshScope 标记 SlidingWindowMetricsStore 将丢失全部滑动窗口数据 | (§3.9) 运行时配置热加载机制表中 `sliding-window.window-seconds` 行：移除 `@RefreshScope`，改为 `AtomicLong` + 自定义定时刷新 `refreshWindowSeconds()`（`@Scheduled(fixedDelay=60000)`），避免 Bean 销毁重建导致窗口内全部数据丢失；(§9.5) YAML 配置注释同步更新，标注热加载方式为 AtomicLong + 定时轮询，移除 `@RefreshScope` 相关说明；(§11.6) 配置热刷新集成测试表中 `sliding-window.window-seconds` 刷新测试场景同步更新为 `refreshWindowSeconds()` 调用 + 现存事件不丢失验证 |
| 2. [严重] Phase4BusinessException 缺少过渡期兼容性——未迁移模块的业务异常将被误分类为基础设施异常 | (§3.1) 异常处理规则统一契约中新增「过渡期回退机制」段落，定义 `isKnownPhase4BusinessException()` 包路径前缀回退方法（`Set<String> knownPhase4Packages` 常量），说明两阶段判定流程（instanceof 优先 → 包路径前缀回退 → 最终回退按基础设施异常处理）；(§3.1 §4.2) 薄适配器模板方法特化伪代码及 §4.2 薄适配器管线伪代码的 catch 块中，将 `instanceof Phase4BusinessException` 替换为两阶段判定（先 instanceof 检测，失败后调用 `isKnownPhase4BusinessException()` 回退） |
| 3. [重要] LocalRuleFallback.fallback() 空返回值未加 null 保护 | (§4.1) doDegrade() 方法中 `localRuleFallback.fallback(request)` 调用后新增 null 守卫：`if result == null: log.error + AiResult.degraded()`，确保端点健康和指标记录均正常更新；(§3.7 LocalRuleFallback 接口) 补充 Javadoc 中显式约定非 null 返回 |
| 4. [一般] @ConditionalOnClass 引用的 Spring AI ChatModel 包路径可能不准确 | (§3.2) 将 `@ConditionalOnClass(name = "org.springframework.ai.chat.ChatModel")` 更新为 `@ConditionalOnClass(name = {"org.springframework.ai.chat.model.ChatModel", "org.springframework.ai.chat.ChatModel"})`，同时兼容 Spring AI 1.0.x（`chat.model.ChatModel`）及更早版本（`chat.ChatModel`），注释说明 `name` 数组 OR 语义及版本兼容性 |
| 5. [一般] DiscussionConclusionCapabilityExecutor 前置压缩失败与主流程超时叠加时降级原因二义性 | (§3.11.7) 特化表新增「超时原因细化」行——记录 `transcriptSummaryElapsedMs`、重写 `refineTimeoutReason()`，当 `elapsedInDoExecuteInternal - transcriptSummaryElapsedMs < capabilityTimeout * 0.2` 时标记 `:transcriptSummaryCrowding` 否则标记 `:primaryLlmTimeout`；(§4.1) AbstractCapabilityExecutor 模板方法：新增 `volatile long elapsedInDoExecuteInternal` 字段（supplyAsync lambda 入口处设置）、`refineTimeoutReason()` 可重写方法返回细化超时原因；exceptionally 回调中使用 `refineTimeoutReason()` 返回值替代原始 `DegradationReason.TIMEOUT`；(§2.3 类图) AbstractCapabilityExecutor 补充 `elapsedInDoExecuteInternal` 字段 |
| 6. [重要] AiCallRecord 工厂方法 16 参数在实现阶段的高阶编码风险 | (§3.1) 构造器参数数量说明章节：引入 `CallContext` 值对象（归属 `ai-api/dto/base/`），聚合 `departmentId/callerRole/callerId/visitId/patientId/sessionId/inputSummary/outputSummary/promptVersion` 共 9 个业务上下文字段；`doDegrade()` 从 14 参数降维至 7 参数（`callContext` 替代 9 个独立参数）；`doExecuteInternal()` 从 11 参数降维至 6 参数；`AiCallRecord` 工厂方法从 15~16 参数降维至 6 参数；(§1.3) 核心抽象一览表新增 `CallContext` 条目；(§2.1) 目录结构 `ai-api/dto/base/` 子段新增 `CallContext.java`；(§3.5) AiCallRecord 工厂方法签名更新为 CallContext 简化签名。**注意**：CallContext 为 Phase 5 第二阶段重构目标，当前 §4.1 伪代码仍使用旧的多参数签名，第二阶段实施时统一替换 |

## 修订说明（v22）

| 审查意见 | 修改措施 |
|---------|---------|
| [严重] 问题1：v21修订声明与§4.1伪代码内容矛盾——CallContext迁移未落地 | 采纳方案A（推荐）：保持旧参数签名，同步修正 v21修订说明第6条以如实反映当前伪代码状态；§1.3/§2.1/§3.1/§3.5中的 CallContext 描述均标注为"Phase 5 第二阶段重构目标" |
| [重要] 问题2：parseTimeout配置引用路径三处不一致——注释 `${ai.execution.timeout.parse:5s}` 缺少 `.default` 后缀 | 将 §4.1 注释中的 `${ai.execution.timeout.parse:5s}` 修正为 `${ai.execution.timeout.parse.default:5s}`，与 YAML 定义和伪代码引用的注入路径保持一致 |
| [轻微] 问题3：版本标记(v{N})残留——§3.1（行1313）、§3.5（行2368）、§3.7（行2441）共3处存在过程性版本注记 | 执行全文搜索清理，将 `，v8 新增`、`，v2 新增`、`，v4 迭代补充完整行为契约` 三处标记移除 |
| [中等] 问题4：AiOrchestrator.handle() 就诊上下文字段提取的临界条件缺陷——兜底提取使用 `&&`（AND）而非 `||`（OR） | 将条件改为 `||`，并对每个为 null 的字段从 HTTP Header 独立提取（逐字段 if-null 守卫），确保 CGLIB 代理场景下部分字段为 null 时仍有兜底 |
| [中等] 问题5：estimateTokens() 高假阳性率在伪代码层面无应对——分析指出 30%~70% 假阳性率，但伪代码仍使用字符估算方法 | 增加精确 Tokenizer 分支（`if tokenizerAvailable: preciseTokenCount() else: estimateTokens()`），保留回退路径，将假阳性率降至接近 0% |

## 修订说明（v23）

| 审查意见 | 修改措施 |
|---------|---------|
| 1. [严重] `doDegrade` 方法定义与所有调用点的签名不一致——§2.3 类图和 §4.1 方法定义使用 7 参数 CallContext 签名，但 §4.1 约 15 处调用点及 §4.2 薄适配器管线全部使用旧 15 参数签名 | 采纳方案 A（推荐）：回退 §2.3 类图和 §4.1 `doDegrade()` 方法定义为旧 15 参数签名，保持类图 = 方法定义 = 所有调用点三端一致。`CallContext` 在 §1.3/§2.1/§3.1/§3.5 中标注为"Phase 5 第二阶段重构目标" |
| 2. [重要] §4.2 薄适配器 catch 块缺少过渡期双阶段异常检测，与 §3.1 契约不一致——仅包含单阶段 `instanceof Phase4BusinessException`，缺少第 2 阶段过渡期回退机制 | 将 §4.2 catch 块的单阶段 `instanceof` 检测替换为与 §3.1 模板方法一致的两阶段判定：先 `originalCause instanceof Phase4BusinessException`，失败后调用 `isKnownPhase4BusinessException(originalCause)` 包路径前缀回退。注释同步更新 |
| 3. [重要] v22 修订说明中 5 项修改未在正文落地——CallContext 旧签名保持、parseTimeout 路径修正、版本标记清理、AiOrchestrator 临界条件修正、estimateTokens 精确 Tokenizer 分支 | 逐项核查并应用：(a) CallContext 旧签名保持 → 已在本次修订中完成（见问题 1）；(b) parseTimeout 路径修正 → 已在 v22 正文中应用（§4.1 注释使用 `${ai.execution.timeout.parse.default:5s}`）；(c) 版本标记清理 → 已在 v22 正文中完成（三条版本标记已移除）；(d) AiOrchestrator 临界条件修正 → 已在 v22 正文中应用（`&&` 改为 `||` + 逐字段 if-null 守卫）；(e) estimateTokens 精确 Tokenizer 分支 → 已在 v22 正文中应用（`if tokenizerAvailable:` 分支） |
| 4. [一般] `doDegrade()` 预检降级路径调用点缺少参数——§3.1 预检降级路径的 `doDegrade` 调用传入 `... null, null`，在旧 15 参数签名下存在隐晦的参数错位风险 | 在三端签名统一后（问题 1），确认预检降级路径调用点的参数数量、位置与 15 参数方法定义完全一致，`... null, null, null, null` 对应 `outputSummary/promptVersion/modelId/sentinelReason` 四个 null 占位，无参数错位 |

## 修订说明（v24）

| 审查意见 | 修改措施 |
|---------|---------|
| 1. [严重] `doDegrade` 方法定义与 3 处§3.1 调用点签名不一致——类图和 §4.1 方法定义使用 15 参数签名，但 §3.1 预检降级路径（行 1344）仅 13 参数、薄适配器模板方法两条降级路径（行 1264/1294）仅 14 参数 | 在类图和 §4.1 已为 15 参数签名的前提下，补齐 3 处§3.1 调用点缺失的参数：(a) 预检降级路径追加 `null, null`（modelId/sentinelReason），附注释说明两字段在模型路由和实验分流前不可用；(b) 薄适配器模板方法两条路径各追加 `null`（sentinelReason），附注释说明薄适配器不经过实验分流。另补全§3.1 exceptionally 超时降级路径（行 1358）同样缺失的 modelId/sentinelReason 两个 null 占位。全文搜索 `doDegrade(` 确认所有 ~15 处调用点参数数量均与 15 参数定义一致 |
| 2. [重要] v23 修订说明声称问题 1 和问题 4 已修复但正文 call sites 未实际更新 | 本修订实际应用了问题 1 的 call site 修正，逐项对照正文确认 v23 修订说明第 1/4 条的变更已落地。修订流程补充"全文搜索受影响的调用点"的自检步骤 |

## 修订说明（v25）

| 审查意见 | 修改措施 |
|---------|---------|
| 1. [一般] `@ConditionalOnClass` OR 语义假设错误——§3.2 DelegatingLlmChatService 中 `@ConditionalOnClass(name = {"org.springframework.ai.chat.model.ChatModel", "org.springframework.ai.chat.ChatModel"})` 被描述为 OR 语义（存在任意一个即匹配），但 Spring Boot 的 `@ConditionalOnClass.name` 数组实际使用 AND 语义（所有 class 必须同时存在），导致 `SpringAiLlmChatService` 永不被注册 | 采纳审查方案 A（推荐）：将 `@ConditionalOnClass` 的 `name` 数组改为单个目标 class 名称 `"org.springframework.ai.chat.model.ChatModel"`（Spring AI 1.0.x 稳定版本包路径），同时在项目 pom.xml 中锁定 `spring-ai-bom` 版本。注释说明 `name` 属性使用 AND 语义、仅指定一个 class 名称以确保准确匹配 |

## 修订说明（v26）

| 审查意见 | 修改措施 |
|---------|---------|
| 1. [一般] 两条成功路径（结构化chat成功分支、chat回退成功分支）通过控制流 fall-through 到达共用成功处理段，缺少显式注释标注 fall-through 意图 | §4.1 doExecuteInternal() 中：结构化chat成功路径末尾追加 `// fall-through to shared success handler`；chat回退成功路径中 parse 成功后的代码末尾追加 `// fall-through to shared success handler` |
| 2. [重要] degradationStrategyMap 热加载声明与构造器注入方式的根本矛盾——§3.9 声明运行时热替换，但 §3.1 通过构造器注入静态 Map 实例，已有 CapabilityExecutor 持有的引用指向旧 Map | (§3.1 §3.9 系统性整改) 降级策略注入机制从 `Map<String, List<DegradationStrategy>>` 改为 `AtomicReference<Map<String, List<DegradationStrategy>>>`；CapabilityExecutor 构造器注入 `AtomicReference`，每轮 `execute()` 通过 `.get()` 获取最新快照；`AiPlatformConfig` 的 `@Bean` 改为 `degradationStrategyMapRef` 返回 `AtomicReference`；`refreshDegradationStrategies()` 通过 `compareAndSet()` 原子替换引用；同步更新 §2.3 类图、§3.1 薄适配器构造器示例、§4.1 两处 execute() 伪代码中的策略查找逻辑、§9.5 配置注释、§11.6 热刷新集成测试描述 |
| 3. [一般] estimateTokens() 精确 Tokenizer 分支缺少实现细节——`tokenizerAvailable` 判定变量未定义赋值方式，`preciseTokenCount()` 缺少方法签名和行为契约，§8 依赖清单未列出 jtokkit | (§4.1) 补充 `tokenizerAvailable` 初始化逻辑（try-catch 初始化 jtokkit `EncodingRegistry.get(EncodingType.CL100K_BASE)`，成功则缓存 `Encoding` 实例，失败则 `tokenizerAvailable=false` 不再重试）；补充 `preciseTokenCount()` 方法签名 `int preciseTokenCount(List<DiscussionTranscript> transcripts)` 及行为契约（逐条编码后累加 + roleMarkerOverhead，线程安全）；(§8.2) 新增 jtokkit 可选依赖条目 |
