...
| `degradation.strategies` | `AiDegradationProperties` | 自定义定时刷新 + 策略列表热替换 | `AiPlatformConfig` 中定时重新构建 `degradationStrategyMap` Bean（通过 `refreshDegradationStrategies()` 方法），替换 `Map<String, List<DegradationStrategy>>` 实例后通过 `AtomicReference` 发布 |
...
DiscussionConclusionCapabilityExecutor.doExecuteInternal(startTime, request, capabilityId,
    ...
    if tokenizerAvailable:
        ...
        count = preciseTokenCount(transcripts)  // 逐条消息编码后 sum，不含公式估算偏差
...
| `degradation.strategies` 热替换 | 模拟 YAML 中 degradation.strategies 配置变更，调用 `refreshDegradationStrategies()` 方法重建 `degradationStrategyMap` | 重建后的 Map 中策略列表与配置一致，新旧 Map 引用不同（通过 `AtomicReference` 发布），已在运行的 `CapabilityExecutor` 实例在下次 `execute()` 调用时读取到新 Map |
