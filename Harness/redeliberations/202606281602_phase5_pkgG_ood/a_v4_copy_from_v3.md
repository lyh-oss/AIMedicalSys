# Phase 5 包 G — AI 进阶底座 架构级 OOD 设计方案（v4）
...
**依赖规则**：
- `ai-api` 接口签名冻结（变更范围严格限定为 §1.4 变更总结表中列出的 4 项），业务模块仅依赖 `ai-api`
...
**降级策略注入机制**（v4 新增，v5 细化装配路径，v6 统一约定，v8 明确推导规则，v4 迭代修正——移除 `@Qualifier` 伪代码，统一为 Map 方案）：每个 `CapabilityExecutor` 实现通过构造器注入 `Map<String, List<DegradationStrategy>> degradationStrategyMap`，在 `execute()` 中按 `getCapabilityId()` 从 Map 中查找对应的策略列表。此 Map 由 `AiPlatformConfig` 在 `@PostConstruct` 阶段构建并作为 `@Bean` 暴露，构建逻辑见下方 YAML→Bean 装配路径。
...
**条件注册保护**（v20 新增，v4 迭代修正——统一为 `@Component` 自注册 + `@ConditionalOnProperty` 条件化激活，删除"AiPlatformConfig 中统一注册"的表述）：全部 13 个 CapabilityExecutor 均通过 `@Component` 自注册到 Spring 容器，同时标注 `@ConditionalOnProperty(name = "ai.platform.enabled", havingValue = "true")`，确保 `ai.platform.enabled=false` 时底座关闭，所有 CapabilityExecutor 不被实例化。完整管线型按 `@PostConstruct` 扫描 `List<CapabilityExecutor>` 自动构建映射表（见 §3.1 映射机制），薄适配器型在底座关闭时避免因 Phase 4 业务服务未加载导致容器启动失败。
...
**departmentId 提取策略**（v8 新增，v11 修正提取时机，v4 迭代修正——删除"Prompt 模板回退"描述，薄适配器不含模板渲染步骤）：`departmentId` 的提取在 `AbstractCapabilityExecutor.execute()` 入口处（`supplyAsync()` 之前）调用 `doExtractDepartmentId(request)` 完成，此时所在线程为 Tomcat 容器线程，Spring `RequestContextHolder` 上下文可用。Phase 4 薄适配器型 CapabilityExecutor 的 `request` DTO 尚未继承 `AiRequestBase` 基类，无 `getDepartmentId()` 方法，故重写 `doExtractDepartmentId()` 从 `RequestContextHolder` 或 HTTP `X-Department-ID` Header 中获取科室标识。若无法获取则 departmentId 保持 null，对应字段在 `AiCallRecord` 中可空。Phase 5 底座能力的 DTO 已继承 `AiRequestBase`，`doExtractDepartmentId()` 默认实现直接从 `request.getDepartmentId()` 返回，不依赖 RequestContextHolder。
...
extractCallerRole():
    ...
    // 实现自定义拼接逻辑。
    // ROLE_ 前缀过滤规则（v4 新增）：优先遍历 authority 列表查找第一个以 "ROLE_" 开头
...
- **超时层级关系**（v23 明确）：完整管线场景下 `capabilityTimeout`（通过 `orTimeout()` 设置）与薄适配器场景下 `thinAdapterTimeout`（通过 `delegateFuture.get()` 设置）同时存在时，两超时值的关系定义如下：
  - **薄适配器场景**：`capabilityTimeout` 应设置为 `thinAdapterTimeout` + 合理缓冲值（如 +5 秒），确保内部 `delegateFuture.get(thinAdapterTimeout)` 优先触发并进入已定义的降级路径，外层 `orTimeout` 仅作为兜底保护。§9.5 配置中薄适配器能力的 `per-capability` 值（35 秒）比 `thin-adapter-default: 30s` 多 5 秒缓冲，遵循此层级约束。`per-capability` 必须 >= `thinAdapterTimeout`，推荐保持 +5 秒缓冲，防止两超时同时触发导致同一请求被降级两次
...
**structuredChat() 回退路径超时叠加风险**（v3 新增）：
...
- `topP`（Double，可空）：核采样参数，控制候选 token 的累积概率阈值（对应 OpenAI `top_p`），可空时由实现层使用默认值（v4 迭代新增）
- `frequencyPenalty`（Double，可空）：频率惩罚系数，减少已出现 token 的重复生成（对应 OpenAI `frequency_penalty`），可空时由实现层使用默认值（v4 迭代新增）
- `presencePenalty`（Double，可空）：存在惩罚系数，鼓励讨论新主题（对应 OpenAI `presence_penalty`），可空时由实现层使用默认值（v4 迭代新增）

**参数映射扩展点规则**（v4 迭代新增）：`LlmChatOptions` 与 `ModelRoute.parameters` 之间的 key 映射采用**白名单显式映射**模式。当前白名单包含 `temperature`、`maxTokens`、`stopSequences`、`topP`、`frequencyPenalty`、`presencePenalty` 六个 key。不在白名单中的 `parameters` key 不进入 `LlmChatOptions` 的强类型字段——若需新增参数，需同步在 `LlmChatOptions` 类中新增强类型字段并更新阶段一映射逻辑。此设计避免了 `Map<String, Object>` 的弱类型风险，但要求每新增一个 LLM 参数支持都需要修改 `LlmChatOptions` 类。若后续出现供应商特有参数（如阿里云的 `enable_search`、OpenAI 的 `logprobs`），可考虑在 `LlmChatOptions` 中新增 `Map<String, Object> extensionParameters` 兜底字段接收白名单外参数，由 `LlmChatService` 实现层按需读取。
...
#### `ExperimentGroup` — 实验分组 JPA Entity（v4 迭代新增）
...
**现有 DTO 影响评估与向后兼容策略**（v4 新增，v24 补充 Jackson 兼容性评估）：
- **现状**：当前代码库中 13 项 AI 能力的请求 DTO（`TriageRequest`、`DiagnosisRequest` 等）各自独立定义 `visitId`/`patientId`/`sessionId` 字段，未继承任何公共基类。`AiRequestBase` 基类当前**不存在于代码库中**，管线伪代码中 `request.getDepartmentId()`/`getVisitId()`/`getPatientId()`/`getSessionId()` 的调用隐含了对该基类存在的假设
- **迁移代价**：引入 `AiRequestBase` 需要完成以下操作：
   ...
   2. 将 13 个请求 DTO 的公共字段声明改为 `extends AiRequestBase`，删除重复字段
   3. 检查各 DTO 是否存在字段名/类型不兼容（如某 DTO 的 `visitId` 类型为 `Long` 而非 `String`），若发现则统一为 `String`
   4. 确保 Jackson 序列化/反序列化兼容——在 `AiRequestBase` 基类上标注 `@JsonIgnoreProperties(ignoreUnknown = true)` 和 `@JsonTypeInfo(use = Id.CLASS, include = As.PROPERTY, property = "@class")`（若涉及多态），同时在各个子类上验证 `@JsonCreator` + `@ConstructorProperties` 或 Lombok `@Jacksonized` 注解的兼容性；具体而言，引入基类后需验证 3 个序列化场景：(a) 旧 DTO JSON（无基类字段）反序列化为新 DTO（继承基类）—— `ignoreUnknown = true` 使 JSON 中不存在的字段安全忽略；(b) 新 DTO JSON（含基类字段）反序列化为旧 DTO——基类字段自动被 `ignoreUnknown = true` 忽略，业务模块无需修改；(c) 新 DTO 之间互相序列化/反序列化——基类公共字段正常映射
...
**AiCallLogStats JPA Entity 定义**（v4 迭代新增）：
...
- **策略自动注册抑制**（v4 修订，v6 同步命名约定，v4 迭代修正——移除 `@Qualifier` 伪代码，统一为 Map 方案）：新增的 `TimeoutDegradationStrategy` 和 `CircuitBreakerDegradationStrategy` 作为 `@Component` 不会自动注入 `FallbackAiService` 的策略列表（降级判定已移至 `CapabilityExecutor` 管线内部），而是由 `AiPlatformConfig` 按能力配置的策略白名单驱动，通过注入 `Map<String, List<DegradationStrategy>>` 到各 `CapabilityExecutor` 构造器实现。各能力的策略列表在 `AiPlatformConfig` 中按能力标识配置：
...
doExecuteInternal(startTime, request, capabilityId, departmentId, userId, sessionId, callerRole, callerId, visitId, patientId, inputSummary):
    ...
    try:
      ...
      aiResult = llmChatService.structuredChat(chatRequest, outputType).join()
      ...
      // chat() 返回 CompletableFuture<AiResult<LlmChatResponse>>，同样需两层解包
      chatAiResult = llmChatService.chat(chatRequest).join()
...
| LocalRuleFallback 泛型类型安全（v24 新增） | 不记录风险 vs 在 §7 显式记录 unchecked 转换风险并增加防御措施 | 在 §7 记录风险 + 在 `AbstractCapabilityExecutor` 中增加 `Class<T>` 显式类型信息 + 在 `doDegrade()` 中增加类型检查日志 | `doDegrade()` 中 `localRuleFallback.fallback(request)` 调用在编译期为 unchecked 转换（`T` → `R`），因泛型擦除在 Phase 4/Phase 5 DTO 混合场景下可能抛出 `ClassCastException`。防御措施：(1) `AbstractCapabilityExecutor` 构造器注入 `Class<T> inputType` 字段，在 `doDegrade()` 中增加 `inputType.isInstance(request)` 类型检查日志；(2) 类型不匹配时日志 ERROR + 返回 `AiResult.degraded()` 而非传播 `ClassCastException`
...
### 9.2 FallbackAiService 迁移路径（v6 新增，v24 修订系统性偏差）
...
**配置绑定方式**（v3 新增）：`FallbackAiService` 中 `aiPlatformEnabled` 通过 `@Value("${ai.platform.enabled:false}")` 构造器注入，而非字段级 `@Value` 或从 `AiPlatformConfig` 间接引用。理由如下：
...
**`ai.platform.enabled` 与 `ai.mock.enabled` 同时激活的风险**（v3 标注）：若 `AiPlatformEnvironmentPostProcessor` 的转发被绕过（如用户显式在 YAML 中配置了 `ai.mock.enabled=true` 同时 `ai.platform.enabled=true`），则 `AiOrchestrator`（底座激活）和 `MockAiService`（Mock 激活）将同时注册为 `AiService` 的非装饰器实现，导致 `AiOrchestrator` 的 `@Bean` 直接注入 `AiService` 依赖时出现 `NoUniqueBeanDefinitionException`。缓解措施：
  ...
  execution:
    timeout:
      ...
      per-capability:
        ...
        # ⚠️ 以下薄适配器能力的 per-capability 超时值必须 >= thin-adapter-default（当前 30s）且保持合理缓冲
        # 层级约束：per-capability = thinAdapterTimeout + 缓冲值（建议 +5s）
        # 理由：外层 orTimeout 作为兜底保护，内部 thinAdapterTimeout（delegateFuture.get()）优先触发降级路径
        # 若两值相等（如均为 30s），同一请求可能同时触发两超时，导致同一降级原因被误记两次
        DIAGNOSIS: 35s                  # 薄适配器；thinAdapterTimeout=30s，per-capability=35s（+5s 缓冲）
        ANALYSIS_REPORT_INSPECTION: 35s # 薄适配器
        ANALYSIS_REPORT_LABTEST: 35s    # 薄适配器
        IMAGE_ANALYSIS: 35s             # 薄适配器
        RECOMMEND_EXAM: 35s             # 薄适配器
        RECOMMEND_EXEC_ORDER: 35s       # 薄适配器
      default: 60s
...
| 输入 DTO | `TriageRequest`（继承 `AiRequestBase`），扩展字段：`complaint: String`（主诉）、`symptomDescription: String`（症状描述）、`medicalHistory: String`（既往史，可选）、`vitalSigns: String`（生命体征，可选） |
| 输出 DTO | `TriageResponse`，字段：`recommendedDepartment: String`（推荐科室）、`urgencyLevel: UrgencyLevel`（紧急程度枚举：EMERGENCY/URGENT/ROUTINE）、`reason: String`（推荐理由） |
...
| 输入 DTO | `PrescriptionCheckRequest`（继承 `AiRequestBase`），扩展字段：`medications: List<Medication>`（处方药品列表）、`patientInfo: PatientInfo`（患者信息，含年龄/体重/过敏史/孕产状态）、`diagnosisCodes: List<String>`（诊断编码列表） |
| 输出 DTO | `PrescriptionCheckResponse`，字段：`overallStatus: CheckStatus`（整体状态枚举：PASS/REVIEW_REQUIRED/REJECTED）、`checks: List<CheckResult>`（逐项检查结果列表）、`summary: String`（审核意见摘要） |
...
| 输入 DTO | `MedicalRecordGenRequest`（继承 `AiRequestBase`），扩展字段：`chiefComplaint: String`（主诉）、`presentIllness: String`（现病史）、`pastHistory: String`（既往史）、`physicalExamination: String`（体格检查）、`auxiliaryExaminations: List<String>`（辅助检查列表，可选）、`diagnosis: String`（初步诊断，可选） |
| 输出 DTO | `MedicalRecordGenResponse`，字段：`recordContent: String`（病历正文）、`structuredSections: List<RecordSection>`（结构化段落列表）、`generatedAt: LocalDateTime`（生成时间）。`RecordSection` 内嵌字段：`sectionName: String`（段落名称）、`content: String`（段落内容）、`sectionType: SectionType`（段落类型枚举：CHIEF_COMPLAINT/PRESENT_ILLNESS/PAST_HISTORY/PHYSICAL_EXAM/AUXILIARY_EXAM/DIAGNOSIS） |
...
#### 3.11.5 `PrescriptionAssistCapabilityExecutor` — AI 辅助开方（继承能力）
...
| 输入 DTO | `PrescriptionAssistRequest`（继承 `AiRequestBase`），扩展字段：`diagnosisCode: String`（诊断编码，如 ICD-10）、`diagnosisName: String`（诊断名称）、`patientAge: int`、`patientWeight: double`（可选，用于剂量计算）、`allergyInfo: List<String>`（过敏史）、`existingMedications: List<Medication>`（当前用药列表，可选） |
| 输出 DTO | `PrescriptionAssistResponse`，字段：`recommendations: List<MedicationRecommendation>`（推荐药品列表）。`MedicationRecommendation` 内嵌字段：`drugCode: String`、`drugName: String`、`dosage: String`（如 "250mg"）、`frequency: String`（如 "tid"）、`route: String`（如 "po"）、`duration: String`（如 "7d"）、`rationale: String`（推荐理由）、`confidenceLevel: ConfidenceLevel`（枚举：HIGH/MEDIUM/LOW） |
...
#### 3.11.6 `KbQueryCapabilityExecutor` — AI 知识库问答（首次落地能力）

| 设计维度 | 说明 |
|---------|------|
| 职责 | 基于患者症状/诊断信息，从医疗知识库中检索匹配的知识条目并生成自然语言回答 |
| 输入 DTO | `KbQueryRequest`（继承 `AiRequestBase`），扩展字段：`queryText: String`（查询文本）、`queryScope: KbQueryScope`（查询范围枚举：DRUG/DIAGNOSIS/TREATMENT/GUIDELINE，默认 ALL）、`departmentId: String`（科室过滤，可选）、`maxResults: int`（返回条目上限，默认 5） |
| 输出 DTO | `KbQueryResponse`，字段：`answer: String`（综合回答文本）、`sources: List<KbSource>`（知识来源列表），`KbSource` 内嵌字段：`title: String`、`summary: String`、`relevanceScore: double`、`sourceUrl: String`（可选） |
...
#### 3.11.7 `ScheduleCapabilityExecutor` — AI 医生排班（首次落地能力）

| 设计维度 | 说明 |
|---------|------|
| 职责 | 根据科室排班规则、医生可用性和工作量约束，生成或推荐排班方案 |
| 输入 DTO | `ScheduleRequest`（继承 `AiRequestBase`），扩展字段：`departmentId: String`（排班科室）、`scheduleDate: LocalDate`（排班日期）、`shiftType: ShiftType`（班次类型枚举：MORNING/AFTERNOON/NIGHT/ALL_DAY，可选）、`requiredDoctors: int`（所需医生人数，可选）、`preferredDoctors: List<String>`（优先排班的医生 ID 列表，可选）、`constraints: List<ScheduleConstraint>`（排班约束，如"某医生当天不可排班"） |
| 输出 DTO | `ScheduleResponse`，字段：`schedule: List<ShiftAssignment>`（排班结果列表）。`ShiftAssignment` 内嵌字段：`doctorId: String`、`doctorName: String`、`shiftType: ShiftType`、`departmentId: String`、`date: LocalDate` |
...
#### 3.11.8 `DiscussionConclusionCapabilityExecutor` — AI 综合讨论结论（首次落地能力）
...
| 模板变量提取策略 | 方式 B（自定义，v4 迭代补充前置 LLM 调用契约）：从 `DiscussionConclusionRequest` 提取 `discussionId`、`discussionType`；`transcripts` 列表先按角色分组统计 `speakerCount`；若 `transcripts` 原始文本超过 3000 tokens，使用首轮 LLM 调用压缩为摘要后注入 `transcriptSummary` 变量（避免模板层级 Token 超限）。**前置 LLM 调用契约**：使用 `LlmChatService.chat()` 方法（非 structuredChat）发送压缩摘要请求，不经过实验分流和模板渲染（直接使用硬编码压缩 Prompt "请将以下讨论记录压缩为 500 字以内的摘要，保留关键诊断意见和分歧点"）；设置独立超时 `transcriptSummaryTimeout`（默认 15 秒，通过 `@Value("${ai.execution.timeout.transcript-summary:15s}")` 注入），与主流程 `capabilityTimeout` 独立；若超时或调用失败，回退到截断 `transcripts` 原始文本至 2000 tokens 作为 `transcriptSummary`（确保主流程 LLM 调用仍可执行），日志 WARN 记录失败原因 |
...
Phase 5 底座对 `ai-api` 模块的变更严格限定为以下范围，非列表内的所有接口和类型**保持签名冻结**：

| 变更类型 | 目标类型（所属 `ai-api/degradation/`） | 变更说明 | 兼容性 |
|---------|--------------------------------------|---------|--------|
| 接口新增 default method | `DegradationStrategy` | 新增 `default int getOrder() { return 0; }` | 二进制兼容，现有实现无需修改 |
| 值对象字段扩展 | `DegradationContext` | 新增 `invocationCount`/`lastFailureTime`/`elapsedTime`/`requestType`/`failureCount`/`departmentId` 字段 + `serializedTimestamp` + `postDeserializationValidate()`/`isFresh()`/`isInitialized()`；声明 `implements Serializable` | 二进制兼容，保留无参构造器，新增字段取语言默认值 |
| 新增 enum | `DegradationReason` | 新增降级原因/错误码枚举（NO_AVAILABLE_ROUTE / ENDPOINT_UNAVAILABLE / CIRCUIT_BREAKER_OPEN / PARSE_FAILURE / TIMEOUT / STRATEGY_TRIGGERED / INTERNAL_ERROR / INFRASTRUCTURE_ERROR） | 纯新增，无兼容影响 |
| 新增抽象基类 | `AiRequestBase`（新增于 `ai-api/dto/base/`） | 新增 abstract class，封装 visitId/patientId/sessionId/departmentId 跨能力通用字段 | 纯新增，不影响现有 DTO 类型声明 |

**"不变"约束声明**：除上表列出的 4 项变更外，`ai-api` 模块中所有接口（`AiService`、`AiResult`）及现有 DTO 类型（`TriageRequest`、`DiagnosisRequest` 等）**保持方法签名、字段定义、构造器签名的完全冻结**。业务模块对 `ai-api` 的 Maven 依赖无需修改，编译期不受底座模块变更影响。
...
| 审查意见 | 修改措施 |
|---------|---------|
| [严重] reactor-core 依赖性质存在事实矛盾 — §3.2 声称流式接口独立于非流式、reactor-core 为可选依赖，但 LlmChatStreamService 接口方法签名直接引用 `Flux<LlmChatResponse>`，编译器层面要求 reactor-core 必须在编译期 classpath 上；§8.2 中 `<optional>true</optional>` 被注释掉（实际为强制依赖） | 采用方案(b)：如实承认 reactor-core 为编译期强制依赖，删除"隔离 reactor-core"的宣称。具体修改：(1) §1.3 LlmChatStreamService 描述删除"隔离 Reactor 依赖"字眼，明确标注方法签名直接引用 Flux；(2) §3.2 LlmChatStreamService 职责描述和"为何独立于 LlmChatService"段落删除"可选依赖"字眼，新增 ⚠️ 说明块；(3) §8.2 reactor-core 依赖注释修正为"编译期强制依赖"，取消 `&lt;optional&gt;true&lt;/optional&gt;` 的注释屏蔽并增加说明 |
| [严重] 薄适配器超时配置示例违反自身层级约束 — §3.1 要求薄适配器场景 `capabilityTimeout` 应设为 `thinAdapterTimeout` + 缓冲值（如 +5s），但 §9.5 YAML 配置示例中薄适配器能力的 `per-capability` 值均设为 `30s`，与 `thin-adapter-default: 30s` 相等，无任何缓冲 | §9.5 中 6 项薄适配器能力的 `per-capability` 超时值从 `30s` 修正为 `35s`（30s + 5s 缓冲）；YAML 注释中显式标注层级约束（per-capability = thinAdapterTimeout + 缓冲值）；§3.1 超时层级关系文本同步修正引用值 |
| [重要] LocalRuleFallback 泛型方法 vs 实际管线调用类型不一致 — §2.3 类图将 `LocalRuleFallback<T, R>` 的 `fallback()` 签名定义为 `fallback(T request) AiResult<R>`，但 §4.1 调用时传入的 `request` 由于泛型擦除存在 unchecked 转换，可能在 Phase 4/Phase 5 DTO 混合场景下抛出 `ClassCastException` | (1) §2.3 AbstractCapabilityExecutor 类图新增 `#Class~T~ inputType` 字段；(2) §4.1 doDegrade() 方法注释块新增 ⚠️ 类型安全说明；(3) §4.1 doDegrade() 方法体中在 `localRuleFallback.fallback(request)` 之前插入 `inputType.isInstance(request)` 检查，类型不匹配时日志 ERROR + 返回 AiResult.degraded()；(4) §7 设计决策表新增 "LocalRuleFallback 泛型类型安全" 行 |
| [重要] SlidingWindowMetricsStore 惰性淘汰写/读并发竞争条件未定义 — §3.5 定义的惰性淘汰策略——写方法和读方法均在入口处执行过期事件移除，但两线程的淘汰操作可能针对同一 Deque 产生竞争。文档仅声明"写锁保护队列尾写入"，但惰性淘汰操作涉及队列头和队列尾同时操作，锁范围不足 | §3.5 线程安全段落重写为完整锁协议：(1) 惰性淘汰与写入统一纳入同一 `synchronized (deque)` 块；(2) 读取路径惰性淘汰同样在 `synchronized` 块内完成淘汰 + 快照复制；(3) 写-读在 Deque 级别互斥以优先保证数据一致性；(4) 标明 `ReentrantReadWriteLock` 作为性能瓶颈时的演进路径 |
| [重要] ModelEndpointHealthManager 的 DEGRADED 状态行为空白 — §3.2 状态模型定义 `DEGRADED` 状态语义为"仍然尝试调用，但上报告警"，但 §4.1 `doExecuteInternal()` 伪代码中仅处理了 `UNAVAILABLE` 状态分支，`DEGRADED` 状态直接放行无告警日志也无性能指标记录。同时，LLM 调用后未调用 `recordCallResult()` 更新端点健康状态，导致 DEGRADED→CONNECTED 的恢复路径无法触发 | (1) §4.1 doExecuteInternal() 中新增 `endpointState == DEGRADED` 分支：WARN 日志 + metricsCollector 记录 ENDPOINT_DEGRADED 指标；(2) 在 structuredChat 成功路径和 chat() 回退成功路径中补充 `endpointHealthManager.recordCallResult(endpointId, true, ...)`；(3) 在 LlmInfrastructureException catch 块中补充 `endpointHealthManager.recordCallResult(endpointId, false, ...)` |
| [重要] RequestContextUtils.extractFromRequestContext() 存在 ClassCastException 风险 — §3.10 `extractFromRequestContext()` 实现直接进行 `(ServletRequestAttributes) RequestContextHolder.getRequestAttributes()` 的强转，当底座运行在非 Spring MVC 环境（如 WebFlux）下时抛出 `ClassCastException` | §3.10 核心方法伪代码使用 `instanceof ServletRequestAttributes` 安全检查替代直接强转；非 Servlet 环境下返回 null，与调用方 doExtractXxx() 的 null 回退路径无缝衔接 |
...
| 审查意见 | 修改措施 |
|---------|---------|
| [严重] doExecuteInternal() 伪代码中 LlmChatService 方法调用的返回类型错误——structuredChat() 和 chat() 返回 CompletableFuture<AiResult<X>>，但 §4.1 伪代码直接将返回值作为裸对象使用，缺少 CompletableFuture.join() 和 AiResult.getData() 两层解包 | §4.1 doExecuteInternal() 伪代码中 structuredChat() 调用改为 `.join().getData()` 两层解包获取 StructuredChatResult<T>，再从其调用 `.getData()` 获取解析后 DTO；chat() 调用改为 `.join().getData()` 获取 LlmChatResponse。同步更新所有引用变量（retryCount/tokenUsage/parsedResult）的赋值路径 |
| [严重] LlmChatResponse 仅定义了 content、usage、modelId，但伪代码调用了 chatResponse.getRetryCount()，缺少 retryCount: int 字段 | §1.3 核心抽象表补充 LlmChatResponse 的 retryCount 描述；§2.3 类图 LlmChatResponse 新增 +int retryCount 字段；§3.2 LlmChatResponse 字段级契约新增 retryCount 字段定义，说明在 chat() 返回路径中由实现层填充（structuredChat() 路径的 retryCount 由 StructuredChatResult.getRetryCount() 提供） |
| [重要] 6 项 Phase 4 薄适配器能力的 thinAdapterTimeout 统一 30s，无差异化依据、无按能力覆盖机制、无配置指导 | §3.1 AbstractCapabilityExecutor 整体管线超时机制段补充按能力差异化 thinAdapterTimeout 说明——新增 YAML 配置 `ai.execution.timeout.thin-adapter.per-capability`，各薄适配器能力按 Phase 4 服务 P99 独立配置超时值；§9.5 YAML 配置示例中新增 thin-adapter.per-capability 块，列出 6 项能力的差异化超时值及选择依据 |
| [重要] Phase 4 业务异常以裸字符串 e.getMessage() 通过 AiResult.failure() 传递，缺乏标准化错误码和异常分类体系 | §4.1/§4.2 薄适配器 BusinessException catch 块新增标准化错误码提取逻辑：优先使用 BusinessException.getErrorCode()，回退到 `"PHASE4_" + e.getClass().getSimpleName()`；同步更新 AiCallRecord.failure() 调用参数传入 errorCode；AiResult.failure() 调用改为双参数签名 `failure(errorCode, message)` |
| [中等] CredentialProvider Vault 降级状态机缺少外部调用方可调用的凭据有效性反馈方法，恢复速度仅由定时器驱动 | §3.2 CredentialProvider 段新增"凭据有效性反馈接口"子段落：定义 getState()/isCredentialValid()/probeVault() 三个方法及 CredentialProviderState 枚举；probeVault() 与 ModelEndpointHealthManager.tryProbe() 一致的设计模式，使 CapabilityExecutor 可主动触发探测恢复，不依赖单向定时器驱动 |
| [严重] DegradationStrategy 接口实际代码中不存在 getOrder() 方法，设计 §4.1 伪代码策略链排序逻辑引用不存在的 API；且未验证新增 default int getOrder() 对现有实现（NoOpDegradationStrategy 等）的二进制兼容性及对降级原因字符串稳定性的影响 | §3.8 ModelEndpointHealthManager 交互规则段新增 `getOrder()` 方法说明，明确其在当前代码中不存在、需作为 Phase 5 变更新增 default method；§9.4 补充第 4 步"降级原因字符串稳定性验证"——编写测试验证各策略 getSimpleName() 不受接口变更影响；发版说明标注变更为"二进制兼容" |
| [重要] FallbackAiService 现有实现与设计描述的迁移起点存在系统性偏差（构造器注入方式 ObjectProvider\<AiService\> vs List\<AiService\>、委托选择逻辑、@Primary 标注、applyStrategies() 调用方式、降级策略管理、上下文构造）；过渡期内底座切流时存在双降级判定风险，且构造器签名不匹配导致编译失败 | §9.2 重写为分阶段迁移路径——阶段一：构造器迁移（List→ObjectProvider、@Primary 标注、过渡期双降级风险缓解）；阶段二：applyStrategies() 迁移（@Deprecated 标注、if(aiPlatformEnabled) 条件开关、降级策略列表引用清理、DegradationContext 构造迁移）；§9.3 新增两阶段迁移的解耦说明，强调先完成构造器迁移再实施 applyStrategies() 剥离 |
| [重要] AiRequestBase 基类不存在但管线伪代码已假设其存在；未评估创建一个抽象基类后对 13 个现有 DTO 的 Jackson 序列化兼容性及业务模块的影响范围 | §3.5 AiRequestBase 段重写"现有 DTO 影响评估"：明确基类当前不存在；补充 3 个 Jackson 序列化场景的兼容性分析（旧→新/新→旧/新→新）；新增"业务模块影响范围"子段落，逐条分析 4 类受影响场景及对应处理方案；新增"Jackson 兼容性测试要求"——为 7 项底座能力 DTO 各添加反序列化单元测试 |
| [中等] 4 项 Phase 5 首次落地底座能力（KbQueryCapabilityExecutor、PrescriptionAssistCapabilityExecutor、ScheduleCapabilityExecutor、DiscussionConclusionCapabilityExecutor）缺少能力级别的特化设计——输入/输出 DTO 字段定义、Prompt 模板结构、结构化解析目标类型、模板变量提取策略、YAML 模型路由配置等均未给出 | §3.11 新增"Phase 5 首次落地底座能力特化设计"专节，为 4 项能力各提供一个 9 维度特化设计表（职责/输入 DTO/输出 DTO/Prompt 模板结构/结构化解析目标类型/模板变量提取策略/YAML 路由配置/降级策略），附共同约束说明段落 |
...
| 审查意见 | 修改措施 |
|---------|---------|
| **[严重] 问题1：** "不变"声明与实质性变更之间存在系统性矛盾 — §1.2、§2.1、§2.2 三处声明 `ai-api` 保持不变，但实际包含 4 项实质性变更：`DegradationStrategy.getOrder()` default method、`DegradationContext` 新增字段、`AiRequestBase` 新抽象类、`DegradationReason` 新枚举 | 全局删除"不变"绝对化表述，替换为精确约束声明：(1) §1.2 架构图改为"接口签名冻结，新增 default method & 字段扩展见变更范围表"；(2) §2.1 目录结构逐项标注精确状态（"接口签名冻结"/"新增 default method"/"扩展字段"/"新增"）；(3) §2.2 依赖规则改为"接口签名冻结（变更范围严格限定为 §1.4 变更总结表）"；(4) 新增 §1.4 "ai-api 变更范围总结表"，列出 4 项变更的类型、目标和兼容性说明，约束非列表内接口保持签名冻结 |
| **[严重] 问题2：** 多实例部署场景下 `SlidingWindowMetricsStore`、`EndpointRateLimiter`、`CircuitBreakerDegradationStrategy` 三个组件的跨实例行为未定义 | 新增 §1.5 "多实例行为约束与部署形态"：(1) §1.5.1 明确部署形态前提为单体；(2) §1.5.2 逐组件分析跨实例行为差异表（Per-instance 限流/熔断/滑动窗口的约束和分布式重构建议）；(3) §1.5.3 迁移路径优先级排序（CircuitBreakerDegradationStrategy 最高 → EndpointRateLimiter → SlidingWindowMetricsStore） |
| **[重要] 问题3：** 伪代码引用约 10 个关键接口调用，未标注哪些已有实现、哪些需新建 | 新增 §1.6 "API Surface 状态表"：(1) §1.6.1 ai-api 模块逐项标注三种状态（✓ 已有实现 / △ 已有骨架 / ⊕ 需新建）；(2) §1.6.2 ai-impl 模块按 P0~P3 优先级排序，标注依赖前提；(3) 明确优先级判定规则 |
| **[重要] 问题4：** §3.5 提出的 Jackson 兼容性测试要求自相矛盾——旧 JSON 不包含基类字段则字段当然为 null，此测试无法验证真实兼容性风险 | (1) 将原来单一"旧 JSON→新 DTO"测试替换为 3 个真实兼容性验证场景（旧 JSON→新 DTO 向后兼容 / 新 JSON→旧消费者向前兼容 / 新 JSON→新 DTO 含基类继承注解验证）；(2) 在 AiRequestBase 兼容性段之前新增 `@JsonCreator` + `@ConstructorProperties` 继承兼容性说明段落，解释 Lombok `@Jacksonized` 与手动构造器两种模式下的基类字段传递机制 |
| **[重要] 问题5：** `LlmChatService.structuredChat()` 未定义 tool_use/function_call 的 Tool 定义输入传递方式、JSON mode 检测时机、失败回退超时叠加风险 | (1) `LlmChatRequest` 字段级契约新增 `tools: List<ChatToolDefinition>` 字段定义（含 `ChatToolDefinition` 值对象的内嵌字段）；(2) 新增"JSON mode 检测时机与缓存策略"子段落：运行期首次检测+`Cache<String, Boolean>`（TTL=5 分钟）缓存；(3) 新增"structuredChat() 回退路径超时叠加风险"子段落及三条缓解措施（缓存预判避免无效调用、60%/40% 时间窗口分割、parse() 独立 5s 超时） |
| **[重要] 问题6：** `FallbackAiService.applyStrategies()` 条件开关 `aiPlatformEnabled` 缺少 YAML 配置绑定定义，不同注入方式对测试和部署影响不同 | (1) `aiPlatformEnabled` 推荐 `@Value("${ai.platform.enabled:false}")` 构造器注入，列出优于字段级注入和优于 `AiPlatformConfig` 间接引用的理由；(2) 新增"`ai.platform.enabled` 与 `ai.mock.enabled` 同时激活的风险"子段落：分析 `AiPlatformEnvironmentPostProcessor` 被绕过的场景、缓解措施（FATAL 日志 + WARN 日志）和防御行为 |
| **[中等] 问题7：** §2.1 目录结构中 `ai-impl/` 下无 `thin-adapter/` 子包条目，但 §2.2 依赖关系图中引用了 `thin-adapter/` | (1) §2.1 目录结构新增 `thin-adapter/` 子包，将 6 个薄适配器 CapabilityExecutor 从 `orchestrator/impl/` 移入 `thin-adapter/`；(2) §2.2 依赖规则中 `orchestrator/impl/` 引用同步更新为 `thin-adapter/` |
| **[中等] 问题8：** `StructuredOutputParser.parse()` 方法签名中的参数类型 `LlmResponse` 已在 v17 重构中替换为 `LlmChatResponse`，但 §3.6 接口定义未同步更新 | (1) §2.3 类图中 `StructuredOutputParser.parse()` 签名参数类型从 `LlmResponse` 修正为 `LlmChatResponse`；(2) §3.6 StructuredOutputParser 职责描述中补充说明入参类型为 `LlmChatResponse` 而非已废弃的 `LlmResponse`；(3) §4.6 行为契约中 `llmResponse.text()` 更新为 `llmChatResponse.content` |
| **[中等] 问题9：** 测试策略缺少状态恢复路径验证（熔断器 OPEN→HALF_OPEN→CLOSED、端点健康 UNAVAILABLE→CONNECTED 等）和并发竞争验证（滑动窗口多线程写入、熔断器 CAS 边界竞争等） | (1) 新增 §11.4 "状态恢复路径验证"子节，包含 5 组组件的完整状态机恢复路径验证表及验证方式说明；(2) 新增 §11.5 "并发竞争验证"子节，包含 5 个竞争场景（滑动窗口多线程写入、熔断器 CAS 边界竞争、熔断器 record 竞争、端点健康状态并发转换、`CompletableFuture.orTimeout()` 竞态）的验证目标与测试方法表 |

## 修订说明（v4 — 本轮修订）

| 审查意见 | 修改措施 |
|---------|---------|
| **[重要] 问题1：** §3.11 覆盖范围与需求不对称 — 标题为"Phase 5 首次落地底座能力特化设计"，仅覆盖 4 项能力，但 PrescriptionAssist 源自 Phase 2~4 迁移而非首次落地，而真正的 3 项迁移能力（Triage、PrescriptionCheck、MedicalRecordGen）缺少同等级别的 9 维度特化设计表 | §3.11 标题改为"Phase 5 底座能力特化设计"，范围扩展至全部 7 项底座能力。新增 3 项迁移能力（Triage、PrescriptionCheck、MedicalRecordGen）的 9 维度特化设计表；PrescriptionAssist 从"首次落地"重新归类为"继承能力"（源自 Phase 2~4 已有实现）；KbQuery、Schedule、DiscussionConclusion 保持"首次落地能力"分类；共同约束从"4 项"更新为"7 项" |
| **[严重] 问题2/3：** `@Qualifier("{capabilityId}Strategies")` 无法在 Spring 容器中正确解析 + 降级策略注入机制存在两套并行且互斥的描述 | 全局删除所有 `@Qualifier("{capabilityId}Strategies")` 占位符注入伪代码，统一为 Map 方案：`AbstractCapabilityExecutor` 构造器注入 `Map<String, List<DegradationStrategy>>`，在 `execute()` 中按 `getCapabilityId()` 查找对应策略列表。涉及位置：§3.1 降级策略注入机制段（剔除 @Qualifier 推导规则，改为 Map 方案描述）；§3.1 构造器伪代码（`@Qualifier("{capabilityId}Strategies") List<DegradationStrategy>` → `Map<String, List<DegradationStrategy>>`）；§3.1 薄适配器构造器示例（`@Qualifier("diagnosisStrategies")` → `Map<String, List<DegradationStrategy>>`）；§3.8 策略自动注册抑制段（同步修正为 Map 注入表述）|
| **[重要] 问题4：** 底座能力条件化注册机制描述与实际设计不一致 — §3.1 声称"在 AiPlatformConfig 中统一注册条件"但 §3.9 未显式注册任何 CapabilityExecutor | §3.1 条件注册保护段统一为"所有 CapabilityExecutor 通过 `@Component` 自注册 + `@ConditionalOnProperty` 条件化激活"，删除"在 AiPlatformConfig 中统一注册"的表述 |
| **[重要] 问题5：** 薄适配器 `doExtractDepartmentId()` 回退描述引用不存在的 Prompt 模板 — "Prompt 模板回退到通用模板"不适用于薄适配器 | §3.1 departmentId 提取策略段将"Prompt 模板回退到通用模板"改为"departmentId 保持 null，对应字段在 AiCallRecord 中可空" |
| **[重要] 问题7：** `ExperimentGroup` 实体未被正式定义 — Experiment 类包含 `groups: List<ExperimentGroup>` 但无 ExperimentGroup 的字段表、JPA 注解或定义 | §3.4 新增 `ExperimentGroup` 完整定义，包含字段表（id/experiment/group_id/percentage/target_model_id/target_prompt_version）、JPA 关联关系（`@ManyToOne`）、流量分配算法约束（千分比 0-1000、总和 = 1000、哈希分桶累积匹配规则） |
| **[重要] 问题8：** `AiCallLogStats` 汇总统计表未被正式定义 — 分区清理策略提及但无字段结构、索引策略或定义 | §3.5 新增 `AiCallLogStats` JPA Entity 定义，包含字段表（capability_id/stat_month/total_calls/success_count/degraded_count/failure_count/avg_elapsed_ms/p50_elapsed_ms/p95_elapsed_ms/p99_elapsed_ms）、索引策略、聚合 SQL 伪代码 |
| **[重要] 问题9：** `PrescriptionLocalRuleFallback` 未定义"最小安全规则"的具体内容 — 仅描述为"执行本地规则校验最小检查项" | §3.7 新增 `PrescriptionLocalRuleFallback` 完整行为契约，包含 5 项最小安全规则（配伍禁忌检查、剂量范围检查、重复用药检查、过敏史检查、儿童/孕妇用药警示）各含规则描述和数据来源 + 执行失败时的安全策略（白名单模式：数据不可用时默认"通过"以确保处方流程不阻塞）|
| **[重要] 问题10：** `ModelRoute.parameters` 未定义扩展点 — LlmChatOptions 两阶段填充仅定义 temperature/maxTokens/stopSequences 三个 key，topP/frequencyPenalty/presencePenalty 被静默忽略 | §3.2 LlmChatOptions 字段级契约新增 `topP`、`frequencyPenalty`、`presencePenalty` 三个字段；新增"参数映射扩展点规则"子段落，说明白名单显式映射模式及 `extensionParameters` 兜底字段方案；阶段一映射 key 列表中补充三个新字段 |
| **[中等] 问题11：** DiscussionConclusionCapabilityExecutor 的 transcriptSummary 前置 LLM 调用未定义超时和失败策略 | §3.11.8 模板变量提取策略单元格中补充前置 LLM 调用完整契约：使用的 `LlmChatService` 方法（`chat()` 非 structuredChat）、独立超时（`transcriptSummaryTimeout` 默认 15s）、失败回退策略（截断 transcripts 至 2000 tokens 作为 `transcriptSummary`）|
| **[中等] 问题12：** `extractCallerRole()` 未定义 Spring Security GrantAuthority 的过滤规则 — 取第一个 GrantedAuthority，无 ROLE_ 前缀处理和多 authority 场景拼接规则 | §3.1 `extractCallerRole()` 伪代码新增 ROLE_ 前缀过滤规则：优先遍历 authority 列表查找第一个以 "ROLE_" 开头的 authority 并去除前缀；不存在 ROLE_ 前缀时取第一个 authority 原始值 |
