...
| 缺失维度 | 影响范围 | 根因 | 预期补齐方式 |
|---------|---------|------|------------|
| `modelId` | 6 项薄适配器成功/降级路径均传入 null（见 §4.2 行 3116 和 §4.2 降级路径） | 薄适配器不经过底座模型路由，Phase 4 服务内部自行选择模型且当前接口不暴露模型标识 | Phase 4 服务接口新增可选 `getUsedModelId(): String` 元数据暴露方法；补齐前 AiCallRecord.modelId=null |
| `promptVersion` | 6 项薄适配器所有路径传入 null（成功/降级/超时均无实验分流） | 薄适配器不经过底座实验分流，Phase 4 服务内部无 Prompt 版本概念 | Phase 4 服务接口新增可选 `getUsedPromptVersion(): Integer` 元数据暴露方法；补齐前 AiCallRecord.promptVersion=null |
| `retryCount` | 6 项薄适配器成功路径硬编码为 0 | 底座无法感知 Phase 4 服务内部重试次数 | Phase 4 服务接口新增可选 `getRetryCount(): int` 元数据暴露方法；补齐前 AiCallRecord.retryCount=0 |
...
│           └── Phase4ServiceMetaProvider.java # 新增可选元数据接口（Phase 4 服务选择性实现，实现后底座可通过 instanceof 获取模型/版本/重试元数据）
│
...
**Phase 4 服务可选元数据接口契约**：为补齐 §1.4.1 所述模型/版本/重试三维度分析缺口，薄适配器调用方可尝试加载 Phase 4 服务实现的可选元数据接口 `Phase4ServiceMetaProvider`。该接口定义如下（**可选契约**，Phase 4 服务可选择实现或不实现，底座通过 `instanceof` 检测后安全调用）：

```java
// 可选元数据接口 — 薄适配器底座通过 instanceof 检测后安全调用
// 归属：ai-api/dto/base/ 包（底座侧定义，Phase 4 服务选择性实现）
// 迁移理由：接口定义在 ai-impl/thin-adapter/ 将导致 Phase 4 模块反向依赖 ai-impl（当前 ai-impl → Phase 4 为单向 provided 依赖）。
// 迁移至 ai-api/dto/base/ 后，Phase 4 模块已依赖 ai-api，可实现接口而不引入新依赖方向。
interface Phase4ServiceMetaProvider {
    /**
     * 返回最近一次内部调用使用的模型标识。
     * @return 模型标识字符串，服务未记录模型标识时返回 null
     */
    default String getUsedModelId() { return null; }

    /**
     * 返回最近一次内部调用使用的 Prompt 版本号。
     * @return Prompt 版本号，服务无 Prompt 版本概念时返回 null
     */
    default Integer getUsedPromptVersion() { return null; }

    /**
     * 返回最近一次内部调用的重试次数。
     * @return 内部重试次数，服务无重试机制时返回 0
     */
...
调用约定：
1. 薄适配器 `doExecuteInternal()` 中，在调用 `phase4ServiceDelegate.execute(request)` 返回后，通过 `phase4ServiceDelegate instanceof Phase4ServiceMetaProvider` 检测是否实现该接口
2. 若未实现：modelId=null、promptVersion=null、retryCount=0
3. 若实现：调用 `((Phase4ServiceMetaProvider) phase4ServiceDelegate).getUsedModelId()` 等方法获取元数据后传入 `AiCallRecord` 工厂方法
4. 元数据查询本身不抛异常（default method 返回安全默认值），不存在因元数据查询导致薄适配器管线失败的风险
5. 薄适配器伪代码中在 `doExecuteInternal()` 成功路径返回前新增元数据提取步骤（见 §4.2 薄适配器特化管线伪代码注释补充）

**元数据补齐优先级与时间线**：此接口为可选优化项，不阻塞底座切流。Phase 4 模块在跨包协作会议（§3.1「跨包协作会议的时间线与验收标准」中定义的最晚截止时间）中确认是否实现。若 Phase 4 模块选择不实现，三个维度持续为 null/0，底座侧在 §1.4.1 记录此缺口并在指标报表中以缺失维度聚合。
...
doExecuteInternal(startTime, request, capabilityId, departmentId, userId, sessionId, callerRole, callerId,
    ...
    try {
      ...
      // 按 §3.1 Phase 4 模块异常契约表区分业务异常与基础设施异常：
      //   6 项 Phase 4 模块各有独立异常类型（DiagnosisException 等），无公共 BusinessException 基类
      //   此处通过类名匹配 6 个已知异常类型识别业务异常
      if originalType in ["DiagnosisException", "InspectionException", "LabTestException",
                          "ImageAnalysisException", "ExaminationException", "ExecutionOrderException"]:
        // 业务异常（如参数校验失败、数据不存在）：直接返回失败，不走降级路径
        // 标准化错误码：回退到"PHASE4_" + 异常类名作为错误码
...
DiscussionConclusionCapabilityExecutor.doExecuteInternal(startTime, request, capabilityId,
    ...
    estimatedTokens = estimateTokens(transcripts)  // 按字符数/Token 比例粗略估算
        ...
        try:
            compressedFuture = CompletableFuture.supplyAsync(() -> {
                ...
                compressRequest = new LlmChatRequest(
                    ...
                    options: new LlmChatOptions(), clientType: /* 从 ModelRouter 获取或使用默认 */)
...
ThinAdapterCapabilityExecutor.doExecuteInternal(startTime, request, capabilityId, departmentId, userId, sessionId, callerRole, callerId, visitId, patientId, inputSummary):
    ...
    try:
     ...
     elapsedMs = System.currentTimeMillis() - startTime
    outputSummary = StringUtils.truncate(result.toString(), 500)
    // ⚠️ 薄适配器元数据提取：通过 instanceof 检测 Phase 4 服务是否实现可选元数据接口
    // 若未实现（底座切流初期常态），modelId=null、promptVersion=null、retryCount=0
    // 若已实现，则从 Phase4ServiceMetaProvider 获取内部调用元数据
    modelId = null
    promptVersion = null
    retryCount = 0
    if phase4ServiceDelegate instanceof Phase4ServiceMetaProvider:
        provider = (Phase4ServiceMetaProvider) phase4ServiceDelegate
        modelId = provider.getUsedModelId()     // 可能为 null
        promptVersion = provider.getUsedPromptVersion()  // 可能为 null
        retryCount = provider.getRetryCount()   // 默认为 0
    metricsCollector.record(AiCallRecord.success(capabilityId, LocalDateTime.now(), elapsedMs,
