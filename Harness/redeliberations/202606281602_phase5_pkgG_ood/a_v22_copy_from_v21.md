...
// 参数说明：callContext 由 execute() 入口处构建（聚合 departmentId/callerRole/callerId/visitId/patientId/sessionId/inputSummary/outputSummary/promptVersion），
// 方法体内通过 callContext.getDepartmentId()/getCallerRole()/... getter 获取各字段值，不再访问 ThreadLocal 或 DTO getter
// 子类可通过 callContext.withOutputSummary(newOutputSummary) 生成副本传递新值，保持 callContext 不可变性
...
doDegrade(startTime, degradeReason, request, capabilityId, modelId, sentinelReason, callContext):
  elapsedMs = System.currentTimeMillis() - startTime
  // outputSummary 可能尚未在 callContext 中设置（降级路径中各调用点传入的 callContext 可能为原始副本），
  // 尝试从 callContext.getOutputSummary() 获取，若为空则后续从 fallback 结果推导
  effectiveOutputSummary = callContext.getOutputSummary()
  if localRuleFallback != null:
    ...
    if result == null:
        ...
        metricsCollector.record(AiCallRecord.degraded(capabilityId, LocalDateTime.now(), elapsedMs,
            degradeReason, modelId, callContext, sentinelReason))
    ...
    effectiveCallContext = callContext.withOutputSummary(effectiveOutputSummary)
    metricsCollector.record(AiCallRecord.degraded(capabilityId, LocalDateTime.now(), elapsedMs,
        degradeReason, modelId, effectiveCallContext, sentinelReason))
    slidingWindowMetricsStore.recordDegraded(capabilityId, elapsedMs)
    return result
  else:
    metricsCollector.record(AiCallRecord.degraded(capabilityId, LocalDateTime.now(), elapsedMs,
        degradeReason, modelId, callContext.withOutputSummary(effectiveOutputSummary), sentinelReason))
    slidingWindowMetricsStore.recordFailure(capabilityId)
    return AiResult.degraded(degradeReason)
```
...
ThinAdapterCapabilityExecutor.doExecuteInternal(startTime, request, capabilityId, departmentId, userId, sessionId, callerRole, callerId, visitId, patientId, inputSummary):
    ...
    try:
        ...
        //   - 6 项 Phase 4 模块的业务异常统一继承 `Phase4BusinessException` 公共基类
        //     （归属 ai-api/dto/base/，新增抽象类，Phase 4 模块按其继承）
        //   - 薄适配器通过 instanceof 检测，消除字符串匹配在异常类重命名时的维护脆弱性
        //   - 所有 Phase 4 业务异常已约定继承 RuntimeException
