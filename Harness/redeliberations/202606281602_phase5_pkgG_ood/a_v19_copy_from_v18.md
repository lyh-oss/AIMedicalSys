    ...
    try:
      ...
      try:
        ...
        if chatAiResult.isSuccess():
          ...
          // 回退场景下 retryCount 偏低。此差异为已知的可观测性精度约束：
    ...
    if estimatedTokens > 3000:
        ...
        try:
            compressedFuture = CompletableFuture.supplyAsync(() -> {
                ...
                // 模型选择对主流程的影响有限。
                // 轻量模型配置通过 @Value("${ai.compression.lightweight-endpoint:compress-default}") 注入，
                // 对应 YAML 中 ai.router.routes.DISCUSSION_CONCLUSION_COMPRESS 配置项（可选），
                // 未配置时使用 LlmChatOptions 默认值 + ClientType.HTTP_API 作为回退。
                compressEndpoint = compressionLightweightEndpoint  // 轻量模型端点标识
                compressClientType = compressionLightweightClientType // 轻量模型客户端类型（默认 HTTP_API）
