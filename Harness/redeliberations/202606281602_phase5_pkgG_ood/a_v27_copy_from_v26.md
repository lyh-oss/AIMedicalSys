...
DiscussionConclusionCapabilityExecutor.doExecuteInternal(startTime, request, capabilityId,
    ...
    // 后续复用标准管线流程（实验分流→模板渲染→模型路由→LLM 调用→结构化解析→指标采集）
    // 以下为精简后的标准流程（实际实现中通过 executeStandardPipeline() 调用父类 protected 非抽象方法，
    // 此处展开以体现完整行为契约）：
    // ⚠️ 注意：doExecuteInternal() 在 AbstractCapabilityExecutor 中声明为 abstract，
    // 因此 DiscussionConclusionCapabilityExecutor 无法通过 super.doExecuteInternal() 调用父类实现。
    // 父类提供 protected final executeStandardPipeline() 方法封装标准步骤（实验分流→模板渲染→模型路由→
    // LLM 调用→结构化解析→指标采集），子类在 doExecuteInternal() 的前置阶段（如压缩摘要）完成后
    // 调用 executeStandardPipeline() 继续标准流程。
    boolean experimentAssignFailed = false
    try:
        assignment = experimentManager.assign(capabilityId, userId, sessionId)
    catch (Exception e):
