...
doExecuteInternal(startTime, request, capabilityId, departmentId, userId, sessionId, callerRole, callerId, visitId, patientId, inputSummary):
    ...
    try:
      ...
      try:
        ...
        if aiResult.isSuccess():
          ...
          metricsCollector.record(AiCallRecord.failure(capabilityId, LocalDateTime.now(), elapsedMs,
              ...
              callerRole, callerId, promptVersion))
          slidingWindowMetricsStore.recordFailure(capabilityId)
          return doDegrade(startTime, DegradationReason.INTERNAL_ERROR, request, capabilityId, departmentId, callerRole, callerId, visitId, patientId, sessionId, inputSummary, null, promptVersion)
        ...
        if originalCause instanceof StructuredOutputNotSupportedException:
          ...
          elapsedMs = System.currentTimeMillis() - startTime
          endpointHealthManager.recordCallResult(endpointId, false, elapsedMs)
          return doDegrade(startTime, DegradationReason.INFRASTRUCTURE_ERROR + ":" + originalCause.getClass().getSimpleName(), request, capabilityId, departmentId, callerRole, callerId, visitId, patientId, sessionId, inputSummary, null, promptVersion)
      ...
      try:
        ...
        elapsedMs = System.currentTimeMillis() - startTime
        endpointHealthManager.recordCallResult(endpointId, false, elapsedMs)
        return doDegrade(startTime, DegradationReason.INFRASTRUCTURE_ERROR + ":" + ce.getCause().getClass().getSimpleName(), request, capabilityId, departmentId, callerRole, callerId, visitId, patientId, sessionId, inputSummary, null, promptVersion)
