...
本设计实现包D-AI1（处方审核）prescription 模块审核子域的全部代码，覆盖 REST 端点、业务层 DTO、Service 层、本地规则引擎、JPA 实体、Repository 和 Converter，对齐需求文档 3.4.2 契约和 OOD §3.2/§4.2 行为规范。**v11 为 RETRY 修订**：修复父 POM `dependencyManagement` 缺少 `patient` 模块条目导致 Maven 构建失败的问题。
...
| `backend/pom.xml` | **修改** | 在 `<dependencyManagement>` 的 `application` 条目之后、`External libraries` 注释之前，添加 `com.aimedical:patient` 版本声明 |

注：`prescription/pom.xml` 已在 v10 中正确添加了 `com.aimedical:patient` 依赖（无 `<version>`，继承自父 POM），桌面无需调整。
...
- `void revoke(Long auditId)` — 撤销审核记录
...
- `revoke()` 被 PrescriptionAuditController 调用；依赖 AuditRecordRepository
...
### 变更位置
`<dependencyManagement><dependencies>` 块中，在 `application` 条目之后（第 77-81 行）、`<!-- External libraries -->` 注释之前（第 82 行），插入 `patient` 模块的 `<dependency>` 条目。
...
**不变更**：`prescription/pom.xml` 已正确定义 `com.aimedical:patient` 依赖（无 `<version>`，继承自父 POM），无需修改。parent POM `modules` 列表已包含 `<module>modules/patient</module>`，`dependencyManagement` 是唯一缺失项。

## 修订说明（v10 r1–r5）

（保留 v10 全部历史修订，从略；完整记录见 detail_v10.md）

## 修订说明（v11 r1）

| 审查意见 | 修改措施 |
|---------|---------|
| [严重] Maven 构建失败：`'dependencies.dependency.version' for com.aimedical:patient:jar is missing`，parent POM 的 `dependencyManagement` 缺少 `patient` 模块 | 在 `AIMedical/backend/pom.xml` 的 `<dependencyManagement>` 中，`application` 条目之后、`<!-- External libraries -->` 注释之前，添加 `com.aimedical:patient` 依赖（版本 `${project.version}`） |
